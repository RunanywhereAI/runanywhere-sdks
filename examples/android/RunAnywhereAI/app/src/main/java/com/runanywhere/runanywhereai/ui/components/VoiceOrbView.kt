...
// VoiceOrbView
//
// Optimized particle animation for the voice assistant orb.
// Performance-focused for smooth 60fps on low-memory devices.
//
// Ported from the backup app's VoiceAssistantParticleView with
// the new app's accent colors (orange primary palette).
//
// Features:
...
private inline fun fastHash(v: Float): Float {
    ...
    return (SIN_LUT[idx] * 0.5f + 0.5f) // map -1..1 to 0..1
}
...
private fun buildFrameState(
    ...
    width: Float,
    height: Float,
    baseColor: Triple<Float, Float, Float>,
    activeColor: Triple<Float, Float, Float>,
    isDarkMode: Boolean,
): FrameState {
    ...
    return FrameState(
        ...
        activeR = activeColor.first,
        activeG = activeColor.second,
        activeB = activeColor.third,
...
/**
 * Animated orb made of particles on a Fibonacci sphere.
 *
 * @param amplitude Audio level 0..1 driving expansion.
 * @param morphProgress 0 = sphere, 1 = ring.
 * @param scatterAmount Touch scatter intensity (decays over time).
 * @param touchPoint Normalized touch position for scatter origin.
 * @param isDarkMode Whether dark theme is active (affects brightness).
 */
@Composable
fun VoiceOrbCanvas(
    amplitude: Float,
    ...
    // Derive colors from MaterialTheme (orange primary palette)
    val primaryColor = MaterialTheme.colorScheme.primary
    val tertiaryColor = MaterialTheme.colorScheme.tertiary

    // Convert theme colors to RGB triples for per-particle math
    val baseColorTriple = remember(primaryColor, isDarkMode) {
        if (isDarkMode) {
            Triple(primaryColor.red * 0.7f, primaryColor.green * 0.55f, primaryColor.blue * 0.4f)
        } else {
            Triple(primaryColor.red * 0.6f, primaryColor.green * 0.4f, primaryColor.blue * 0.3f)
        }
    }
    val activeColorTriple = remember(primaryColor) {
        Triple(primaryColor.red, primaryColor.green * 0.7f, primaryColor.blue * 0.3f)
    }
    ...
    Canvas(modifier = modifier) {
        ...
        val frame = buildFrameState(
            ...
            width = size.width,
            height = size.height,
            baseColor = baseColorTriple,
            activeColor = activeColorTriple,
 ...
 * Colors are quantized to ~32 levels per channel to group similar particles.
 */
