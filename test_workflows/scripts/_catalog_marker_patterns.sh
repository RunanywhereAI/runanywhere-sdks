#!/usr/bin/env bash
# Catalog §10 log grep patterns — shared by analyzers and iter-5/8 regrade scripts.
...
# --- TC-04 LLM load ---
RAC_REGEX_TC04_LOAD='LLM model loaded|Model load succeeded for|ModelLifecycle.*Model load succeeded|✅ Model load succeeded for|✅ LLM model loaded:|Found downloaded chat model|\[ChatScreen\] Text model loaded: true|Voice agent LLM model loaded:|load_model SUCCESS|✅ LLM models registered|LLM models registered'
...
RAC_REGEX_TC07_STT='STT model loaded successfully|STT model loaded: true|\[STTScreen\] STT model loaded: true|✅ STT model loaded|Sherpa\.STT.*STT model loaded successfully|Voice agent STT model loaded:|Batch transcription complete|✅ Batch transcription complete'
...
# --- TC-09 VLM ---
RAC_REGEX_TC09_VLM='VLM streaming completed|VLM processing complete|Starting VLM streaming|Frame description completed|VLM model loaded: true|VLM model loaded successfully'

# --- TC-10 STT UI ready ---
RAC_REGEX_TC10_STT_UI='Ready to transcribe|STT auto-prepare started|STT auto-prepare skipped'

# --- TC-13 RAG ingest / query ---
RAC_REGEX_TC13_RAG='Document loaded successfully|Embedding generation complete|Query complete|ragIngest|ragQuery'
...
export RAC_REGEX_TC10_STT_UI RAC_REGEX_TC13_RAG RAC_REGEX_TC01_INIT
...
rac_catalog_collect_logs() {
  ...
  while IFS= read -r -d '' f; do
    ...
    \) ! -path '*/iter5-regrade/*' ! -path '*/iter8-regrade/*' \
