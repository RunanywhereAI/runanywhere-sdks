...
| `07_web` | Chrome/Safari version; `web/capture-web-logs.sh init`; `vite-start`; wait for `http://127.0.0.1:5173`; build WASM if missing | catalog §8 incl. TC-Storage-OPFS; cursor-ide-browser `browser_navigate` → clear site storage → `browser_lock` | TC-06, TC-17, TC-18, TC-19, TC-21 | None — runs independently |
