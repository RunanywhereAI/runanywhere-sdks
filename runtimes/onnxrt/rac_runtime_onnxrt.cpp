...
rac_result_t apply_active_ep(const OrtApi *api,
...
rac_result_t Session::run(const TensorInput *inputs, size_t input_count,
  ...
  OrtStatus *status = api->CreateCpuMemoryInfo(OrtArenaAllocator,
  ...
  for (size_t i = 0; i < input_count; ++i) {
    if (!inputs[i].name || !inputs[i].data || !inputs[i].shape ||
        inputs[i].rank == 0) {
      cleanup_inputs();
      return RAC_ERROR_NULL_POINTER;
    }
    ...
    status = api->CreateTensorWithDataAsOrtValue(
        memory_info, const_cast<void *>(inputs[i].data), inputs[i].data_bytes,
        inputs[i].shape, inputs[i].rank, to_ort_type(inputs[i].type),
        &input_values[i]);
    if (status != nullptr) {
      if (out_error)
        *out_error = status_message(api, status);
  ...
  std::vector<const OrtValue *> const_input_values(input_values.begin(),
                                                   input_values.end());
  std::vector<OrtValue *> output_values(output_count, nullptr);
  status = api->Run(impl_->session, nullptr, input_names.data(),
                    const_input_values.data(), input_count, output_names,
...
rac::runtime::ProviderRegistry<rac_onnxrt_runtime_provider_t> &
onnxrt_provider_registry() {
...
rac_result_t onnxrt_run_session(rac_runtime_session_t *session,
  ...
  if (session->is_provider_session) {
    ...
    return session->provider.run_session(session->provider_session, inputs,
  ...
  std::string error;
  rac_result_t rc = session->session->run(
      runtime_inputs.data(), runtime_inputs.size(), output_names.data(),
...
rac_result_t onnxrt_run_session_v2(rac_runtime_session_t *session,
                                   ...
                                   size_t n_in, rac_runtime_tensor_t *outputs,
                                   size_t n_out) {
  if (!session)
    return RAC_ERROR_INVALID_HANDLE;
  if (n_in > 0 && !inputs)
...
rac_result_t onnxrt_map_buffer(rac_runtime_buffer_t *buffer, size_t offset,
...
rac_result_t onnxrt_copy_buffer(rac_runtime_buffer_t *dst, size_t dst_offset,
  ...
  if (!dst || !src)
    return RAC_ERROR_NULL_POINTER;
  return rac::runtime::rac_runtime_copy_buffer(dst->data, dst->bytes,
                                               dst_offset, src->data,
...
rac_result_t onnxrt_capabilities(rac_runtime_capabilities_t *out) {
  ...
  *out = rac_runtime_capabilities_t{};
  ...
  out->supported_formats_count =
      sizeof(k_supported_formats) / sizeof(k_supported_formats[0]);
  ...
  onnxrt_provider_registry().for_each(
...
const rac_runtime_vtable_t *runtime_vtable() { return &k_onnxrt_vtable; }
...
extern "C" RAC_API void
rac_onnxrt_runtime_unregister_provider(const char *name) {
...
extern "C" RAC_API rac_result_t rac_onnxrt_runtime_enable_execution_provider(
  ...
  runanywhere::runtime::onnxrt::ep_state().set(config->type,
                                               config->config_json);
  return RAC_SUCCESS;
}

extern "C" RAC_API rac_result_t
rac_onnxrt_runtime_get_active_ep(rac_onnxrt_ep_type_t *out) {
  if (out == nullptr)
    return RAC_ERROR_NULL_POINTER;
...
extern "C" RAC_API int
rac_onnxrt_runtime_ep_is_available(rac_onnxrt_ep_type_t type) {
