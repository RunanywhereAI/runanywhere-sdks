...
- The registry accepts **ABI v2 only**. `metadata.abi_version` must equal `RAC_RUNTIME_ABI_VERSION` exactly, or registration is **hard-rejected** with `RAC_ERROR_ABI_VERSION_MISMATCH` — a distinct check from the `reserved_slot_0` presence check below, which fails with a different code.
- The v2 extension `rac_runtime_vtable_v2_t` (`rac_runtime_vtable.h:291-318`) hangs off `reserved_slot_0` and carries `run_session_v2` + device-aware buffer ops (`alloc_buffer`/`buffer_info`/`map_buffer`/`unmap_buffer`/`copy_buffer`) + `release_tensor`. Probe it via the inline `rac_runtime_vtable_get_v2()`. **`reserved_slot_0` is REQUIRED, not optional** — `rac_runtime_registry.cpp::validate_v2_extension` hard-rejects a NULL `reserved_slot_0` with `RAC_ERROR_INVALID_PARAMETER` (a header comment on `rac_runtime_vtable.h` claims a session-less runtime "may leave `reserved_slot_0` NULL (as onnxrt does)" — that comment is stale; onnxrt's actual vtable sets `reserved_slot_0 = &k_onnxrt_vtable_v2`). Capability-only runtimes (coreml, onnxrt) instead ship a v2 table with every op slot NULL via the `RAC_RUNTIME_VTABLE_V2_CAPABILITY_ONLY` initializer.
...
| **onnxrt** | `runtimes/onnxrt/` | No (slots NULL) | **Not set** | `onnx` engine (EMBED + SEGMENT) | The engine calls the C++ `runanywhere::runtime::onnxrt::Session` class (`Session::create`/`run`) directly; the vtable half publishes runtime capability and availability. Links real `onnxruntime`. | **Live (library), one engine consumer** |
...
- **coreml** — a capability runtime **plus a bag of MLModel loader helpers** (`runtimes/coreml/rac_runtime_coreml.mm:141-255`). Real consumer: the **`neurt` engine** via those loaders + `rac_coreml_runtime_require_available`, in `NeuRT/src/sdk/rac_diffusion_coreml.mm` (that source lives in the sibling `neurun` repo; the SDK compiles it). **The name collision is gone:** the engine used to be called `coreml` too, and the pair was the standing example of "an engine uses a same-named device runtime". Since the engine became `neurt` (registering `rac_plugin_entry_neurt` / `RAC_STATIC_PLUGIN_REGISTER(neurt)`, `engines/neurt/rac_static_register_neurt.cpp`) only the runtime carries the framework name (`rac_runtime_entry_coreml`, `runtimes/coreml/rac_runtime_coreml.mm:262-266`). The separate dirs/registries/symbols were always what made it work; the shared name was never load-bearing.
...
**Pattern 3 — Engine IS our own code on a device-runtime.** The engine is RunAnywhere code that runs on a device runtime via that runtime's loader/helpers. `neurt` declares `RAC_RUNTIME_COREML` + `RAC_RUNTIME_ANE` (`engines/neurt/rac_plugin_entry_neurt.cpp`) and calls the Core ML loader helpers. (Contrast `cloud` engines: no runtime / pure HTTP — they declare no device runtime.)
...
2. **Create `runtimes/<name>/`** with `CMakeLists.txt` + `rac_runtime_<name>.cpp` (`.mm` for Apple frameworks). Fill a `rac_runtime_vtable_t` in `.rodata`: `metadata` (`abi_version = RAC_RUNTIME_ABI_VERSION`, a `rac_runtime_id_t`, `name`, `priority`, formats, devices), mandatory `init`/`destroy`; optional `device_info`/`capabilities` (both MAY be NULL, see the table above), session slots only if applicable, and a v2 extension on `reserved_slot_0` — **this slot is REQUIRED** (the registry rejects a NULL `reserved_slot_0`); if capability-only, point it at an all-NULL-op-slots table built with `RAC_RUNTIME_VTABLE_V2_CAPABILITY_ONLY`, exactly as both `coreml` and `onnxrt` do.
...
- C ABI surface prefixed `rac_`, types `_t`, errors `RAC_ERROR_*`, macros `RAC_*`. C++20 internals, pure C at the boundary. Run `core/scripts/lint-cpp.sh` (`--fix`) before committing.
