...
int main(int argc, char* argv[]) {
    ...
    // Pointers for callbacks (set after construction, captured by reference in lambdas)
    openclaw::WaitingChime* waiting_chime_ptr = nullptr;
    openclaw::VoicePipeline* pipeline_ptr = nullptr;

    // Wake word acknowledgment pointer (set after construction below)
    openclaw::WaitingChime* listening_ack_ptr = nullptr;

    // Wake word callback -- play "I'm listening" so the user knows we heard them.
    // Fire-and-forget on a detached thread so it doesn't block the capture thread
    // (blocking capture would lose audio of the user's command).
    pipeline_config.on_wake_word = [&listening_ack_ptr](const std::string& wake_word, float confidence) {
        std::cout << "\n*** Wake word detected: \"" << wake_word
                  << "\" (confidence: " << confidence << ") ***\n"
                  << "[Listening for command...]" << std::flush;
        // Play "I'm listening" on a separate thread (non-blocking)
        if (listening_ack_ptr) {
            auto* ack = listening_ack_ptr;
            std::thread([ack]() { ack->play_once(); }).detach();
        }
    };
    ...
    std::cout << "Initializing audio feedback...\n";

    // Shared playback callback for both chime and acknowledgment
    auto play_audio_cb = [&playback](const int16_t* samples, size_t num_samples, int sample_rate) {
        if (static_cast<uint32_t>(sample_rate) != playback.config().sample_rate) {
            playback.reinitialize(sample_rate);
        }
    ...
    // Waiting chime: played on repeat while waiting for OpenClaw response
    std::string earcon_path = openclaw::get_earcon_path();
    openclaw::WaitingChime waiting_chime(earcon_path, play_audio_cb);
    waiting_chime_ptr = &waiting_chime;

    // Wake word acknowledgment: "I'm listening" played once when wake word fires
    std::string ack_path = openclaw::get_listening_ack_path();
    openclaw::WaitingChime listening_ack(ack_path, play_audio_cb);
    listening_ack_ptr = &listening_ack;

    std::cout << "  Audio feedback ready\n" << std::endl;
