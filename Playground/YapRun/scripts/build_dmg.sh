#!/bin/bash
#
# build_dmg.sh — Build, sign, notarize, and package YapRun.app into a .dmg
#
# Usage:
...
#   KEYCHAIN_PROFILE  — notarytool keychain profile name (default: YapRun-Notarize)
#
...
PROJECT_FILE="$PROJECT_DIR/YapRun.xcodeproj"
SCHEME="YapRun"
APP_NAME="YapRun"
DMG_VOLUME_NAME="YapRun"
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo ""
            echo "One-time setup:"
            echo "  1. brew install create-dmg"
            echo "  2. xcrun notarytool store-credentials \"YapRun-Notarize\" \\"
...
if [ "$SKIP_BUILD" = false ]; then
    step "Archiving $APP_NAME for macOS"
    rm -rf "$BUILD_DIR"
    mkdir -p "$BUILD_DIR"
    ...
    if [ ! -d "$EXPORT_DIR/$APP_NAME.app" ]; then
        fail "Export failed — $APP_NAME.app not found in $EXPORT_DIR"
    fi
    echo "  Exported: $EXPORT_DIR/$APP_NAME.app"
else
    step "Skipping build (--skip-build)"
    if [ ! -d "$EXPORT_DIR/$APP_NAME.app" ]; then
        fail "No existing export found at $EXPORT_DIR/$APP_NAME.app"
    fi
fi
...
DMG_FILENAME="${APP_NAME}-${APP_VERSION}-mac.dmg"
...
echo "       gh release create v$APP_VERSION-mac \"$DMG_PATH\" --title \"YapRun $APP_VERSION for macOS\" --notes \"Direct download for macOS.\""
echo "    3. Link from runanywhere.ai/yaprun"
echo ""
