...
#   2. Copy: cp -R dist/RunAnywhereCore.xcframework packages/runanywhere_native/ios/Frameworks/
#
...
Pod::Spec.new do |s|
  ...
  s.version          = '0.15.8'
  s.summary          = 'RunAnywhere: Native binaries for on-device AI'
  s.description      = <<-DESC
Privacy-first, on-device AI SDK for Flutter. This package provides the native
binaries (XCFramework) for speech-to-text (STT), text-to-speech (TTS), language
models (LLM), voice activity detection (VAD), and embeddings.
  ...
  s.prepare_command = <<-CMD
    ...
    if [ "$TEST_LOCAL" = "false" ]; then
      ...
      if [ ! -d "Frameworks/RunAnywhereCore.xcframework" ]; then
        ...
        echo "  2. cp -R dist/RunAnywhereCore.xcframework packages/runanywhere_native/ios/Frameworks/"
        echo ""
