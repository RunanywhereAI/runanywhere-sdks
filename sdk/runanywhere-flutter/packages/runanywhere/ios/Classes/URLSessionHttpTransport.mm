 ...
 * adapter for the Flutter plugin. Mirrors the Swift SDK reference at
 *   sdk/runanywhere-swift/Sources/RunAnywhere/HttpTransport/URLSessionHttpTransport.swift
 * but is implemented in ObjC++ because the Flutter plugin does NOT ship a
 * `CRACommons` Swift module map — so we cannot import the C ABI from Swift
 * directly. Direct `#include "rac/..."` works because the podspec adds the
...
// =============================================================================
// Public C entry points (called from Swift façade during plugin init)
// =============================================================================
extern "C" {

void ra_flutter_register_urlsession_transport(void) {
...
void ra_flutter_unregister_urlsession_transport(void) {
    std::lock_guard<std::mutex> lock(sRegistrationMutex);
    bool expected = true;
