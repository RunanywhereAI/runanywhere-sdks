...
class AudioPlaybackManager {
  ...
  void _setupListeners() {
    ...
    _playerStateSubscription = _player.onPlayerStateChanged.listen((state) {
      ...
      if (state == PlayerState.completed) {
        _logger.info('🔊 Playback completed');
        _currentTime = 0.0;
    ...
    _durationSubscription = _player.onDurationChanged.listen((duration) {
      ...
      _logger.info('🔊 Audio duration: ${_duration.toStringAsFixed(1)}s');
    });
  ...
  Future<void> _startPlayback(
    ...
    _logger.info(
        '🔊 Starting playback: ${audioData.length} bytes, ${sampleRate}Hz, ${numChannels}ch');
    ...
    try {
      ...
      _logger.info(
          '🔊 Wrote ${wavData.length} bytes to temp file: ${tempFile.path}');
    ...
    if (_currentTempFile != null) {
      try {
        if (await _currentTempFile!.exists()) {
          ...
          _logger.info('🗑️ Cleaned up temp audio file');
        }
      } catch (e) {
        _logger.warning('⚠️ Failed to cleanup temp file: $e');
      }
