...
function createBaseAudioFrameEvent(): AudioFrameEvent {
  return { pcm: Buffer.alloc(0), sampleRateHz: 0, channels: 0, encoding: 0 };
}
...
export const AudioFrameEvent = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          message.pcm = reader.bytes() as Buffer;
          continue;
        case 2:
          if (tag !== 16) {
            break;
  ...
  fromJSON(object: any): AudioFrameEvent {
    return {
      pcm: isSet(object.pcm) ? Buffer.from(bytesFromBase64(object.pcm)) : Buffer.alloc(0),
  ...
  fromPartial<I extends Exact<DeepPartial<AudioFrameEvent>, I>>(object: I): AudioFrameEvent {
    const message = createBaseAudioFrameEvent();
    message.pcm = object.pcm ?? Buffer.alloc(0);
