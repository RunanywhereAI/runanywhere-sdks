...
export class VADError extends SDKError {
  ...
  static notInitialized(): VADError {
    return new VADError(
      ...
      'VAD service not initialized. Call initialize() first.'
    );
  }
  ...
  static initializationFailed(reason: string): VADError {
    return new VADError(
      ErrorCode.NotInitialized,
      `VAD initialization failed: ${reason}`,
      {
        details: { reason },
      }
    );
  ...
  static invalidConfiguration(reason: string): VADError {
    return new VADError(
      ...
      `Invalid VAD configuration: ${reason}`,
      {
        details: { reason },
      }
    );
  ...
  static emptyAudioBuffer(): VADError {
    return new VADError(
      ErrorCode.InvalidInput,
      'Cannot process empty audio buffer'
    );
  }
