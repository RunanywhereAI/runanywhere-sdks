...
// =============================================================================
// SyncHttpDownload — public sync wrapper for the platform-adapter download path.
// Used by HybridRunAnywhereCore::downloadModel (B-RN-3-001 fix) to bypass the
// HTTPS-disabled libcurl runner on Android. Blocks the calling thread until
// the platform download completes or is cancelled.
// =============================================================================
...
// Side map keyed by task id: lets the C-style progress/complete callbacks find
// the function pointer + state pointer associated with a SyncHttpDownload call,
// without needing to extend `runanywhere::bridges::http_download_context`.
std::mutex& syncSideMu() {
    static std::mutex m;
    return m;
}
std::unordered_map<
    std::string,
    std::pair<std::shared_ptr<std::function<void(int64_t, int64_t)>>,
              std::shared_ptr<SyncDownloadState>>>& syncSideMap() {
    static std::unordered_map<
        std::string,
        std::pair<std::shared_ptr<std::function<void(int64_t, int64_t)>>,
                  std::shared_ptr<SyncDownloadState>>> m;
    return m;
}

void syncProgressShim(int64_t downloaded, int64_t total, void* userData) {
    const char* taskCStr = static_cast<const char*>(userData);
    if (!taskCStr) return;
    std::shared_ptr<std::function<void(int64_t, int64_t)>> fn;
    {
        std::lock_guard<std::mutex> lock(syncSideMu());
        auto it = syncSideMap().find(taskCStr);
        if (it != syncSideMap().end()) fn = it->second.first;
    }
    if (fn && *fn) (*fn)(downloaded, total);
}

void syncCompleteShim(rac_result_t resultCode, const char* path, void* userData) {
    const char* taskCStr = static_cast<const char*>(userData);
    if (!taskCStr) return;
    std::shared_ptr<SyncDownloadState> st;
    {
        std::lock_guard<std::mutex> lock(syncSideMu());
        auto it = syncSideMap().find(taskCStr);
        if (it != syncSideMap().end()) st = it->second.second;
    }
    if (!st) return;
    {
        std::lock_guard<std::mutex> lock(st->mu);
        st->result = static_cast<int>(resultCode);
        st->downloadedPath = path ? path : "";
        st->done = true;
    }
    st->cv.notify_all();
}
...
int SyncHttpDownload(
    ...
    auto state = std::make_shared<SyncDownloadState>();
    auto progress = std::make_shared<std::function<void(int64_t, int64_t)>>(onProgress);

    // The static globals `g_http_download_counter`, `g_http_download_mutex`, and
    // `g_http_downloads` live in `runanywhere::bridges` namespace at file scope —
    // visible from within this TU regardless of the namespace we're currently in.
    std::string taskId = "rn_sync_http_" +
        std::to_string(::runanywhere::bridges::g_http_download_counter.fetch_add(
            1, std::memory_order_relaxed));

    {
        std::lock_guard<std::mutex> lock(syncSideMu());
        syncSideMap()[taskId] = {progress, state};
    }

    {
        std::lock_guard<std::mutex> lock(::runanywhere::bridges::g_http_download_mutex);
        ::runanywhere::bridges::g_http_downloads[taskId] =
            ::runanywhere::bridges::http_download_context{
                syncProgressShim,
                syncCompleteShim,
                const_cast<char*>(taskId.c_str())};
    }

    rac_result_t startResult = RAC_ERROR_NOT_SUPPORTED;
#if defined(ANDROID) || defined(__ANDROID__)
    startResult = AndroidBridge::httpDownload(
        url.c_str(), destinationPath.c_str(), taskId.c_str());
#elif defined(__APPLE__)
    startResult = static_cast<rac_result_t>(
        PlatformAdapter_httpDownload(
            url.c_str(), destinationPath.c_str(), taskId.c_str()));
#endif

    if (startResult != RAC_SUCCESS) {
        {
            std::lock_guard<std::mutex> lock(::runanywhere::bridges::g_http_download_mutex);
            ::runanywhere::bridges::g_http_downloads.erase(taskId);
        }
        {
            std::lock_guard<std::mutex> lock(syncSideMu());
            syncSideMap().erase(taskId);
        }
        return static_cast<int>(startResult);
    }

    // Wait for completion, periodically polling cancelFlag.
    bool cancelledByUs = false;
    {
        std::unique_lock<std::mutex> lock(state->mu);
        while (!state->done) {
            if (cancelFlag && cancelFlag->load() && !cancelledByUs) {
#if defined(ANDROID) || defined(__ANDROID__)
                AndroidBridge::httpDownloadCancel(taskId.c_str());
#elif defined(__APPLE__)
                PlatformAdapter_httpDownloadCancel(taskId.c_str());
#endif
                cancelledByUs = true;
            }
    ...
    if (cancelledByUs && state->result == RAC_SUCCESS) {
