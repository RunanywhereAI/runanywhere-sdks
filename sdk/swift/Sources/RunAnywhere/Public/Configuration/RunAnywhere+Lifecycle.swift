...
public struct LLMGenerationResult: Sendable {
    ...
    /// Main-branch parity field. Time from the first `generate` call to the
    /// first streamed token, in milliseconds. `nil` when unknown.
...
public struct LLMStreamingResult: Sendable {
    ...
    /// Awaitable final result — fires after the stream exhausts. Carries
    /// aggregate metrics (TTFT, tokens/sec) matching main. When the caller
    /// creates an instance without an explicit Task, `.value` resolves to
    /// an empty result once the stream completes.
    public let result: Task<LLMGenerationResult, Error>

    public init(stream: AsyncThrowingStream<String, Error>,
                result: Task<LLMGenerationResult, Error>? = nil) {
        self.stream = stream
        self.result = result ?? Task {
            LLMGenerationResult(text: "", tokensUsed: 0,
                                 modelUsed: "", latencyMs: 0,
                                 tokensPerSecond: 0)
        }
    }
}
...
/// Typed per-call STT metadata — matches main's benchmark fields.
public struct STTMetadata: Sendable {
    public var language: String?
    public var realTimeFactor: Double
    public var inputAudioDurationSeconds: Double
    public var provider: String?
    public init(language: String? = nil, realTimeFactor: Double = 0,
                inputAudioDurationSeconds: Double = 0, provider: String? = nil) {
        self.language = language; self.realTimeFactor = realTimeFactor
        self.inputAudioDurationSeconds = inputAudioDurationSeconds
...
public struct STTOutput: Sendable {
    ...
    public let metadata: STTMetadata
    public init(text: String, isFinal: Bool = true, confidence: Float = 1.0,
                metadata: STTMetadata = STTMetadata()) {
        self.text = text; self.isFinal = isFinal
...
public struct TTSOptions: Sendable {
    ...
    /// Alias for `speakingRate` matching main's `rate:` label.
    public var rate: Float {
        get { speakingRate }
        set { speakingRate = newValue }
    }
    /// Synthesis pitch (semitones; 0 = neutral).
    public var pitch: Float
    public init(voice: String = "default", speakingRate: Float = 1.0, pitch: Float = 0) {
        self.voice = voice; self.speakingRate = speakingRate; self.pitch = pitch
    }
    /// Main-branch parity initialiser used by the sample's
    /// `TTSOptions(rate:pitch:)` call site.
    public init(rate: Float, pitch: Float) {
        self.init(voice: "default", speakingRate: rate, pitch: pitch)
    }
}
...
public struct TTSMetadata: Sendable {
    ...
    public var voice: String
    public var provider: String?
    public init(characterCount: Int = 0, voice: String = "default",
                provider: String? = nil) {
        self.characterCount = characterCount
        self.voice = voice; self.provider = provider
    }
}
...
public struct TTSResult: Sendable {
    ...
    public init(pcm: [Float], sampleRateHz: Int,
        ...
        self.duration = duration
            ?? (sampleRateHz > 0 ? Double(pcm.count) / Double(sampleRateHz) : 0)
...
public extension RunAnywhere {
    ...
    // Nullable `getCurrentModelId() -> String?` lives in
    // `Public/Extensions/LLM/RunAnywhere+LLMState.swift` — the main-branch
    // sample shape. Internal call sites that need a `String` fall back to
    // `SessionRegistry.currentModelId` directly.
    ...
    /// Main-branch parity overload — takes `ToolCallingOptions`. Wraps the
    /// canonical `LLMGenerationOptions`-based path and returns a
    /// `ToolCallingResult` so samples can inspect individual tool calls.
    static func generateWithTools(_ prompt: String,
                                    options: ToolCallingOptions) async throws
        -> ToolCallingResult
    {
        var llmOpts = LLMGenerationOptions()
        llmOpts.temperature = options.temperature
        llmOpts.maxTokens = options.maxTokens
        let genResult = try await generateWithTools(prompt, options: llmOpts)
        return ToolCallingResult(text: genResult.text,
