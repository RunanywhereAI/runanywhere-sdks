...
public enum ModelArtifactType: Sendable, Codable, Equatable {
    case singleFile
    case archive(ArchiveFormat, structure: ArchiveStructure = .flat)
    case multiFile
    /// Artifact shipped with the host app (Foundation Models, SystemTTS, …)
    /// — never downloaded, always local.
    case builtIn
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .builtIn:              try c.encode("builtIn",    forKey: .kind)
...
public struct ModelInfo: Sendable, Codable, Identifiable {
    ...
    /// Absolute local path once downloaded, as a URL. Matches main-branch
    /// sample call sites that expect `.path` / `.lastPathComponent`. Backed
    /// by `localPathString` for legacy String-consumers.
    public var localPath: URL?
    /// String form of `localPath` — retained so internal call sites that
    /// type `info.localPath ?? ""` keep working without a churn pass.
    public var localPathString: String? {
        get { localPath?.path }
        set {
            if let s = newValue, !s.isEmpty {
                localPath = URL(fileURLWithPath: s)
            } else {
                localPath = nil
            }
        }
    ...
    public init(id: String, name: String, url: URL? = nil,
        ...
        if let s = localPath, !s.isEmpty {
            self.localPath = URL(fileURLWithPath: s)
        } else {
            self.localPath = nil
        }
        self.files = files
...
public struct LoRAAdapterConfig: Sendable {
    ...
    /// Main-branch parity initialiser — derives id/name from the path's
    /// last path component.
    public init(path: String, scale: Float = 1.0, baseModelId: String = "") {
        let filename = (path as NSString).lastPathComponent
        self.init(id: filename, name: filename, localPath: path,
                  baseModelId: baseModelId, scale: scale)
    }
}
...
public struct LoraAdapterCatalogEntry: Sendable {
    ...
    /// Alias used by sample code — same value as `url`.
    public var downloadURL: URL { url }
    /// Alias used by sample code — same value as `sizeBytes`.
    public var fileSize: Int64 { sizeBytes ?? 0 }

    public init(id: String, name: String, url: URL,
                baseModelId: String, sha256: String? = nil,
                sizeBytes: Int64? = nil) {
        self.id = id; self.name = name
        self.description = ""
        self.url = url
        self.filename = url.lastPathComponent
        self.compatibleModelIds = [baseModelId]
        self.baseModelId = baseModelId; self.sha256 = sha256
        self.sizeBytes = sizeBytes
        self.defaultScale = 1.0
    }

    /// Main-branch parity initialiser — matches the sample's registration
    /// call. `compatibleModelIds.first` is used as the canonical
    /// `baseModelId`.
    public init(id: String, name: String, description: String,
                downloadURL: URL, filename: String,
                compatibleModelIds: [String], fileSize: Int64,
        ...
        self.baseModelId = compatibleModelIds.first ?? ""
        self.sha256 = nil
...
internal enum ModelCatalog {
    ...
    /// Convenience accessor matching main's `StorageInfo.storedModels`
    /// field — returns the subset of registered models currently on
    /// disk wrapped in `StoredModel`.
    static var storedModels: [StoredModel] {
        entries.values.compactMap { info in
            guard let url = info.localPath else { return nil }
            let size = (try? FileManager.default
                .attributesOfItem(atPath: url.path)[.size] as? Int64) ?? 0
            return StoredModel(info: info, sizeBytes: size, path: url.path)
        }
    }
...
public extension RunAnywhere {
    ...
    /// Main-branch parity overload — id is derived from `name` when not
    /// supplied. Returns the registered `ModelInfo` so sample code can
    /// echo it back into its list view.
    @discardableResult
    static func registerModel(name: String, url: URL,
                                framework: InferenceFramework,
                                memoryRequirement: Int64? = nil,
                                supportsThinking: Bool = false) -> ModelInfo {
        let id = name
            .lowercased()
    ...
    /// Register a LoRA adapter in the catalog. Async/throws signature
    /// matches main's shape; the registry update itself is synchronous.
    ...
    /// Delete cached model files. Returns the number of bytes freed.
    /// Async/throws signature matches main's shape; the underlying
    /// C ABI is synchronous.
    @discardableResult
    static func clearCache() async throws -> Int64 { ra_file_clear_cache() }

    /// Delete temporary download files.
    @discardableResult
    static func cleanTempFiles() async throws -> Int64 { ra_file_clear_tmp() }
    ...
    /// Main-branch parity overload — `try await` friendly. Wraps the sync
    /// path and rethrows nothing today; kept `async throws` for forward
    /// compatibility if the adapter gains an async-removal path.
    static func deleteStoredModel(_ modelId: String,
                                   framework: InferenceFramework) async throws {
        _ = deleteStoredModel(modelId, framework: framework) as Bool
    }
...
public struct DownloadProgress: Sendable {
    ...
    /// Normalized 0…1 progress fraction used by UI code paths.
    public var overallProgress: Double { percent / 100.0 }

    /// Coarse-grained stage — matches the `DownloadStage` string enum
    /// used by sample UIs. Exposes `.completed` when the underlying
    /// state has a payload (e.g. `.complete(localPath:)`).
...
public extension RunAnywhere {
    ...
    /// `try await` variant — matches main's sample call site
    /// `let stream = try await RunAnywhere.downloadModel(id)`. Returns a
    /// non-throwing `AsyncStream` so consumers can iterate with plain
    /// `for await`; failures arrive as a final `DownloadProgress` with
    /// `state == .failed(message:)`.
    static func downloadModel(_ modelId: String) async throws
        -> AsyncStream<DownloadProgress>
    {
        let throwing = downloadModelStream(modelId)
        return AsyncStream { continuation in
            Task {
                do {
                    for try await p in throwing { continuation.yield(p) }
                    continuation.finish()
                } catch {
    ...
    private static func downloadModelStream(_ modelId: String)
