...
public final class EventBus {
    ...
    ///   consumers.
    /// - `events` (see `Public/Events/SDKEvent.swift`): a Combine
    ///   `AnyPublisher<any SDKEvent, Never>` that republishes every
    ///   emission. Use from Combine `.sink` / `.receive(on:)` callers.
