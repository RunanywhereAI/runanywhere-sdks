...
//
// Wave 2: Maps RAC_ERROR_* codes from runanywhere-commons directly to
// `SDKException` (proto-canonical). C++ error codes mirror RAErrorCode
// numerically (positive proto code = abs(negative rac_result_t)).
//
public enum CommonsErrorMapping {

    // MARK: - C++ to Swift

    /// Convert a `rac_result_t` to an `SDKException`, or nil on success.
    public static func toSDKException(_ result: rac_result_t) -> SDKException? {
        guard result != RAC_SUCCESS else { return nil }
        let (code, message) = mapErrorCode(result)
        return SDKException.make(code: code, message: message, category: categoryFor(code))
    }

    // swiftlint:disable:next cyclomatic_complexity
    private static func mapErrorCode(_ result: rac_result_t) -> (RAErrorCode, String) {
    ...
    // swiftlint:disable:next cyclomatic_complexity
    private static func categoryFor(_ code: RAErrorCode) -> RAErrorCategory {
        switch code {
        case .notInitialized, .alreadyInitialized, .initializationFailed,
             .invalidConfiguration, .environmentMismatch, .invalidParameter:
            return .configuration
        case .invalidApiKey, .authenticationFailed, .unauthorized, .forbidden,
             .keychainError, .secureStorageFailed:
            return .auth
        case .modelNotFound, .modelLoadFailed, .modelValidationFailed,
             .modelIncompatible, .invalidModelFormat, .modelStorageCorrupted,
             .modelNotLoaded:
            return .model
        case .generationFailed, .generationTimeout, .contextTooLong,
             .tokenLimitExceeded, .costLimitExceeded, .inferenceFailed,
             .processingFailed, .startFailed, .componentNotReady, .invalidState,
             .serviceNotAvailable, .serviceBusy, .moduleNotFound, .moduleAlreadyRegistered,
             .moduleLoadFailed, .serviceNotFound, .serviceAlreadyRegistered,
             .serviceCreateFailed, .capabilityNotFound, .providerNotFound,
             .noCapableProvider:
            return .component
        case .networkUnavailable, .networkError, .requestFailed, .downloadFailed,
             .serverError, .timeout, .invalidResponse, .httpError, .connectionLost,
             .partialDownload, .httpRequestFailed, .httpNotSupported:
            return .network
        case .insufficientStorage, .storageFull, .storageError, .fileNotFound,
             .fileReadFailed, .fileWriteFailed, .permissionDenied, .deleteFailed,
             .moveFailed, .directoryCreationFailed, .directoryNotFound,
             .invalidPath, .invalidFileName, .tempFileCreationFailed,
             .extractionFailed, .checksumMismatch, .unsupportedArchive:
            return .io
        case .validationFailed, .invalidInput, .invalidFormat, .emptyInput,
             .textTooLong, .invalidSsml, .invalidSpeakingRate, .invalidPitch,
             .invalidVolume, .invalidArgument, .nullPointer, .bufferTooSmall,
             .audioFormatNotSupported, .audioSessionFailed, .microphonePermissionDenied,
             .insufficientAudioData, .emptyAudioBuffer, .audioSessionActivationFailed,
             .languageNotSupported, .voiceNotAvailable, .streamingNotSupported,
             .streamCancelled, .calibrationFailed, .calibrationTimeout, .encodingError,
             .decodingError:
            return .validation
        default:
            return .internal
    ...
    private static func mapInitializationError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapModelError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapGenerationError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapNetworkError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapStorageError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapHardwareError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapComponentStateError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapValidationError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapAudioError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapLanguageVoiceError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapAuthenticationError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapSecurityError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapExtractionError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapCalibrationError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapCancellationError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapModuleServiceError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapPlatformAdapterError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapBackendError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapEventError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    private static func mapOtherError(_ result: rac_result_t) -> (RAErrorCode, String)? {
        switch result {
    ...
    /// Convert an `SDKException` to a `rac_result_t` for passing back to C++.
    // swiftlint:disable:next cyclomatic_complexity function_body_length
    public static func fromSDKException(_ ex: SDKException) -> rac_result_t {
        switch ex.proto.code {
        ...
        case .modelNotLoaded: return RAC_ERROR_MODEL_NOT_LOADED
        ...
        case .inferenceFailed: return RAC_ERROR_INFERENCE_FAILED
    ...
    /// Throw an SDKException if the result indicates failure.
    public static func throwIfError(_ result: rac_result_t) throws {
        if let ex = toSDKException(result) {
            throw ex
        }
    }

    /// Map a C error code to an `SDKException`. Always returns a non-nil value.
    public static func mapCommonsError(_ result: rac_result_t) -> SDKException {
        let (code, message) = mapErrorCode(result)
        return SDKException.make(code: code, message: message, category: categoryFor(code))
    }
}
