...
//  RunAnywhere SDK
//
//  Internal actor for managing all AI capability components.
//  Provides thread-safe access to LLM, STT, TTS, VAD, and VoiceAgent.
//
//  This is the single point of contact between Swift and C++ components.
//  All component lifecycle management happens here.
//

import CRACommons
import Foundation

/// Internal actor that manages all AI capability components.
/// Provides thread-safe access to LLM, STT, TTS, VAD, and VoiceAgent.
internal actor CapabilityManager {
    static let shared = CapabilityManager()

    // MARK: - Component Handles (C++ opaque pointers)
    ...
    private let logger = SDKLogger(category: "CapabilityManager")

    // MARK: - LLM Capability

    /// Get or create LLM component
    ...
    // MARK: - VoiceAgent Capability

    /// Create voice agent with component handles
    ...
    /// Get voice agent (creates if needed)
    ...
    /// Destroy all components (called on SDK shutdown)
    func destroyAll() {
        // Destroy voice agent first (it uses other components)
        if let handle = voiceAgentHandle {
            rac_voice_agent_destroy(handle)
            voiceAgentHandle = nil
        }
        ...
        logger.info("All capabilities destroyed")
    }
