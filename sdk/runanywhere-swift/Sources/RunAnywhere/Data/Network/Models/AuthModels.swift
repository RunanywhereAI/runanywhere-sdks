...
/// Response from GET /api/v1/model-assignments/for-sdk
public struct ModelAssignmentResponse: Codable {
    let models: [APIModelInfo]
...
/// Individual model assignment from model-assignments endpoint
public struct ModelAssignment: Codable {
    let id: String
    let name: String
    let provider: String
    let modelId: String
    ...
    func toAPIModelInfo() -> APIModelInfo {
        // Map provider to compatible frameworks (use exact enum raw values)
        ...
        case "tflite":
            frameworks = ["TFLite"]
        default:
            // Default to llamacpp for unknown formats
            frameworks = ["LlamaCpp"]
        }
