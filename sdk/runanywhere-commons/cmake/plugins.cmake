...
# rac_add_plugin_entry_test(<executable> TEST_NAME <ctest name> SOURCE <path>
#                           [LINK <targets...>] [INCLUDE_DIRS <dirs...>])
# Plugin-entry smoke test wired from the engine's own CMakeLists (the backend
# target only exists once the engine subdir has configured). No-op unless
# RAC_BUILD_TESTS is ON.
function(rac_add_plugin_entry_test target)
    cmake_parse_arguments(T "" "TEST_NAME;SOURCE" "LINK;INCLUDE_DIRS" ${ARGN})
    if(NOT RAC_BUILD_TESTS OR TARGET ${target})
        return()
    endif()
    add_executable(${target} ${T_SOURCE})
    target_include_directories(${target} PRIVATE
        ${CMAKE_SOURCE_DIR}/tests
        ${CMAKE_SOURCE_DIR}/include
        ${T_INCLUDE_DIRS}
    )
    target_link_libraries(${target} PRIVATE rac_commons ${T_LINK})
    foreach(_dep archive_static zlibstatic bz2_bundled)
        if(TARGET ${_dep})
            target_link_libraries(${target} PRIVATE ${_dep})
        endif()
    endforeach()
    target_compile_features(${target} PRIVATE cxx_std_17)
    add_test(NAME ${T_TEST_NAME} COMMAND ${target})
endfunction()
