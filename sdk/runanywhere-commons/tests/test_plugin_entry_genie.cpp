...
int main() {
    ...
    if (std::strcmp(build_info, "genie:sdk-unavailable") != 0) {
        std::fprintf(stdout, "  skip: Genie SDK is available in this build (%s)\n", build_info);
        return 0;
    }
    ...
    if (vt->metadata.priority != 0 || vt->metadata.capability_flags != 0 ||
        ...
        std::fprintf(stderr, "SDK-unavailable Genie advertised routing metadata\n");
        return 1;
    }
    ...
    if (genie_backend_unavailable() != RAC_ERROR_BACKEND_UNAVAILABLE) {
        std::fprintf(stderr, "genie_backend_unavailable did not return BACKEND_UNAVAILABLE\n");
        return 1;
    }

    void* impl = reinterpret_cast<void*>(0x1);
    if (g_genie_llm_ops.create("genie-test", "{}", &impl) != RAC_ERROR_BACKEND_UNAVAILABLE ||
        impl != nullptr ||
        g_genie_llm_ops.initialize(nullptr, nullptr) != RAC_ERROR_BACKEND_UNAVAILABLE ||
        g_genie_llm_ops.generate(nullptr, nullptr, nullptr, nullptr) !=
            RAC_ERROR_BACKEND_UNAVAILABLE ||
        g_genie_llm_ops.generate_stream(nullptr, nullptr, nullptr, nullptr, nullptr) !=
            RAC_ERROR_BACKEND_UNAVAILABLE ||
    ...
    if (rc != RAC_ERROR_CAPABILITY_UNSUPPORTED) {
        std::fprintf(stderr, "rac_plugin_register should reject SDK-unavailable Genie, got %d\n",
                     (int)rc);
        return 1;
    }
    if (rac_plugin_find(RAC_PRIMITIVE_GENERATE_TEXT) != nullptr) {
        std::fprintf(stderr, "SDK-unavailable Genie was inserted into the LLM registry\n");
        return 1;
    }

    std::fprintf(stdout, "  ok: SDK-unavailable Genie is not advertised or routable\n");
    return 0;
}
