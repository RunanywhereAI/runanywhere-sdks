...
int test_diffusion_generated_service_contract() {
    ...
    const google::protobuf::ServiceDescriptor* service =
        file->FindServiceByName("Diffusion");
    ...
    const google::protobuf::MethodDescriptor* stream =
        service->FindMethodByName("Stream");
    ...
    if (stream) {
        ...
        CHECK(stream->output_type()->full_name() ==
    ...
    const google::protobuf::Descriptor* event =
        runanywhere::v1::DiffusionStreamEvent::descriptor();
    const google::protobuf::FieldDescriptor* progress =
        event->FindFieldByName("progress");
    CHECK(progress != nullptr, "Diffusion stream event has progress field");
    if (progress) {
        CHECK(progress->message_type()->full_name() ==
    ...
    if (kind) {
        ...
        CHECK(kind->FindValueByName("DIFFUSION_STREAM_EVENT_KIND_INTERMEDIATE_IMAGE") !=
                  nullptr,
