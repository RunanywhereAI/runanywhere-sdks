...
void check_unary_rpc(const google::protobuf::ServiceDescriptor* service,
    ...
    CHECK(method->input_type()->full_name() == input_type,
          "ToolCalling RPC input type");
    CHECK(method->output_type()->full_name() == output_type,
          "ToolCalling RPC output type");
...
int test_tool_calling_generated_service_contract() {
    ...
    const google::protobuf::FieldDescriptor* options =
        format_request->FindFieldByName("options");
    ...
    if (options) {
        ...
        CHECK(options->has_presence(),
              "ToolPromptFormatRequest options field has presence");
    }

    const google::protobuf::FieldDescriptor* tool_results =
        format_request->FindFieldByName("tool_results");
    CHECK(tool_results != nullptr,
          "ToolPromptFormatRequest carries follow-up tool results");
    if (tool_results) {
        CHECK(tool_results->is_repeated(),
              "ToolPromptFormatRequest tool_results are repeated");
        CHECK(tool_results->message_type()->full_name() ==
    ...
    if (tool_call) {
        ...
        CHECK(tool_call->has_presence(),
              "ToolCallValidationRequest tool_call field has presence");
    }
    ...
    if (errors) {
        CHECK(errors->is_repeated(),
              "ToolCallValidationResult validation_errors are repeated");
    ...
    CHECK(matched_tool != nullptr,
          "ToolCallValidationResult carries matched tool definition");
    if (matched_tool) {
        CHECK(matched_tool->message_type()->full_name() ==
