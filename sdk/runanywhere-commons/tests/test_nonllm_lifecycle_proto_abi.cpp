...
int test_lifecycle_proto_operations(rac_model_registry_handle_t registry) {
    ...
    // VADProcessRequest has no request_id field (only audio + options).
    ...
    CHECK(embeddings_result.vectors(0).input_index() == 0,
          "embeddings lifecycle result preserves positional ordering");
    rac_proto_buffer_free(&out);
    ...
    // DiffusionGenerationRequest has no request_id field (only options + model_id).
    ...
    // DiffusionResult.width/height/image_data were replaced by a repeated
    // `images` field (one DiffusionImage per requested image; commons emits
    // exactly one until the C ABI grows a list) carrying its own width/
    // height/data per entry.
    CHECK(diffusion_result.images_size() == 1, "diffusion lifecycle result carries one image");
    CHECK(diffusion_result.images(0).width() == 4 && diffusion_result.images(0).height() == 2,
          "diffusion lifecycle ABI uses loaded diffusion ops");
    CHECK(diffusion_result.images(0).data().size() == 32,
...
int test_portable_source_blockers() {
    ...
    // VADAudioSource.adapter_handle was deleted outright (no SDK set it, no
    // engine read it -- same dead surface as STTAudioSource's). The
    // remaining portable-ABI blocker is a container-encoded native format:
    // decode_vad_samples only understands PCM_F32_LE/PCM_S16_LE (and
    // UNSPECIFIED, treated as F32_LE) and rejects AUDIO_ENCODING_CONTAINER.
    runanywhere::v1::VADProcessRequest vad_request;
    vad_request.mutable_audio()->set_audio_data("native-container-bytes");
    vad_request.mutable_audio()->set_encoding(runanywhere::v1::AUDIO_ENCODING_CONTAINER);
    CHECK(serialize(vad_request, &bytes), "VAD container-encoding request serializes");
    rac_proto_buffer_init(&out);
    rc = rac_vad_process_lifecycle_proto(bytes.data(), bytes.size(), &out);
    CHECK(rc == RAC_ERROR_NOT_SUPPORTED && out.status == RAC_ERROR_NOT_SUPPORTED,
