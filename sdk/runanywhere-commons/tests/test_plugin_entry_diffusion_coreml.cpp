 ...
 * test_diffusion_coreml_generate.cpp exercises the backend
 ...
 * COREML runtime/format, losing the diffusion_ops slot, or breaking
 * capability_check) would silently break image generation across every SDK
 * without failing any test. This locks the vtable wiring.
 *
 * Two build modes covered, discriminated by capability_check() exactly as the
 * plugin entry's own RAC_DIFFUSION_COREML_ROUTABLE switch does:
 *   - Routable build (Apple + RAC_DIFFUSION_COREML_GENERATE_AVAILABLE):
 ...
 *     an inert engine.
 *
 * The disjoint-slot invariant is asserted in both modes: diffusion-coreml is a
 * DIFFUSION-only engine, so every non-diffusion ops slot stays NULL.
 */
...
int main() {
    std::fprintf(stdout, "test_plugin_entry_diffusion_coreml\n");

    const rac_engine_vtable_t* vt = rac_plugin_entry_diffusion_coreml();
    if (vt == nullptr) {
        std::fprintf(stderr, "rac_plugin_entry_diffusion_coreml returned NULL\n");
        return 1;
    }
    ...
    // dynamic loader derives — commons-134 made it snake_case so a future
    // dlopen of librunanywhere_diffusion_coreml.{dylib,so} resolves cleanly.
    if (vt->metadata.name == nullptr ||
        std::strcmp(vt->metadata.name, "diffusion_coreml") != 0) {
    ...
    if (cap == RAC_SUCCESS) {
        ...
        if (vt->diffusion_ops == nullptr) {
            std::fprintf(stderr, "routable diffusion-coreml has NULL diffusion_ops slot\n");
            return 1;
        }
        // Disjoint-slot invariant: diffusion-coreml is a DIFFUSION-only engine.
        if (vt->llm_ops != nullptr || vt->stt_ops != nullptr || vt->tts_ops != nullptr ||
            ...
            std::fprintf(stderr, "diffusion-coreml advertised a non-DIFFUSION ops slot\n");
            return 1;
        }

        const rac_result_t rc = rac_plugin_register(vt);
        if (rc != RAC_SUCCESS) {
            std::fprintf(stderr, "rac_plugin_register rejected routable diffusion-coreml, got %d\n",
                         (int)rc);
            return 1;
        }
        if (rac_plugin_find(RAC_PRIMITIVE_DIFFUSION) != vt) {
            std::fprintf(stderr,
                         "plugin_find(RAC_PRIMITIVE_DIFFUSION) did not return diffusion-coreml\n");
            rac_plugin_unregister(vt->metadata.name);
            return 1;
        }
        rac_plugin_unregister(vt->metadata.name);
        std::fprintf(stdout,
                     "  ok: routable diffusion-coreml advertises DIFFUSION+CoreML and routes\n");
        return 0;
    }
    ...
    if (vt->diffusion_ops != nullptr || vt->llm_ops != nullptr || vt->stt_ops != nullptr ||
        ...
        std::fprintf(stderr, "stub diffusion-coreml advertised an ops slot\n");
        return 1;
    }
    const rac_result_t rc = rac_plugin_register(vt);
    if (rc != RAC_ERROR_CAPABILITY_UNSUPPORTED) {
        std::fprintf(stderr, "rac_plugin_register should reject stub diffusion-coreml, got %d\n",
                     (int)rc);
        return 1;
    }
    if (rac_plugin_find(RAC_PRIMITIVE_DIFFUSION) == vt) {
        std::fprintf(stderr, "stub diffusion-coreml was inserted into the registry anyway\n");
        return 1;
    }
    std::fprintf(stdout, "  ok: stub diffusion-coreml is not advertised or routable\n");
    return 0;
}
