 ...
 *   - Struct APIs (rac_diffusion_component_create, configure, load_model,
 *     unload, cleanup, cancel, generate, generate_with_callbacks,
 *     get_capabilities, get_info, get_state, get_metrics, destroy):
 *     `delete after SDK migration` for SDK callers — replaced by
 *     diffusion proto APIs over runanywhere.v1.DiffusionConfiguration /
 *     DiffusionGenerationOptions / DiffusionResult bytes.
 *   - JSON helpers (configure_json, generate_json, get_info_json):
 *     `delete after SDK migration` (see JSON APIs section).
 */
