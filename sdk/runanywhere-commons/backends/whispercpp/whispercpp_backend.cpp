...
#define LOGI(...) RA_LOG_INFO("STT.WhisperCpp", __VA_ARGS__)
#define LOGE(...) RA_LOG_ERROR("STT.WhisperCpp", __VA_ARGS__)
#define LOGW(...) RA_LOG_WARNING("STT.WhisperCpp", __VA_ARGS__)
...
BackendInfo WhisperCppBackend::get_info() const {
    BackendInfo info;
    info.name = "whispercpp";
    info.version = "1.0.0";
    info.description = "Speech-to-Text inference via whisper.cpp";
    info.supported_capabilities = {CapabilityType::STT};
    info.metadata = {{"whisper_version", "1.7.2"},
                     {"supports_streaming", true},
                     {"supports_translation", true},
                     {"supported_languages", 98}};
    return info;
}
...
bool WhisperCppBackend::initialize(const nlohmann::json& config) {
    ...
    if (num_threads_ <= 0) {
        // Auto-detect: use fewer threads than available to leave headroom
...
void WhisperCppBackend::create_capabilities() {
    register_capability(CapabilityType::STT, std::make_unique<WhisperCppSTT>(this));
    LOGI("Registered STT capability");
}

ra_device_type WhisperCppBackend::get_device_type() const {
...
size_t WhisperCppBackend::get_memory_usage() const {
    // TODO: Implement actual memory tracking if whisper.cpp provides it
    return 0;
}
...
bool WhisperCppSTT::load_model(const std::string& model_path, STTModelType model_type,
    ...
    // DTW for word-level timestamps if requested
    if (config.contains("word_timestamps") && config["word_timestamps"].get<bool>()) {
        cparams.dtw_token_timestamps = true;
        cparams.dtw_aheads_preset = WHISPER_AHEADS_LARGE_V3;
    }

    // Flash attention for better performance (if available)
...
STTResult WhisperCppSTT::transcribe_internal(const std::vector<float>& audio,
    ...
    for (int i = 0; i < n_segments; ++i) {
        ...
        if (text) {
            ...
            // Segment timing (whisper times are in centiseconds)
            ...
            // Get no_speech probability as confidence proxy
...
bool WhisperCppSTT::supports_streaming() const {
    return true;  // whisper.cpp supports streaming via state separation
}
...
bool WhisperCppSTT::is_stream_ready(const std::string& stream_id) {
    ...
    // Ready when we have enough audio (at least 1 second) or input is finished
...
STTResult WhisperCppSTT::decode(const std::string& stream_id) {
    ...
    whisper_full_params wparams = whisper_full_default_params(WHISPER_SAMPLING_GREEDY);
    wparams.n_threads = backend_->get_num_threads();
    wparams.single_segment = !stream_state->input_finished;  // Single segment for streaming
    wparams.no_context = false;                              // Use context from previous segments
    wparams.print_progress = false;
    wparams.print_realtime = false;
    wparams.print_timestamps = false;
...
bool WhisperCppSTT::is_endpoint(const std::string& stream_id) {
    ...
    // For whisper.cpp, we don't have built-in endpoint detection
...
void WhisperCppSTT::reset_stream(const std::string& stream_id) {
    ...
    if (it != streams_.end()) {
        ...
        // Note: We keep the whisper_state as it maintains context
...
std::vector<std::string> WhisperCppSTT::get_supported_languages() const {
    ...
    // Get all language codes from whisper.cpp
...
std::unique_ptr<Backend> create_whispercpp_backend() {
    return std::make_unique<WhisperCppBackend>();
}

void register_whispercpp_backend() {
    static bool registered = false;
    if (registered)
        return;

    BackendRegistry::instance().register_backend("whispercpp", create_whispercpp_backend);
    LOGI("WhisperCPP backend registered");
    registered = true;
}
