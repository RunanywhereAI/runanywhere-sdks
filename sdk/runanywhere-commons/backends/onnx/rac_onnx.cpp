...
rac_result_t rac_stt_onnx_create(const char* model_path, const rac_stt_onnx_config_t* config,
    ...
    // Get STT capability
    auto* cap = handle->backend->get_capability(runanywhere::CapabilityType::STT);
    handle->stt = dynamic_cast<runanywhere::ONNXSTT*>(cap);
    if (!handle->stt) {
        delete handle;
        rac_error_set_details("STT capability not available");
        return RAC_ERROR_BACKEND_INIT_FAILED;
    }
...
rac_result_t rac_tts_onnx_create(const char* model_path, const rac_tts_onnx_config_t* config,
    ...
    auto* cap = handle->backend->get_capability(runanywhere::CapabilityType::TTS);
    handle->tts = dynamic_cast<runanywhere::ONNXTTS*>(cap);
    if (!handle->tts) {
        delete handle;
        rac_error_set_details("TTS capability not available");
        return RAC_ERROR_BACKEND_INIT_FAILED;
    }
...
rac_result_t rac_vad_onnx_create(const char* model_path, const rac_vad_onnx_config_t* config,
    ...
    auto* cap = handle->backend->get_capability(runanywhere::CapabilityType::VAD);
    handle->vad = dynamic_cast<runanywhere::ONNXVAD*>(cap);
    if (!handle->vad) {
        delete handle;
        rac_error_set_details("VAD capability not available");
        return RAC_ERROR_BACKEND_INIT_FAILED;
    }
