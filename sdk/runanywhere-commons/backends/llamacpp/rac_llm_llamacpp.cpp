...
rac_result_t rac_llm_llamacpp_create(const char* model_path,
    ...
    auto* cap = handle->backend->get_capability(runanywhere::CapabilityType::TEXT_GENERATION);
    handle->text_gen = dynamic_cast<runanywhere::LlamaCppTextGeneration*>(cap);
    if (!handle->text_gen) {
        delete handle;
        rac_error_set_details("Failed to get text generation capability");
        return RAC_ERROR_BACKEND_INIT_FAILED;
    }
