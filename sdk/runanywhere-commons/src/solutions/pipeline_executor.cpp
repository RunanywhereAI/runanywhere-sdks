...
public:
    PayloadTypeGuardNode(std::string name,
    ...
    void set_input(std::shared_ptr<OperatorEdge> input) noexcept {
...
private:
    void run() {
        while (cancel_ && !cancel_->is_cancelled()) {
            ...
            if (!output_->push(std::move(*item), cancel_.get())) break;
        }
        if (output_) output_->close();
    }
    ...
    void report_body_violation(const std::string& detail) const {
        const std::string msg =
            "payload body contract violation on edge '" + from_endpoint_ +
            " -> " + to_endpoint_ + "': " + detail;
    ...
    void report_violation(const Item& item) const {
        const std::string actual =
            item.type_id.empty() ? std::string("<empty>") : item.type_id;
        const std::string msg =
            "payload type contract violation on edge '" + from_endpoint_ +
...
bool parse_endpoint(const std::string& endpoint,
                    ...
                    std::string*       err) {
    if (endpoint.empty()) {
        if (err) *err = std::string("edge ") + field_name + " endpoint is empty";
        return false;
    }
    ...
    if (dot + 1 == endpoint.size()) {
        if (err) {
            *err = std::string("edge ") + field_name +
                   " endpoint has empty port name: " + endpoint;
        }
        return false;
    }
...
bool validate_edge_capacity(std::uint32_t capacity,
...
bool validate_edge_policy(runanywhere::v1::EdgePolicy policy,
                          const std::string& endpoint,
                          std::string* err) {
    if (!runanywhere::v1::EdgePolicy_IsValid(static_cast<int>(policy))) {
        if (err) {
            *err = "edge policy for '" + endpoint +
                   "' is not a recognised EdgePolicy value";
        }
        return false;
    }
    return true;
...
bool validate_payload_type(const std::string& type,
                           const std::string& endpoint,
                           std::string* err) {
    if (type.empty()) {
        if (err) *err = "payload type for endpoint '" + endpoint + "' is empty";
        return false;
    }
    if (type == "opaque.bytes") {
        if (err) {
            *err = "payload type for endpoint '" + endpoint +
                   "' uses legacy opaque.bytes metadata";
        }
        return false;
    }
    return true;
...
std::string unsupported_port_message(const std::string& op,
                                     const std::string& type,
                                     const char*        direction,
                                     const std::string& port) {
    return "factory for operator '" + op + "' (type '" + type +
           "') does not expose " + direction + " port '" + port +
...
PipelineExecutor::build(rac_result_t* out_error) {
    ...
    for (const auto& edge : spec_.edges()) {
        ...
        if (!registry.has_input_port(dst_type->second, to.port)) {
            return emit_null(RAC_ERROR_INVALID_CONFIGURATION,
                             "edge references unknown input port '" + to.port +
                                 "' on operator '" + to.op + "'");
        }
        const auto& output_type =
            registry.output_port_type(src_type->second, from.port);
        const auto& input_type =
            registry.input_port_type(dst_type->second, to.port);
        ...
        resolved_edges.push_back(
            {std::move(from), std::move(to), output_type, capacity,
    ...
    std::unordered_map<std::string, std::shared_ptr<rac::graph::SplitNode<Item>>>
        split_nodes;
    std::unordered_map<std::string, std::shared_ptr<rac::graph::MergeNode<Item>>>
        merge_nodes;
    ...
    for (const auto& [from_endpoint, edge_indices] : outgoing_by_endpoint) {
        if (edge_indices.size() <= 1) continue;
        ...
        auto split = std::make_shared<rac::graph::SplitNode<Item>>(
            "__split_" + from_endpoint, edge_indices.size(), first.capacity,
            first.policy);
        auto internal_edge =
            std::make_shared<OperatorEdge>(first.capacity, first.policy);
        auto producer = nodes.find(first.from.op);
        if (producer == nodes.end()) {
            return emit_null(RAC_ERROR_INVALID_CONFIGURATION,
                             "split adapter references missing producer '" +
                                 first.from.op + "'");
        }
        ...
        if (!producer->second->set_output(first.from.port, internal_edge)) {
            return emit_null(
                RAC_ERROR_INVALID_CONFIGURATION,
                unsupported_port_message(first.from.op, type, "output",
                                         first.from.port));
        }
    ...
    for (const auto& [to_endpoint, edge_indices] : incoming_by_endpoint) {
        ...
        auto merge = std::make_shared<rac::graph::MergeNode<Item>>(
            "__merge_" + to_endpoint, edge_indices.size(), first.capacity,
            first.policy);
        auto consumer = nodes.find(first.to.op);
        if (consumer == nodes.end()) {
            return emit_null(RAC_ERROR_INVALID_CONFIGURATION,
                             "merge adapter references missing consumer '" +
                                 first.to.op + "'");
        }
        ...
        if (!consumer->second->set_input(first.to.port, merge->output())) {
            return emit_null(
                RAC_ERROR_INVALID_CONFIGURATION,
                unsupported_port_message(first.to.op, type, "input",
                                         first.to.port));
        }
    ...
    for (std::size_t i = 0; i < resolved_edges.size(); ++i) {
        ...
        guard_order.push_back(std::make_shared<PayloadTypeGuardNode>(
            "__payload_type_guard_" + std::to_string(i) + "_" +
                endpoint_key(edge.from) + "_to_" + endpoint_key(edge.to),
            edge.payload_type, endpoint_key(edge.from), endpoint_key(edge.to),
            edge.capacity, edge.policy));
    }
    ...
    for (std::size_t i = 0; i < resolved_edges.size(); ++i) {
        ...
        if (split != split_nodes.end()) {
            ...
            upstream_stream =
                std::make_shared<OperatorEdge>(edge.capacity, edge.policy);
            auto type = op_types.find(edge.from.op);
            const std::string type_name =
                type == op_types.end() ? std::string{} : type->second;
            if (!src->second->set_output(edge.from.port, upstream_stream)) {
                return emit_null(
                    RAC_ERROR_INVALID_CONFIGURATION,
                    unsupported_port_message(edge.from.op, type_name, "output",
                                             edge.from.port));
            }
        }
        ...
        if (merge != merge_nodes.end()) {
            ...
            if (!dst->second->set_input(edge.to.port, stream)) {
                return emit_null(
                    RAC_ERROR_INVALID_CONFIGURATION,
                    unsupported_port_message(edge.to.op, type_name, "input",
                                             edge.to.port));
            }
        }
    ...
    auto choose_input = [&](const std::shared_ptr<OperatorAdapter>& node,
        ...
        if (edge) return {edge, registry.input_port_type(type, "in")};
        for (const auto& port : registry.input_ports(type)) {
            edge = node->input(port);
            if (edge) return {edge, registry.input_port_type(type, port)};
        }
        return {};
    };
    auto choose_output = [&](const std::shared_ptr<OperatorAdapter>& node,
        ...
        if (edge) return {edge, registry.output_port_type(type, "out")};
        for (const auto& port : registry.output_ports(type)) {
            edge = node->output(port);
            if (edge) return {edge, registry.output_port_type(type, port)};
        }
        return {};
    };
    ...
    for (const auto& op : spec_.operators()) {
        ...
        if (node == nodes.end()) {
            return emit_null(RAC_ERROR_INVALID_CONFIGURATION,
                             "materialized node missing for operator '" +
                                 op.name() + "'");
        }
