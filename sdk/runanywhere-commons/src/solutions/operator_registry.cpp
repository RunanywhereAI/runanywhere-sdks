...
bool is_image_bytes_payload_type(const std::string& type_id) {
    return type_id == kPayloadImagePng ||
           type_id == kPayloadImageJpeg ||
           type_id == kPayloadImageWebp ||
           type_id == kPayloadImageRawRgb ||
...
bool is_control_signal_payload_type(const std::string& type_id) {
    return type_id == kPayloadControlTerminal ||
           type_id == kPayloadControlCancellation ||
...
bool has_prefix(const std::string& value,
    ...
    for (std::size_t i = 0; i < expected_size; ++i) {
        if (static_cast<unsigned char>(value[i]) != expected[i]) return false;
    }
    return true;
}

bool has_ascii_at(const std::string& value,
                  std::size_t offset,
                  const char* expected,
                  std::size_t expected_size) {
    if (value.size() < offset + expected_size) return false;
    for (std::size_t i = 0; i < expected_size; ++i) {
...
bool has_jpeg_signature(const std::string& value) {
    return value.size() >= 4 &&
           static_cast<unsigned char>(value[0]) == 0xFFu &&
...
bool has_webp_signature(const std::string& value) {
    return value.size() >= 12 &&
           has_ascii_at(value, 0, "RIFF", 4) &&
           has_ascii_at(value, 8, "WEBP", 4);
}

bool replace_all(std::string& value,
...
protected:
    void process(Item item, OutputEdge& out) override {
        if (!out.push(Item::text(build_prompt(item.text())),
                      this->cancel_token())) {
            return;
        }
    }
...
private:
    std::string build_prompt(const std::string& input) const {
        if (prompt_template_.empty()) return input;
...
int positive_int_param_or_default(const runanywhere::v1::OperatorSpec& spec,
                                  const char* key,
                                  int fallback) {
    ...
    if (ec != std::errc{} || ptr != last || value <= 0) return fallback;
    return value;
}
...
OperatorFactory make_window_factory() {
    return [](const runanywhere::v1::OperatorSpec& spec)
        ...
        return std::make_shared<WindowNode>(
            spec.name(), static_cast<std::size_t>(size),
...
OperatorFactory make_context_build_factory() {
    return [](const runanywhere::v1::OperatorSpec& spec)
        ...
        return std::make_shared<ContextBuildNode>(
            spec.name(), std::move(prompt_template));
    };
}

OperatorPortSchema make_schema(std::vector<std::string> inputs,
...
bool contains_port(const std::vector<std::string>& ports,
...
const std::string& payload_type_or_empty(
    const std::unordered_map<std::string, std::string>& port_types,
    const std::string&                                  port) {
    auto it = port_types.find(port);
    if (it == port_types.end() || it->second.empty()) return empty_payload_type();
    return it->second;
}

class SinglePortOperatorAdapter final : public OperatorAdapter {
public:
    SinglePortOperatorAdapter(std::shared_ptr<OperatorNode> node,
                              OperatorPortSchema           schema)
        : node_(std::move(node)),
    ...
    std::shared_ptr<OperatorEdge> input(
        ...
        if (!can_reference_input(port)) return nullptr;
        return node_->input();
    }

    std::shared_ptr<OperatorEdge> output(
        const std::string& port) const noexcept override {
        if (!can_reference_output(port)) return nullptr;
        return node_->output();
    }

    bool set_input(const std::string& port,
                   std::shared_ptr<OperatorEdge> edge) noexcept override {
        if (!can_bind_input(port)) return false;
        if (bound_input_ && *bound_input_ != port) return false;
        bound_input_ = port;
    ...
    bool set_output(const std::string& port,
                    std::shared_ptr<OperatorEdge> edge) noexcept override {
        if (!can_bind_output(port)) return false;
        if (bound_output_ && *bound_output_ != port) return false;
        bound_output_ = port;
...
Payload Payload::text(std::string value, std::string payload_type) {
    return Payload(std::move(payload_type), std::move(value),
                   PayloadBodyKind::TextUtf8);
}

Payload Payload::audio_pcm_s16le(std::string payload_bytes,
                                  std::string payload_type) {
    return Payload(std::move(payload_type), std::move(payload_bytes),
                   PayloadBodyKind::AudioPcmS16Le);
}

Payload Payload::image_bytes(std::string payload_bytes,
                             std::string payload_type) {
    return Payload(std::move(payload_type), std::move(payload_bytes),
                   PayloadBodyKind::ImageBytes);
}

Payload Payload::embedding_vector(std::vector<float> values,
                                  std::string payload_type) {
...
Payload Payload::control_signal(PayloadControlSignalKind kind,
                                std::string reason,
                                int status_code,
                                std::string payload_type) {
...
Payload Payload::terminal_control(std::string reason,
                                  int status_code,
                                  std::string payload_type) {
    return control_signal(PayloadControlSignalKind::Terminal,
                          std::move(reason), status_code,
                          std::move(payload_type));
}

Payload Payload::sdk_event_proto(std::string serialized_event,
                                 std::string payload_type) {
    return Payload(std::move(payload_type), std::move(serialized_event),
                   PayloadBodyKind::SdkEventProto);
}

Payload Payload::voice_event_proto(std::string serialized_event,
                                   std::string payload_type) {
    return Payload(std::move(payload_type), std::move(serialized_event),
                   PayloadBodyKind::VoiceEventProto);
}

Payload Payload::typed_bytes(std::string payload_type,
...
bool Payload::validate_body_contract(std::string* err) const {
    ...
    case PayloadBodyKind::TextUtf8:
        if (type_id != kPayloadTextUtf8) {
            return fail("payload body type 'text.utf8' does not match payload "
                        "type '" + type_id + "'");
        }
    ...
    case PayloadBodyKind::AudioPcmS16Le:
        if (type_id != kPayloadAudioPcmS16Le) {
            return fail("payload body type 'audio.pcm_s16le' does not match "
                        "payload type '" + type_id + "'");
        }
        if ((bytes.size() % 2u) != 0u) {
            return fail("payload body type 'audio.pcm_s16le' contains a "
                        "partial 16-bit sample");
        }
        return true;
    case PayloadBodyKind::ImageBytes:
        if (!is_image_bytes_payload_type(type_id)) {
            return fail("payload body type 'image.bytes' does not match "
                        "payload type '" + type_id + "'");
        }
        if (bytes.empty()) {
        ...
        if (type_id == kPayloadImagePng && !has_png_signature(bytes)) {
            return fail("payload body type 'image.bytes' for 'image/png' "
                        "has invalid PNG signature");
        }
        if (type_id == kPayloadImageJpeg && !has_jpeg_signature(bytes)) {
            return fail("payload body type 'image.bytes' for 'image/jpeg' "
                        "has invalid JPEG signature");
        }
        if (type_id == kPayloadImageWebp && !has_webp_signature(bytes)) {
            return fail("payload body type 'image.bytes' for 'image/webp' "
                        "has invalid WEBP signature");
        }
        if (type_id == kPayloadImageRawRgb && (bytes.size() % 3u) != 0u) {
            return fail("payload body type 'image.bytes' for 'image/raw-rgb' "
                        "contains a partial RGB pixel");
        }
        if (type_id == kPayloadImageRawRgba && (bytes.size() % 4u) != 0u) {
            return fail("payload body type 'image.bytes' for "
                        "'image/raw-rgba' contains a partial RGBA pixel");
        }
        return true;
    case PayloadBodyKind::EmbeddingVector:
        if (!is_embedding_vector_payload_type(type_id)) {
            return fail("payload body type 'embedding.vector' does not match "
                        "payload type '" + type_id + "'");
        }
        if (embedding_vector_body.element_type_id !=
            kPayloadEmbeddingElementFloat32) {
            return fail("payload body type 'embedding.vector' has unsupported "
                        "element type '" +
        ...
        if (embedding_vector_body.element_count !=
            embedding_vector_body.values.size()) {
            return fail("payload body type 'embedding.vector' has misaligned "
    ...
    case PayloadBodyKind::ControlSignal: {
        if (!is_control_signal_payload_type(type_id)) {
            return fail("payload body type 'control.signal' does not match "
                        "payload type '" + type_id + "'");
        }
        if (control_signal_body.kind == PayloadControlSignalKind::Invalid) {
            return fail("payload body type 'control.signal' has invalid "
                        "control kind");
        }
        const std::string expected_type =
            control_payload_type_for(control_signal_body.kind);
        if (type_id != expected_type) {
            return fail("payload body type 'control.signal' kind requires "
                        "payload type '" + expected_type + "', got '" +
                        type_id + "'");
        }
        return true;
    }
    case PayloadBodyKind::SdkEventProto: {
        if (type_id != kPayloadSdkEvent) {
            return fail("payload body type 'proto.runanywhere.v1.SDKEvent' "
                        "does not match payload type '" + type_id + "'");
        }
        if (bytes.empty()) {
            return fail("payload body type 'proto.runanywhere.v1.SDKEvent' "
                        "is empty");
    ...
    case PayloadBodyKind::VoiceEventProto: {
        if (type_id != kPayloadVoiceEvent) {
            return fail("payload body type 'proto.runanywhere.v1.VoiceEvent' "
                        "does not match payload type '" + type_id + "'");
        }
        if (bytes.empty()) {
            return fail("payload body type 'proto.runanywhere.v1.VoiceEvent' "
        ...
        if (event.payload_case() ==
            runanywhere::v1::VoiceEvent::PAYLOAD_NOT_SET) {
            return fail("payload body type 'proto.runanywhere.v1.VoiceEvent' "
    ...
    case PayloadBodyKind::RawBytes:
        if (type_id == kPayloadTextUtf8) {
            return fail("payload type 'text.utf8' requires body type "
                        "'text.utf8', got 'bytes.raw'");
        }
        if (type_id == kPayloadAudioPcmS16Le) {
            return fail("payload type 'audio.pcm_s16le' requires body type "
                        "'audio.pcm_s16le', got 'bytes.raw'");
        }
        if (type_id == kPayloadBodyImageBytes ||
            is_image_bytes_payload_type(type_id)) {
            return fail("payload type '" + type_id + "' requires body type "
        ...
        if (type_id == kPayloadBodyControlSignal ||
            is_control_signal_payload_type(type_id)) {
            return fail("payload type '" + type_id + "' requires body type "
                        "'control.signal', got 'bytes.raw'");
        }
        if (type_id == kPayloadBodySdkEventProto ||
            type_id == kPayloadSdkEvent) {
            return fail("payload type '" + type_id + "' requires body type "
                        "'proto.runanywhere.v1.SDKEvent', got 'bytes.raw'");
        }
        if (type_id == kPayloadBodyVoiceEventProto ||
            type_id == kPayloadVoiceEvent) {
            return fail("payload type '" + type_id + "' requires body type "
...
bool OperatorRegistry::register_adapter_factory(const std::string& type,
                                                ...
                                                OperatorPortSchema ports) {
    auto [it, inserted] =
        adapter_factories_.insert_or_assign(type, std::move(factory));
    (void)it;
...
std::shared_ptr<OperatorAdapter> OperatorRegistry::create_adapter(
    const runanywhere::v1::OperatorSpec& spec) const {
    ...
    if (schema == port_schemas_.end()) {
        return std::make_shared<SinglePortOperatorAdapter>(
            std::move(node), default_schema());
    }
    return std::make_shared<SinglePortOperatorAdapter>(
        std::move(node), schema->second);
}
...
bool OperatorRegistry::has_input_port(const std::string& type,
    ...
    if (it == port_schemas_.end()) return false;
...
const std::vector<std::string>& OperatorRegistry::input_ports(
    const std::string& type) const noexcept {
    auto it = port_schemas_.find(type);
    if (it == port_schemas_.end()) return empty_ports();
    return it->second.input_ports;
}

const std::vector<std::string>& OperatorRegistry::output_ports(
    const std::string& type) const noexcept {
    auto it = port_schemas_.find(type);
    if (it == port_schemas_.end()) return empty_ports();
...
const std::string& OperatorRegistry::input_port_type(
    const std::string& type,
    const std::string& port) const noexcept {
    auto it = port_schemas_.find(type);
    if (it == port_schemas_.end()) return empty_payload_type();
    return payload_type_or_empty(it->second.input_port_types, port);
}

const std::string& OperatorRegistry::output_port_type(
    const std::string& type,
    const std::string& port) const noexcept {
    auto it = port_schemas_.find(type);
    if (it == port_schemas_.end()) return empty_payload_type();
...
void register_builtin_operators(OperatorRegistry& registry) {
    ...
    registry.register_factory("context_build", make_context_build_factory(),
                              default_schema());
