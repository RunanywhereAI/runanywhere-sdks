...
bool detect_qnn_android() {
    ...
    struct stat st{};
    if (stat("/dev/fastrpc-adsp", &st) != 0 && stat("/dev/fastrpc-cdsp", &st) != 0)
        return false;
...
HardwareProfile HardwareProfile::detect() {
...
#elif defined(__ANDROID__)
    {
        ...
        if (vendor.find("qcom") != std::string::npos || vendor.find("kona") != std::string::npos ||
            vendor.find("sm") == 0) {
...
bool HardwareProfile::supports_runtime(rac_runtime_id_t r) const {
    switch (r) {
        ...
        case RAC_RUNTIME_ONNXRT:
            return true; /* host runtime; EP selection happens inside ORT */
