 ...
 *   - `rac_analytics_emit_*` (C linkage) — convenience emitters for platform
 *     SDKs (Web via Emscripten ccall, Kotlin via JNI) that emit from outside
 *     the C++ component layer.
 *   - `rac::events::emit_*` (C++) — internal emitters used by the JNI bridge
 *     and the voice-agent feature layer.
 *
...
// =============================================================================
// PLATFORM EMIT HELPERS (C-linkage, callable from WASM via ccall / JNI)
// =============================================================================
...
void rac_analytics_emit_stt_model_load_completed(const char* model_id, const char* model_name,
    ...
    m.set_framework(rac::events::framework_to_proto_int(
        static_cast<rac_inference_framework_t>(framework)));
    rac::events::publish(v1::SDK_COMPONENT_STT, v1::EVENT_CATEGORY_MODEL, std::move(m));
#else
    (void)model_id;
    (void)model_name;
    (void)duration_ms;
...
void rac_analytics_emit_stt_model_load_failed(const char* model_id, int32_t error_code,
    ...
    set_event_error(event, static_cast<rac_result_t>(error_code), error_message);
    rac::events::publish(event, v1::SDK_COMPONENT_STT, v1::EVENT_CATEGORY_MODEL);
#else
    (void)model_id;
    (void)error_code;
    (void)error_message;
...
void rac_analytics_emit_stt_transcription_completed(
    ...
    voice.set_framework(rac::events::framework_to_proto_int(
        static_cast<rac_inference_framework_t>(framework)));
    rac::events::publish_with_session(v1::SDK_COMPONENT_STT, v1::EVENT_CATEGORY_STT,
                                      std::move(voice), transcription_id);
#else
    (void)transcription_id;
    (void)model_id;
    (void)text;
    (void)confidence;
    (void)duration_ms;
    (void)audio_length_ms;
    (void)audio_size_bytes;
    (void)word_count;
    (void)real_time_factor;
    (void)language;
    (void)sample_rate;
    (void)framework;
#endif
...
void rac_analytics_emit_stt_transcription_failed(const char* transcription_id, const char* model_id,
    ...
    set_event_error(event, static_cast<rac_result_t>(error_code), error_message);
    rac::events::publish(event, v1::SDK_COMPONENT_STT, v1::EVENT_CATEGORY_STT);
#else
    (void)transcription_id;
    (void)model_id;
    (void)error_code;
...
void rac_analytics_emit_tts_voice_load_completed(const char* model_id, const char* model_name,
    ...
    m.set_framework(rac::events::framework_to_proto_int(
        static_cast<rac_inference_framework_t>(framework)));
    rac::events::publish(v1::SDK_COMPONENT_TTS, v1::EVENT_CATEGORY_MODEL, std::move(m));
#else
    (void)model_id;
    (void)model_name;
    (void)duration_ms;
...
void rac_analytics_emit_tts_voice_load_failed(const char* model_id, int32_t error_code,
    ...
    if (error_message)
        m->set_error(error_message);
    set_event_error(event, static_cast<rac_result_t>(error_code), error_message);
    rac::events::publish(event, v1::SDK_COMPONENT_TTS, v1::EVENT_CATEGORY_MODEL);
#else
    (void)model_id;
    (void)error_code;
    (void)error_message;
...
void rac_analytics_emit_tts_synthesis_completed(const char* synthesis_id, const char* model_id,
    ...
    voice.set_framework(rac::events::framework_to_proto_int(
        static_cast<rac_inference_framework_t>(framework)));
    rac::events::publish_with_session(v1::SDK_COMPONENT_TTS, v1::EVENT_CATEGORY_TTS,
                                      std::move(voice), synthesis_id);
#else
    (void)synthesis_id;
    (void)model_id;
    (void)character_count;
    (void)audio_duration_ms;
    (void)audio_size_bytes;
    (void)processing_duration_ms;
    (void)characters_per_second;
    (void)sample_rate;
    (void)framework;
#endif
}
...
void rac_analytics_emit_tts_synthesis_failed(const char* synthesis_id, const char* model_id,
    ...
    set_event_error(event, static_cast<rac_result_t>(error_code), error_message);
    rac::events::publish(event, v1::SDK_COMPONENT_TTS, v1::EVENT_CATEGORY_TTS);
#else
    (void)synthesis_id;
    (void)model_id;
    (void)error_code;
...
void rac_analytics_emit_vad_speech_started(void) {
    ...
    rac::events::publish_with_session(v1::SDK_COMPONENT_VAD, v1::EVENT_CATEGORY_VAD,
                                      std::move(voice), /*session_id=*/nullptr,
...
void rac_analytics_emit_vad_speech_ended(double speech_duration_ms, float energy_level) {
    (void)energy_level;  // not consumed by telemetry
#if defined(RAC_HAVE_PROTOBUF)
    v1::VoiceLifecycleEvent voice;
...
void rac_analytics_emit_model_download_started(const char* model_id) {
    ...
    if (model_id)
        m.set_model_id(model_id);
    rac::events::publish(v1::SDK_COMPONENT_UNSPECIFIED, v1::EVENT_CATEGORY_DOWNLOAD, std::move(m));
#else
    (void)model_id;
#endif
}
...
void rac_analytics_emit_model_download_completed(const char* model_id, int64_t file_size_bytes,
    ...
    if (model_id)
        m.set_model_id(model_id);
    m.set_model_size_bytes(file_size_bytes);
    m.set_duration_ms(static_cast<int64_t>(duration_ms));
    m.set_progress(1.0f);
    rac::events::publish(v1::SDK_COMPONENT_UNSPECIFIED, v1::EVENT_CATEGORY_DOWNLOAD, std::move(m));
#else
...
void rac_analytics_emit_model_download_failed(const char* model_id, const char* error_message) {
    ...
    if (error_message)
        m->set_error(error_message);
    set_event_error(event, RAC_ERROR_DOWNLOAD_FAILED, error_message);
    rac::events::publish(event, v1::SDK_COMPONENT_UNSPECIFIED, v1::EVENT_CATEGORY_DOWNLOAD);
#else
    (void)model_id;
    (void)error_message;
#endif
