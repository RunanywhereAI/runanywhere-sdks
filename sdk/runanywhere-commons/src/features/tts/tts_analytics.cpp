...
rac_result_t rac_tts_analytics_start_synthesis(
    ...
    *out_synthesis_id = static_cast<char*>(malloc(id.size() + 1));
    ...
    log_debug("TTS.Analytics", "Synthesis started: %s, voice: %s, %d characters", id.c_str(),
              voice, character_count);

    return RAC_SUCCESS;
}

rac_result_t rac_tts_analytics_track_synthesis_chunk(
    rac_tts_analytics_handle_t handle,
...
rac_result_t rac_tts_analytics_complete_synthesis(
    rac_tts_analytics_handle_t handle,
    const char* synthesis_id,
    ...
    double chars_per_second =
        processing_time_ms > 0 ? static_cast<double>(character_count) / (processing_time_ms / 1000.0) : 0;
...
rac_result_t rac_tts_analytics_track_synthesis_failed(
    rac_tts_analytics_handle_t handle,
    const char* synthesis_id,
...
rac_result_t rac_tts_analytics_track_error(
    rac_tts_analytics_handle_t handle,
    rac_result_t error_code,
    const char* error_message,
...
rac_result_t rac_tts_analytics_get_metrics(
    rac_tts_analytics_handle_t handle,
