...
rac_result_t rac_tts_onnx_create(const char* model_path, const rac_tts_onnx_config_t* config,
    ...
    if (is_unified_kokoro) {
        ...
        kokoro_config.nnapi_config.cpu_disabled = RAC_TRUE;  // NPU-ONLY MODE - no CPU fallback, all ops must run on NPU
...
extern "C" rac_result_t rac_tts_kokoro_run_standalone_benchmark(const char* model_path, const char* test_text,
    ...
    config.nnapi_config.cpu_disabled = RAC_TRUE;  // NPU-ONLY MODE - no CPU fallback, all ops must run on NPU
    config.nnapi_config.min_api_level = 27;
    ONNX_TTS_LOG("NNAPI backend enabled for benchmark (NPU-ONLY MODE - cpu_disabled=TRUE)");
#else
