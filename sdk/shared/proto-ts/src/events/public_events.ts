...
> =
  ...
  | { type: 'usage'; requestId: string; sequence: number; usage: TUsage }
  | { type: 'completed'; requestId: string; result: TResult }
  | { type: 'failed'; requestId: string; partial?: TPartial; error: SDKError }
  | { type: 'cancelled'; requestId: string; partial?: TPartial };
 ...
 */
export type GenerationEventWithDeprecated<
  TResult = unknown,
  TPartial = Partial<TResult>,
  TToolCall = ToolCall,
  TUsage = TokenUsage,
  TItem = unknown,
> =
  | GenerationEvent<TResult, TPartial, TToolCall, TUsage, TItem>
...
export type TranscriptionEvent<
  TSegment = unknown,
  TAlternative = TranscriptAlternative,
> =
  | { type: 'started'; requestId: string }
  ...
  | { type: 'transcriptFinal'; requestId: string; sequence: number; segment: TSegment }
 ...
 */
export type RagEventWithDeprecated<TMatch = unknown, TResult = unknown> =
  | RagEvent<TMatch, TResult>
  | { type: 'token'; text: string; kind: TokenKind };

/** Progress of one image generation. */
export type ImageEvent<TResult = unknown, TPartialImage = Uint8Array> =
  | { type: 'started' }
  | { type: 'progress'; step: number; totalSteps: number; partialImage?: TPartialImage }
  | { type: 'completed'; result: TResult }
  | { type: 'failed'; error: SDKError };

/** Progress of one model download. */
export type DownloadEvent<
  TCompleted = { modelId: string },
  TProgress = {
    bytesDone: number;
    bytesTotal: number;
    file?: string;
...
> =
  ...
  | ({ type: 'progress'; operationId: string; sequence: number } & TProgress)
  | { type: 'verifying'; operationId: string; sequence: number }
  | { type: 'extracting'; operationId: string; sequence: number; percent?: number }
  | ({ type: 'completed'; operationId: string; sequence: number } & TCompleted)
