...
void progress_callback(const uint8_t* proto_bytes, size_t proto_size, void* /*user_data*/) {
    if (!g_state) {
        return;
    }
...
int pull_model_flow(const GlobalOptions& options, const std::string& model_id) {
    ...
    v1::ModelInfo model_info;
    {
        ...
        if (rac_model_registry_get_proto_buffer(rac_get_model_registry(),
                                                resolved.model_id.c_str(),
                                                &info_out) != RAC_SUCCESS ||
            !proto::parse_proto_buffer(&info_out, &model_info, &error)) {
    ...
    *plan_request.mutable_model() = model_info;
    ...
    rac_result_t rc = rac_download_plan_proto(
        reinterpret_cast<const uint8_t*>(plan_bytes.data()), plan_bytes.size(), &plan_out);
    if (rc != RAC_SUCCESS) {
    ...
    *start_request.mutable_plan() = plan;
    ...
    rc = rac_download_start_proto(reinterpret_cast<const uint8_t*>(start_bytes.data()),
                                  start_bytes.size(), &start_out);
    v1::DownloadStartResult start;
    if (rc != RAC_SUCCESS || !proto::parse_proto_buffer(&start_out, &start, &error)) {
        ...
        while (!state.terminal) {
            ...
            if (g_interrupted && !cancel_sent) {
                ...
                rac_download_cancel_proto(
                    reinterpret_cast<const uint8_t*>(cancel_bytes.data()), cancel_bytes.size(),
                    &cancel_out);
    ...
    // Report the saved entry.
    rac_proto_buffer_t model_out;
    rac_proto_buffer_init(&model_out);
    v1::ModelInfo model;
    if (rac_model_registry_get_proto_buffer(rac_get_model_registry(), resolved.model_id.c_str(),
                                            &model_out) == RAC_SUCCESS &&
        ...
        if (options.json) {
            ...
            json.begin_object()
                ...
                .field("bytes", static_cast<int64_t>(final_progress.bytes_downloaded()))
                .end_object();
            out::result_line(json.str());
        } else {
            out::result_line("pulled " + model.id() +
                             (model.local_path().empty() ? "" : " → " + model.local_path()));
        }
    } else {
        rac_proto_buffer_free(&model_out);
...
void register_pull(CLI::App& app, GlobalOptions& options) {
    CLI::App* cmd =
        app.add_subcommand("pull", "Download a model (catalog id, hf.co/... or direct URL)");
    auto ref = std::make_shared<std::string>();
    cmd->add_option("model", *ref, "Model id, alias, hf.co/org/repo/file or URL")->required();
