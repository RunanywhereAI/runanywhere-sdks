...
bool confined_to(const fs::path& target, const fs::path& models_root) {
    ...
    return target_str.size() > root_str.size() + 1 && target_str.starts_with(root_str + "/");
}
...
int run_rm(const GlobalOptions& options, const std::string& ref, bool force) {
    ...
    if (rac_model_registry_get_proto_buffer(rac_get_model_registry(), resolved.model_id.c_str(),
        ...
        !proto::parse_proto_buffer(&model_out, &model, &error)) {
    ...
    if (!model.local_path().empty()) {
        ...
        if (fs::exists(target, ec)) {
            ...
            if (ec) {
                out::error_line("failed to delete " + target.string() + ": " + ec.message());
                return 1;
            }
        }
    } else {
    ...
    url_registry::forget(resolved.model_id);
    ...
    if (options.json) {
        ...
        out::result_line("deleted " + resolved.model_id +
