...
rac_result_t bootstrap(const GlobalOptions& options, Bootstrapped* out) {
    ...
    if (!g_bootstrapped) {
        ...
        if (rc != RAC_SUCCESS) {
            out::error_line("desktop adapter init failed: " + out::describe_result(rc));
            return rc;
        }
        ...
        if (rc != RAC_SUCCESS) {
            out::error_line("HTTP transport registration failed: " + out::describe_result(rc));
            return rc;
        }
        ...
        // example apps (the registry is in-memory) — plus any user URL
        // registrations persisted by previous `rcli pull <url>` runs.
        catalog::register_all();
        url_registry::register_all_persisted();
    ...
    if (out) {
        out->home = home;
        char models[1024] = {};
        if (rac_model_paths_get_models_directory(models, sizeof(models)) == RAC_SUCCESS) {
