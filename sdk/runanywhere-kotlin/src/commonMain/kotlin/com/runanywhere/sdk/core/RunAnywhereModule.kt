...
object ModuleDiscovery {
    ...
    fun registerDiscoveredModules() {
        ...
        for (module in discovered) {
            if (!ModuleRegistryMetadata.isRegistered(module.moduleId)) {
                module.register(module.defaultPriority)
                ModuleRegistryMetadata.registerModule(module)
...
/**
 * Module metadata registry for tracking registered modules.
 *
 * This extends ModuleRegistry to track module metadata,
 * storage strategies, and download strategies.
 *
 * Matches iOS ModuleRegistry's module tracking capabilities.
 */
object ModuleRegistryMetadata {
    private val lock = Any()
    private val _registeredModules = mutableMapOf<String, ModuleMetadata>()
    private val _moduleTypes = mutableMapOf<String, RunAnywhereModule>()
    private val _storageStrategies = mutableMapOf<InferenceFramework, ModelStorageStrategy>()
    private val _downloadStrategies = mutableMapOf<InferenceFramework, DownloadStrategy>()
    private val logger = SDKLogger("ModuleRegistryMetadata")

    /**
     * Register a module with metadata tracking
     * Provides duplicate prevention (matches iOS)
     *
    ...
    fun registerModule(
        ...
        synchronized(lock) {
            // Check for duplicate registration (matches iOS)
            if (_registeredModules.containsKey(module.moduleId)) {
                logger.warning("Module '${module.moduleId}' already registered, skipping")
                return
            }
            ...
            module.storageStrategy?.let { strategy ->
                _storageStrategies[module.inferenceFramework] = strategy
                logger.info("Storage strategy registered for ${module.inferenceFramework.displayName}")
            }

            // Store download strategy if provided
            module.downloadStrategy?.let { strategy ->
                _downloadStrategies[module.inferenceFramework] = strategy
                logger.info("Download strategy registered for ${module.inferenceFramework.displayName}")
            }
    ...
    fun isRegistered(moduleId: String): Boolean =
        synchronized(lock) { _registeredModules.containsKey(moduleId) }

    /**
     * Get metadata for a registered module
     */
    fun metadata(moduleId: String): ModuleMetadata? =
        synchronized(lock) { _registeredModules[moduleId] }

    /**
     * Get all registered module IDs
     */
    val moduleIds: List<String>
        get() = synchronized(lock) { _registeredModules.keys.sorted() }
     ...
     */
    fun modules(capability: CapabilityType): List<ModuleMetadata> =
        synchronized(lock) {
     ...
     */
    fun moduleIds(capability: CapabilityType): List<String> =
        modules(capability).map { it.moduleId }

    /**
     * Check if any module provides a specific capability
     */
    fun hasCapability(capability: CapabilityType): Boolean =
        synchronized(lock) {
            _registeredModules.values.any { it.capabilities.contains(capability) }
        }
    ...
    fun storageStrategy(framework: InferenceFramework): ModelStorageStrategy? =
        synchronized(lock) { _storageStrategies[framework] }

    /**
     * Check if a storage strategy is registered for a framework
     */
    fun hasStorageStrategy(framework: InferenceFramework): Boolean =
        synchronized(lock) { _storageStrategies.containsKey(framework) }

    /**
     * Get the download strategy for a framework
     */
    fun downloadStrategy(framework: InferenceFramework): DownloadStrategy? =
        synchronized(lock) { _downloadStrategies[framework] }

    /**
     * Get all registered download strategies
     */
    val allDownloadStrategies: List<DownloadStrategy>
        get() = synchronized(lock) { _downloadStrategies.values.toList() }

    /**
     * Check if a download strategy is registered for a framework
     */
    fun hasDownloadStrategy(framework: InferenceFramework): Boolean =
        synchronized(lock) { _downloadStrategies.containsKey(framework) }
    ...
    fun reset() {
        ...
        logger.info("Module registry metadata reset")
    }
}
