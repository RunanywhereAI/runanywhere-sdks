...
public class ModelRegistryEvent(
  ...
  public val kind: ModelRegistryEventKind =
  ...
  init {
    require(countNonNull(refresh_result, list_result, get_result, import_result, discovery_result,
        compatibility_result, current_model_result) <= 1) {
  ...
  ): ModelRegistryEvent = ModelRegistryEvent(kind, model_id, assignment_id, assigned_component,
      framework, source_path, error, refresh_result, list_result, get_result, import_result,
      discovery_result, compatibility_result, current_model_result, unknownFields)

  public companion object {
    @JvmField
    public val ADAPTER: ProtoAdapter<ModelRegistryEvent> = object :
        ProtoAdapter<ModelRegistryEvent>(
      ...
      override fun encodedSize(`value`: ModelRegistryEvent): Int {
        ...
        if (value.kind != ModelRegistryEventKind.MODEL_REGISTRY_EVENT_KIND_UNSPECIFIED) size +=
            ModelRegistryEventKind.ADAPTER.encodedSizeWithTag(1, value.kind)
        if (value.model_id != "") size += ProtoAdapter.STRING.encodedSizeWithTag(2, value.model_id)
        if (value.assignment_id != "") size += ProtoAdapter.STRING.encodedSizeWithTag(3,
            value.assignment_id)
        if (value.assigned_component != SDKComponent.SDK_COMPONENT_UNSPECIFIED) size +=
            SDKComponent.ADAPTER.encodedSizeWithTag(4, value.assigned_component)
        ...
        if (value.source_path != "") size += ProtoAdapter.STRING.encodedSizeWithTag(6,
            value.source_path)
      ...
      override fun encode(writer: ProtoWriter, `value`: ModelRegistryEvent) {
        if (value.kind != ModelRegistryEventKind.MODEL_REGISTRY_EVENT_KIND_UNSPECIFIED)
        ...
        if (value.assignment_id != "") ProtoAdapter.STRING.encodeWithTag(writer, 3,
            value.assignment_id)
        if (value.assigned_component != SDKComponent.SDK_COMPONENT_UNSPECIFIED)
        ...
        if (value.source_path != "") ProtoAdapter.STRING.encodeWithTag(writer, 6, value.source_path)
      ...
      override fun decode(reader: ProtoReader): ModelRegistryEvent {
        var kind: ModelRegistryEventKind =
            ModelRegistryEventKind.MODEL_REGISTRY_EVENT_KIND_UNSPECIFIED
        var model_id: String = ""
      ...
      override fun redact(`value`: ModelRegistryEvent): ModelRegistryEvent = value.copy(
        ...
        compatibility_result =
            value.compatibility_result?.let(ModelCompatibilityResult.ADAPTER::redact),
