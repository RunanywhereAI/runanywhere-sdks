package com.runanywhere.sdk.capabilities.tts

import com.runanywhere.sdk.components.TTSComponent
import com.runanywhere.sdk.components.TTSConfiguration
import com.runanywhere.sdk.components.TTSOutput as ComponentTTSOutput
import com.runanywhere.sdk.components.TTSOptions as ComponentTTSOptions
import com.runanywhere.sdk.components.AudioFormat as ComponentAudioFormat
...
class TTSCapability internal constructor(
    ...
    val isVoiceLoaded: Boolean
        get() = _isVoiceLoaded && getComponent().state == com.runanywhere.sdk.components.base.ComponentState.READY
    ...
    fun synthesizeStream(text: String, options: TTSOptions): Flow<ByteArray> {
        ...
        return component.synthesizeStream(text, componentOptions)
    }
    ...
    private fun ComponentTTSOutput.toTTSResult(): TTSResult {
        return TTSResult(
            audioData = this.audioData,
            format = when (this.format) {
                ComponentAudioFormat.PCM -> AudioFormat.PCM_16BIT
                ComponentAudioFormat.WAV -> AudioFormat.WAV
                ComponentAudioFormat.MP3 -> AudioFormat.MP3
                ComponentAudioFormat.AAC -> AudioFormat.PCM_16BIT // fallback
                ComponentAudioFormat.FLAC -> AudioFormat.PCM_16BIT // fallback
                ComponentAudioFormat.OPUS -> AudioFormat.OGG
            },
            duration = this.duration
        )
    }

    private fun TTSOptions.toComponentOptions(): ComponentTTSOptions {
        return ComponentTTSOptions(
            voice = this.voiceId,
            language = "en-US",
            rate = this.rate,
            pitch = this.pitch,
            volume = this.volume,
            audioFormat = ComponentAudioFormat.PCM,
            sampleRate = 16000,
            useSSML = false
        )
    }
...
/**
 * Internal TTS options used by the capability layer
 */
...
/**
 * Internal TTS result with audio data
 */
