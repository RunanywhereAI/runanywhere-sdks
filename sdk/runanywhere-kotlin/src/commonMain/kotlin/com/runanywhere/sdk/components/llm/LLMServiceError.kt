...
sealed class LLMServiceError : Exception() {
    ...
    /** Model not found */
    data class ModelNotFound(val model: String) : LLMServiceError() {
        override val message: String = "Model not found: $model"
    }

    /** Generation failed */
    data class GenerationFailed(override val cause: Throwable) : LLMServiceError() {
    ...
    data class ModelLoadFailed(val model: String, override val cause: Throwable) : LLMServiceError() {
        override val message: String = "Failed to load model: $model - ${cause.message}"
    }

    /** Service initialization failed */
    data class InitializationFailed(val reason: String) : LLMServiceError() {
        override val message: String = "LLM service initialization failed: $reason"
    }

    /** Insufficient memory */
    data class InsufficientMemory(val required: Long, val available: Long) : LLMServiceError() {
        override val message: String = "Insufficient memory: required ${required / 1024 / 1024}MB, available ${available / 1024 / 1024}MB"
    }

    /** Unsupported model format */
    data class UnsupportedFormat(val format: String) : LLMServiceError() {
        override val message: String = "Unsupported model format: $format"
    }

    /** Generation timeout */
    data class GenerationTimeout(val timeoutMs: Long) : LLMServiceError() {
        override val message: String = "Generation timed out after ${timeoutMs}ms"
    }

    /** Service unavailable */
    data class ServiceUnavailable(val reason: String) : LLMServiceError() {
        override val message: String = "LLM service unavailable: $reason"
    }

    /** Invalid input */
    data class InvalidInput(val reason: String) : LLMServiceError() {
        override val message: String = "Invalid input: $reason"
    }

    /** Model download failed */
    data class ModelDownloadFailed(val model: String, override val cause: Throwable) : LLMServiceError() {
        override val message: String = "Failed to download model: $model - ${cause.message}"
    }

    /** Hardware not supported */
    data class HardwareNotSupported(val hardware: String) : LLMServiceError() {
        override val message: String = "Hardware not supported: $hardware"
    }

    /** Framework not available */
    data class FrameworkNotAvailable(val framework: String) : LLMServiceError() {
