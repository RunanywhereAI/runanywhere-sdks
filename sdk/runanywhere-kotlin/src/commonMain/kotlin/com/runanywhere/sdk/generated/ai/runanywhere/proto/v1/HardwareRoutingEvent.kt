...
public class HardwareRoutingEvent(
  ...
  public val kind: HardwareRoutingEventKind =
  ...
  ): HardwareRoutingEvent = HardwareRoutingEvent(kind, component, framework, capability, route,
      reason, error, hardware_profile, unknownFields)

  public companion object {
    @JvmField
    public val ADAPTER: ProtoAdapter<HardwareRoutingEvent> = object :
        ProtoAdapter<HardwareRoutingEvent>(
      ...
      override fun encodedSize(`value`: HardwareRoutingEvent): Int {
        ...
        if (value.kind != HardwareRoutingEventKind.HARDWARE_ROUTING_EVENT_KIND_UNSPECIFIED) size +=
            HardwareRoutingEventKind.ADAPTER.encodedSizeWithTag(1, value.kind)
        ...
        if (value.framework != InferenceFramework.INFERENCE_FRAMEWORK_UNSPECIFIED) size +=
            InferenceFramework.ADAPTER.encodedSizeWithTag(3, value.framework)
        if (value.capability != "") size += ProtoAdapter.STRING.encodedSizeWithTag(4,
            value.capability)
        if (value.route != "") size += ProtoAdapter.STRING.encodedSizeWithTag(5, value.route)
        if (value.reason != "") size += ProtoAdapter.STRING.encodedSizeWithTag(6, value.reason)
        if (value.error != "") size += ProtoAdapter.STRING.encodedSizeWithTag(7, value.error)
        if (value.hardware_profile != null) size +=
            HardwareProfileResult.ADAPTER.encodedSizeWithTag(20, value.hardware_profile)
        return size
      }

      override fun encode(writer: ProtoWriter, `value`: HardwareRoutingEvent) {
        if (value.kind != HardwareRoutingEventKind.HARDWARE_ROUTING_EVENT_KIND_UNSPECIFIED)
        ...
        if (value.capability != "") ProtoAdapter.STRING.encodeWithTag(writer, 4, value.capability)
        if (value.route != "") ProtoAdapter.STRING.encodeWithTag(writer, 5, value.route)
        if (value.reason != "") ProtoAdapter.STRING.encodeWithTag(writer, 6, value.reason)
        if (value.error != "") ProtoAdapter.STRING.encodeWithTag(writer, 7, value.error)
        if (value.hardware_profile != null) HardwareProfileResult.ADAPTER.encodeWithTag(writer, 20,
      ...
      override fun decode(reader: ProtoReader): HardwareRoutingEvent {
        var kind: HardwareRoutingEventKind =
            HardwareRoutingEventKind.HARDWARE_ROUTING_EVENT_KIND_UNSPECIFIED
