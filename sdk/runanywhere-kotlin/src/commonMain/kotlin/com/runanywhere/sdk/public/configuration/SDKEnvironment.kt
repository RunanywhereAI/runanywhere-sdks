...
/** Backing for [SDKEnvironment.requiresAuthentication]. Calls JNI when available. */
internal expect fun sdkEnvironmentRequiresAuthentication(env: SDKEnvironment): Boolean

/** Backing for [SDKEnvironment.requiresBackendURL]. Calls JNI when available. */
internal expect fun sdkEnvironmentRequiresBackendURL(env: SDKEnvironment): Boolean

/** Backing for [SDKInitParams.validate] — returns true when [key] is acceptable. */
internal expect fun sdkInitParamsValidateApiKey(key: String, env: SDKEnvironment): Boolean

/** Backing for [SDKInitParams.validate] — returns true when [url] is acceptable. */
internal expect fun sdkInitParamsValidateBaseUrl(url: String, env: SDKEnvironment): Boolean

/**
 * Backing for [SDKInitParams.validate] — returns the canonical error message
 * for an invalid `(env, key, url)` triple, or null when validation passes.
 */
internal expect fun sdkInitParamsValidationErrorMessage(
    env: SDKEnvironment,
    key: String,
    url: String,
): String?
