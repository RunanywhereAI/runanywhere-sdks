...
class ManagedLifecycle<ServiceType : Any>(
     ...
     */
    fun trackOperationError(error: Throwable, operation: String) {
     ...
     */
    suspend fun getLifecycleMetrics(): ModelLifecycleMetrics = mutex.withLock {
    ...
    private fun createLLMEvent(
        ...
        return when (type) {
            ...
            LifecycleEventType.LOAD_COMPLETED -> LLMEvent.ModelLoadCompleted(
                modelId = resourceId,
                durationMs = durationMs ?: 0.0
            )
            LifecycleEventType.LOAD_FAILED -> LLMEvent.ModelLoadFailed(
                modelId = resourceId,
    ...
    private fun createTTSEvent(
        ...
        return when (type) {
            ...
            LifecycleEventType.LOAD_COMPLETED -> TTSEvent.ModelLoadCompleted(
                voiceId = resourceId,
                durationMs = durationMs ?: 0.0
            )
            LifecycleEventType.LOAD_FAILED -> TTSEvent.ModelLoadFailed(
                voiceId = resourceId,
                error = error?.message ?: "Unknown error"
            )
    ...
    private fun createVADEvent(
        ...
        return when (type) {
            ...
            LifecycleEventType.LOAD_COMPLETED -> VADEvent.ModelLoadCompleted(
                modelId = resourceId,
                durationMs = durationMs ?: 0.0
            )
            LifecycleEventType.LOAD_FAILED -> VADEvent.ModelLoadFailed(
                modelId = resourceId,
                error = error?.message ?: "Unknown error"
            )
