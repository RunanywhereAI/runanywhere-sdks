...
public class ChatStreamEvent(
  ...
  ): ChatStreamEvent = ChatStreamEvent(seq, timestamp_us, request_id, conversation_id, kind, token,
      message, tool_call, tool_result, final_result, error_message, error_code, unknownFields)
    ...
    public val ADAPTER: ProtoAdapter<ChatStreamEvent> = object : ProtoAdapter<ChatStreamEvent>(
      ...
      override fun encodedSize(`value`: ChatStreamEvent): Int {
        ...
        if (value.request_id != "") size += ProtoAdapter.STRING.encodedSizeWithTag(3,
            value.request_id)
        if (value.conversation_id != "") size += ProtoAdapter.STRING.encodedSizeWithTag(4,
            value.conversation_id)
        if (value.kind != ChatStreamEventKind.CHAT_STREAM_EVENT_KIND_UNSPECIFIED) size +=
            ChatStreamEventKind.ADAPTER.encodedSizeWithTag(5, value.kind)
      ...
      override fun encode(writer: ProtoWriter, `value`: ChatStreamEvent) {
        ...
        if (value.conversation_id != "") ProtoAdapter.STRING.encodeWithTag(writer, 4,
            value.conversation_id)
        if (value.kind != ChatStreamEventKind.CHAT_STREAM_EVENT_KIND_UNSPECIFIED)
