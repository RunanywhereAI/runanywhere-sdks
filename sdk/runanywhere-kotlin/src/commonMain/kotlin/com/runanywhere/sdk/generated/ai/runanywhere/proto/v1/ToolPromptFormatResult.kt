// Code generated by Wire protocol buffer compiler, do not edit.
// Source: runanywhere.v1.ToolPromptFormatResult in tool_calling.proto
@file:Suppress("DEPRECATION")

package ai.runanywhere.proto.v1

import com.squareup.wire.FieldEncoding
import com.squareup.wire.Message
import com.squareup.wire.ProtoAdapter
import com.squareup.wire.ProtoReader
import com.squareup.wire.ProtoWriter
import com.squareup.wire.ReverseProtoWriter
import com.squareup.wire.Syntax.PROTO_3
import com.squareup.wire.WireField
import com.squareup.wire.`internal`.JvmField
import com.squareup.wire.`internal`.sanitize
import kotlin.Any
import kotlin.AssertionError
import kotlin.Boolean
import kotlin.Deprecated
import kotlin.DeprecationLevel
import kotlin.Int
import kotlin.Long
import kotlin.Nothing
import kotlin.String
import kotlin.Suppress
import okio.ByteString

public class ToolPromptFormatResult(
  @field:WireField(
    tag = 1,
    adapter = "com.squareup.wire.ProtoAdapter#STRING",
    label = WireField.Label.OMIT_IDENTITY,
    jsonName = "formattedPrompt",
    schemaIndex = 0,
  )
  public val formatted_prompt: String = "",
  @field:WireField(
    tag = 2,
    adapter = "ai.runanywhere.proto.v1.ToolCallFormatName#ADAPTER",
    label = WireField.Label.OMIT_IDENTITY,
    schemaIndex = 1,
  )
  public val format: ToolCallFormatName = ToolCallFormatName.TOOL_CALL_FORMAT_NAME_UNSPECIFIED,
  @field:WireField(
    tag = 3,
    adapter = "com.squareup.wire.ProtoAdapter#STRING",
    label = WireField.Label.OMIT_IDENTITY,
    jsonName = "formatHint",
    schemaIndex = 2,
  )
  public val format_hint: String = "",
  @field:WireField(
    tag = 4,
    adapter = "com.squareup.wire.ProtoAdapter#STRING",
    jsonName = "errorMessage",
    schemaIndex = 3,
  )
  public val error_message: String? = null,
  @field:WireField(
    tag = 5,
    adapter = "com.squareup.wire.ProtoAdapter#INT32",
    label = WireField.Label.OMIT_IDENTITY,
    jsonName = "errorCode",
    schemaIndex = 4,
  )
  public val error_code: Int = 0,
  unknownFields: ByteString = ByteString.EMPTY,
) : Message<ToolPromptFormatResult, Nothing>(ADAPTER, unknownFields) {
  @Deprecated(
    message = "Shouldn't be used in Kotlin",
    level = DeprecationLevel.HIDDEN,
  )
  override fun newBuilder(): Nothing = throw
      AssertionError("Builders are deprecated and only available in a javaInterop build; see https://square.github.io/wire/wire_compiler/#kotlin")

  override fun equals(other: Any?): Boolean {
    if (other === this) return true
    if (other !is ToolPromptFormatResult) return false
    if (unknownFields != other.unknownFields) return false
    if (formatted_prompt != other.formatted_prompt) return false
    if (format != other.format) return false
    if (format_hint != other.format_hint) return false
    if (error_message != other.error_message) return false
    if (error_code != other.error_code) return false
    return true
  }

  override fun hashCode(): Int {
    var result = super.hashCode
    if (result == 0) {
      result = unknownFields.hashCode()
      result = result * 37 + formatted_prompt.hashCode()
      result = result * 37 + format.hashCode()
      result = result * 37 + format_hint.hashCode()
      result = result * 37 + (error_message?.hashCode() ?: 0)
      result = result * 37 + error_code.hashCode()
      super.hashCode = result
    }
    return result
  }

  override fun toString(): String {
    val result = mutableListOf<String>()
    result += """formatted_prompt=${sanitize(formatted_prompt)}"""
    result += """format=$format"""
    result += """format_hint=${sanitize(format_hint)}"""
    if (error_message != null) result += """error_message=${sanitize(error_message)}"""
    result += """error_code=$error_code"""
    return result.joinToString(prefix = "ToolPromptFormatResult{", separator = ", ", postfix = "}")
  }

  public fun copy(
    formatted_prompt: String = this.formatted_prompt,
    format: ToolCallFormatName = this.format,
    format_hint: String = this.format_hint,
    error_message: String? = this.error_message,
    error_code: Int = this.error_code,
    unknownFields: ByteString = this.unknownFields,
  ): ToolPromptFormatResult = ToolPromptFormatResult(formatted_prompt, format, format_hint,
      error_message, error_code, unknownFields)

  public companion object {
    @JvmField
    public val ADAPTER: ProtoAdapter<ToolPromptFormatResult> = object :
        ProtoAdapter<ToolPromptFormatResult>(
      FieldEncoding.LENGTH_DELIMITED, 
      ToolPromptFormatResult::class, 
      "type.googleapis.com/runanywhere.v1.ToolPromptFormatResult", 
      PROTO_3, 
      null, 
      "tool_calling.proto"
    ) {
      override fun encodedSize(`value`: ToolPromptFormatResult): Int {
        var size = value.unknownFields.size
        if (value.formatted_prompt != "") size += ProtoAdapter.STRING.encodedSizeWithTag(1,
            value.formatted_prompt)
        if (value.format != ToolCallFormatName.TOOL_CALL_FORMAT_NAME_UNSPECIFIED) size +=
            ToolCallFormatName.ADAPTER.encodedSizeWithTag(2, value.format)
        if (value.format_hint != "") size += ProtoAdapter.STRING.encodedSizeWithTag(3,
            value.format_hint)
        size += ProtoAdapter.STRING.encodedSizeWithTag(4, value.error_message)
        if (value.error_code != 0) size += ProtoAdapter.INT32.encodedSizeWithTag(5,
            value.error_code)
        return size
      }

      override fun encode(writer: ProtoWriter, `value`: ToolPromptFormatResult) {
        if (value.formatted_prompt != "") ProtoAdapter.STRING.encodeWithTag(writer, 1,
            value.formatted_prompt)
        if (value.format != ToolCallFormatName.TOOL_CALL_FORMAT_NAME_UNSPECIFIED)
            ToolCallFormatName.ADAPTER.encodeWithTag(writer, 2, value.format)
        if (value.format_hint != "") ProtoAdapter.STRING.encodeWithTag(writer, 3, value.format_hint)
        ProtoAdapter.STRING.encodeWithTag(writer, 4, value.error_message)
        if (value.error_code != 0) ProtoAdapter.INT32.encodeWithTag(writer, 5, value.error_code)
        writer.writeBytes(value.unknownFields)
      }

      override fun encode(writer: ReverseProtoWriter, `value`: ToolPromptFormatResult) {
        writer.writeBytes(value.unknownFields)
        if (value.error_code != 0) ProtoAdapter.INT32.encodeWithTag(writer, 5, value.error_code)
        ProtoAdapter.STRING.encodeWithTag(writer, 4, value.error_message)
        if (value.format_hint != "") ProtoAdapter.STRING.encodeWithTag(writer, 3, value.format_hint)
        if (value.format != ToolCallFormatName.TOOL_CALL_FORMAT_NAME_UNSPECIFIED)
            ToolCallFormatName.ADAPTER.encodeWithTag(writer, 2, value.format)
        if (value.formatted_prompt != "") ProtoAdapter.STRING.encodeWithTag(writer, 1,
            value.formatted_prompt)
      }

      override fun decode(reader: ProtoReader): ToolPromptFormatResult {
        var formatted_prompt: String = ""
        var format: ToolCallFormatName = ToolCallFormatName.TOOL_CALL_FORMAT_NAME_UNSPECIFIED
        var format_hint: String = ""
        var error_message: String? = null
        var error_code: Int = 0
        val unknownFields = reader.forEachTag { tag ->
          when (tag) {
            1 -> formatted_prompt = ProtoAdapter.STRING.decode(reader)
            2 -> try {
              format = ToolCallFormatName.ADAPTER.decode(reader)
            } catch (e: ProtoAdapter.EnumConstantNotFoundException) {
              reader.addUnknownField(tag, FieldEncoding.VARINT, e.value.toLong())
            }
            3 -> format_hint = ProtoAdapter.STRING.decode(reader)
            4 -> error_message = ProtoAdapter.STRING.decode(reader)
            5 -> error_code = ProtoAdapter.INT32.decode(reader)
            else -> reader.readUnknownField(tag)
          }
        }
        return ToolPromptFormatResult(
          formatted_prompt = formatted_prompt,
          format = format,
          format_hint = format_hint,
          error_message = error_message,
          error_code = error_code,
          unknownFields = unknownFields
        )
      }

      override fun redact(`value`: ToolPromptFormatResult): ToolPromptFormatResult = value.copy(
        unknownFields = ByteString.EMPTY
      )
    }

    private const val serialVersionUID: Long = 0L
  }
}
