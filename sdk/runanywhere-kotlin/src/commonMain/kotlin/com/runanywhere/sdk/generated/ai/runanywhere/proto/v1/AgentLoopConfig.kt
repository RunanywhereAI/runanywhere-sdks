    ...
    public val ADAPTER: ProtoAdapter<AgentLoopConfig> = object : ProtoAdapter<AgentLoopConfig>(
      ...
      override fun encodedSize(`value`: AgentLoopConfig): Int {
        ...
        if (value.llm_model_id != "") size += ProtoAdapter.STRING.encodedSizeWithTag(1,
            value.llm_model_id)
        if (value.system_prompt != "") size += ProtoAdapter.STRING.encodedSizeWithTag(2,
            value.system_prompt)
        size += ToolSpec.ADAPTER.asRepeated().encodedSizeWithTag(3, value.tools)
        if (value.max_iterations != 0) size += ProtoAdapter.INT32.encodedSizeWithTag(4,
      ...
      override fun encode(writer: ProtoWriter, `value`: AgentLoopConfig) {
        if (value.llm_model_id != "") ProtoAdapter.STRING.encodeWithTag(writer, 1,
            value.llm_model_id)
        if (value.system_prompt != "") ProtoAdapter.STRING.encodeWithTag(writer, 2,
            value.system_prompt)
