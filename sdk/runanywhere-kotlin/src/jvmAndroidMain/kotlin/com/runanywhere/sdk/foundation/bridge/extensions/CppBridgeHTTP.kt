...
object CppBridgeHTTP {
    ...
    fun register() {
        synchronized(lock) {
            ...
            val result = com.runanywhere.sdk.native.bridge.RunAnywhereBridge.racHttpSetExecutor(this)
            if (result != com.runanywhere.sdk.native.bridge.RunAnywhereBridge.RAC_SUCCESS) {
                CppBridgePlatformAdapter.logCallback(
                    CppBridgePlatformAdapter.LogLevel.ERROR,
                    TAG,
                    "Failed to register HTTP executor: $result",
                )
                return
            }
            ...
            CppBridgePlatformAdapter.logCallback(
                ...
                TAG,
                "HTTP executor registered with C++ core",
            )
        }
    }
    ...
    // ========================================================================
    // HTTP EXECUTOR CALLBACK (called by C++ via JNI for cloud backends)
    // ========================================================================

    /**
     * Called by C++ via JNI when the C++ HTTP client needs to make a request.
     * Supports binary bodies (multipart/form-data for audio uploads).
     */
    @Suppress("unused")
    fun httpExecutorCallback(
        requestId: Long,
        url: String,
        method: Int,
        headersJson: String?,
        ...
        CppBridgePlatformAdapter.logCallback(
            ...
            TAG,
            "HTTP executor: ${HttpMethod.getName(method)} $url (body=${body?.size ?: 0} bytes)",
        )

        httpExecutor.execute {
            executeBinaryHttpRequest(requestId, url, method, headersJson, body, timeoutMs)
        }
    }

    private fun executeBinaryHttpRequest(
        requestId: Long,
        url: String,
        method: Int,
        headersJson: String?,
        ...
        try {
            ...
            val timeout = if (timeoutMs > 0) timeoutMs else DEFAULT_CONNECT_TIMEOUT_MS
            ...
            // Write binary body
            if (body != null && body.isNotEmpty() && method != HttpMethod.GET && method != HttpMethod.HEAD) {
            ...
            val inputStream =
                if (HttpStatus.isSuccess(statusCode)) connection.inputStream
                else connection.errorStream

            val responseBytes = inputStream?.use { it.readBytes() }

            val responseStr = responseBytes?.let { String(it, Charsets.UTF_8) }
            CppBridgePlatformAdapter.logCallback(
                CppBridgePlatformAdapter.LogLevel.DEBUG,
                TAG,
                "HTTP executor response: $statusCode (${System.currentTimeMillis() - startTime}ms)" +
                    if (!HttpStatus.isSuccess(statusCode) && responseStr != null) " body=$responseStr" else "",
            )

            com.runanywhere.sdk.native.bridge.RunAnywhereBridge.racHttpExecutorComplete(
                requestId, statusCode, responseBytes, null, HttpErrorCode.NONE, null,
            )
        } catch (e: java.net.SocketTimeoutException) {
            errorMessage = "Timeout: ${e.message}"
            com.runanywhere.sdk.native.bridge.RunAnywhereBridge.racHttpExecutorComplete(
                requestId, -1, null, null, HttpErrorCode.TIMEOUT, errorMessage,
            )
        } catch (e: javax.net.ssl.SSLException) {
            errorMessage = "SSL: ${e.message}"
            com.runanywhere.sdk.native.bridge.RunAnywhereBridge.racHttpExecutorComplete(
                requestId, -1, null, null, HttpErrorCode.SSL_ERROR, errorMessage,
            )
        } catch (e: Exception) {
            errorMessage = e.message ?: "Unknown error"
            com.runanywhere.sdk.native.bridge.RunAnywhereBridge.racHttpExecutorComplete(
                requestId, -1, null, null, HttpErrorCode.NETWORK_ERROR, errorMessage,
            )
        } finally {
    ...
    // ========================================================================
    // HTTP CALLBACK (legacy — used by Kotlin-side direct calls)
    // ========================================================================

    /**
     * HTTP callback invoked by Kotlin code to perform HTTP requests.
     */
    @JvmStatic
    fun httpCallback(
        requestId: String?,
