    ...
    companion object {
        ...
        fun getInstance(): InMemoryDatabase =
            instance ?: synchronized(this) {
                val newInstance = InMemoryDatabase()
...
class InMemoryTelemetryDao : TelemetryDao {
    ...
    override suspend fun getEventById(eventId: String): TelemetryEventEntity? =
        mutex.withLock {
            events[eventId]
        }

    override suspend fun getAllEvents(): List<TelemetryEventEntity> =
        mutex.withLock {
            events.values.sortedByDescending { it.timestamp }
        }

    override suspend fun getEventsBySession(sessionId: String): List<TelemetryEventEntity> =
        mutex.withLock {
            events.values.filter { it.sessionId == sessionId }.sortedByDescending { it.timestamp }
        }

    override suspend fun getEventsByType(eventType: TelemetryEventType): List<TelemetryEventEntity> =
        mutex.withLock {
            events.values.filter { it.type == eventType }.sortedByDescending { it.timestamp }
        }

    override suspend fun getUnsentEvents(): List<TelemetryEventEntity> =
        mutex.withLock {
            events.values.filter { !it.isSent }.sortedBy { it.timestamp }
        }

    override suspend fun getSentEvents(): List<TelemetryEventEntity> =
        mutex.withLock {
            events.values.filter { it.isSent }.sortedByDescending { it.timestamp }
        }

    override suspend fun getFailedEvents(): List<TelemetryEventEntity> =
        mutex.withLock {
    ...
    override suspend fun getEventsByDevice(deviceId: String): List<TelemetryEventEntity> =
        mutex.withLock {
            events.values.filter { it.deviceId == deviceId }.sortedByDescending { it.timestamp }
        }

    override suspend fun getRecentEvents(limit: Int): List<TelemetryEventEntity> =
        mutex.withLock {
            events.values.sortedByDescending { it.timestamp }.take(limit)
        }

    override suspend fun insertEvent(event: TelemetryEventEntity) =
        mutex.withLock {
            events[event.id] = event
        }

    override suspend fun insertEvents(events: List<TelemetryEventEntity>) =
        mutex.withLock {
    ...
    override suspend fun updateEvent(event: TelemetryEventEntity) =
        mutex.withLock {
            events[event.id] = event
        }
    ...
    override suspend fun incrementRetryCount(eventIds: List<String>) =
        mutex.withLock {
            eventIds.forEach { id ->
                events[id]?.let { event ->
    ...
    override suspend fun getFailedEventCount(): Int =
        mutex.withLock {
            events.values.count { !it.success }
        }

    override suspend fun getEventCountByType(eventType: TelemetryEventType): Int =
        mutex.withLock {
            events.values.count { it.type == eventType }
        }

    override suspend fun getAverageDurationByType(eventType: TelemetryEventType): Double? =
        mutex.withLock {
...
class InMemoryConfigurationDao : ConfigurationDao {
    ...
    override suspend fun getConfigurationBySource(source: ConfigurationSource) = null

    override suspend fun getConfigurationByEnvironment(environment: SDKEnvironment) = null
...
class InMemoryModelInfoDao : ModelInfoDao {
    ...
    override suspend fun updateLocalPath(
        modelId: String,
        localPath: String?,
        updatedAt: Long,
    ) {}
...
class InMemoryDeviceInfoDao : DeviceInfoDao {
    ...
    override suspend fun updateDynamicInfo(
        deviceId: String,
