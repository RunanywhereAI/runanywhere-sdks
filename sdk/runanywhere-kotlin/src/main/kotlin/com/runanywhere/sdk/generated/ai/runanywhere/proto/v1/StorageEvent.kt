...
public class StorageEvent(
  ...
  ): StorageEvent = StorageEvent(kind, model_id, error, total_bytes, available_bytes, used_bytes,
      stored_model_count, cache_key, evicted_bytes, freed_bytes, unknownFields)
  ...
  public companion object {
    ...
    public val ADAPTER: ProtoAdapter<StorageEvent> = object : ProtoAdapter<StorageEvent>(
      ...
      override fun encodedSize(`value`: StorageEvent): Int {
        ...
        if (value.kind != StorageEventKind.STORAGE_EVENT_KIND_UNSPECIFIED) size +=
            StorageEventKind.ADAPTER.encodedSizeWithTag(1, value.kind)
        if (value.model_id != "") size += ProtoAdapter.STRING.encodedSizeWithTag(2, value.model_id)
        if (value.error != "") size += ProtoAdapter.STRING.encodedSizeWithTag(3, value.error)
        if (value.total_bytes != 0L) size += ProtoAdapter.INT64.encodedSizeWithTag(4,
            value.total_bytes)
        if (value.available_bytes != 0L) size += ProtoAdapter.INT64.encodedSizeWithTag(5,
            value.available_bytes)
        if (value.used_bytes != 0L) size += ProtoAdapter.INT64.encodedSizeWithTag(6,
            value.used_bytes)
        if (value.stored_model_count != 0) size += ProtoAdapter.INT32.encodedSizeWithTag(7,
            value.stored_model_count)
        if (value.cache_key != "") size += ProtoAdapter.STRING.encodedSizeWithTag(8,
            value.cache_key)
        if (value.evicted_bytes != 0L) size += ProtoAdapter.INT64.encodedSizeWithTag(9,
            value.evicted_bytes)
        if (value.freed_bytes != 0L) size += ProtoAdapter.INT64.encodedSizeWithTag(10,
            value.freed_bytes)
        return size
      }

      override fun encode(writer: ProtoWriter, `value`: StorageEvent) {
        if (value.kind != StorageEventKind.STORAGE_EVENT_KIND_UNSPECIFIED)
        ...
        if (value.error != "") ProtoAdapter.STRING.encodeWithTag(writer, 3, value.error)
        if (value.total_bytes != 0L) ProtoAdapter.INT64.encodeWithTag(writer, 4, value.total_bytes)
        if (value.available_bytes != 0L) ProtoAdapter.INT64.encodeWithTag(writer, 5,
            value.available_bytes)
        if (value.used_bytes != 0L) ProtoAdapter.INT64.encodeWithTag(writer, 6, value.used_bytes)
        if (value.stored_model_count != 0) ProtoAdapter.INT32.encodeWithTag(writer, 7,
            value.stored_model_count)
        if (value.cache_key != "") ProtoAdapter.STRING.encodeWithTag(writer, 8, value.cache_key)
        if (value.evicted_bytes != 0L) ProtoAdapter.INT64.encodeWithTag(writer, 9,
            value.evicted_bytes)
        if (value.freed_bytes != 0L) ProtoAdapter.INT64.encodeWithTag(writer, 10, value.freed_bytes)
