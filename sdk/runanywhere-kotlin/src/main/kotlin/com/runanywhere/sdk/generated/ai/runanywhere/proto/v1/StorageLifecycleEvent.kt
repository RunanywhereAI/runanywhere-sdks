...
public class StorageLifecycleEvent(
  ...
  public val kind: StorageLifecycleEventKind =
  ...
  ): StorageLifecycleEvent = StorageLifecycleEvent(kind, model_id, cache_key, bytes, error,
      info_result, availability_result, delete_plan, delete_result, unknownFields)

  public companion object {
    @JvmField
    public val ADAPTER: ProtoAdapter<StorageLifecycleEvent> = object :
        ProtoAdapter<StorageLifecycleEvent>(
      ...
      override fun encodedSize(`value`: StorageLifecycleEvent): Int {
        ...
        if (value.kind != StorageLifecycleEventKind.STORAGE_LIFECYCLE_EVENT_KIND_UNSPECIFIED)
        ...
        if (value.cache_key != "") size += ProtoAdapter.STRING.encodedSizeWithTag(3,
            value.cache_key)
        if (value.bytes != 0L) size += ProtoAdapter.INT64.encodedSizeWithTag(4, value.bytes)
        if (value.error != "") size += ProtoAdapter.STRING.encodedSizeWithTag(5, value.error)
      ...
      override fun encode(writer: ProtoWriter, `value`: StorageLifecycleEvent) {
        ...
        if (value.cache_key != "") ProtoAdapter.STRING.encodeWithTag(writer, 3, value.cache_key)
        if (value.bytes != 0L) ProtoAdapter.INT64.encodeWithTag(writer, 4, value.bytes)
        if (value.error != "") ProtoAdapter.STRING.encodeWithTag(writer, 5, value.error)
      ...
      override fun decode(reader: ProtoReader): StorageLifecycleEvent {
        var kind: StorageLifecycleEventKind =
            StorageLifecycleEventKind.STORAGE_LIFECYCLE_EVENT_KIND_UNSPECIFIED
        var model_id: String = ""
      ...
      override fun redact(`value`: StorageLifecycleEvent): StorageLifecycleEvent = value.copy(
        ...
        availability_result =
            value.availability_result?.let(StorageAvailabilityResult.ADAPTER::redact),
