...
public class NetworkEvent(
  ...
  ): NetworkEvent = NetworkEvent(kind, url, status_code, is_online, error, latency_ms,
      unknownFields)
  ...
  public companion object {
    ...
    public val ADAPTER: ProtoAdapter<NetworkEvent> = object : ProtoAdapter<NetworkEvent>(
      ...
      override fun encodedSize(`value`: NetworkEvent): Int {
        ...
        if (value.kind != NetworkEventKind.NETWORK_EVENT_KIND_UNSPECIFIED) size +=
            NetworkEventKind.ADAPTER.encodedSizeWithTag(1, value.kind)
        if (value.url != "") size += ProtoAdapter.STRING.encodedSizeWithTag(2, value.url)
        if (value.status_code != 0) size += ProtoAdapter.INT32.encodedSizeWithTag(3,
            value.status_code)
        if (value.is_online != false) size += ProtoAdapter.BOOL.encodedSizeWithTag(4,
            value.is_online)
        if (value.error != "") size += ProtoAdapter.STRING.encodedSizeWithTag(5, value.error)
        if (value.latency_ms != 0L) size += ProtoAdapter.INT64.encodedSizeWithTag(6,
            value.latency_ms)
        return size
      }

      override fun encode(writer: ProtoWriter, `value`: NetworkEvent) {
        if (value.kind != NetworkEventKind.NETWORK_EVENT_KIND_UNSPECIFIED)
        ...
        if (value.status_code != 0) ProtoAdapter.INT32.encodeWithTag(writer, 3, value.status_code)
        if (value.is_online != false) ProtoAdapter.BOOL.encodeWithTag(writer, 4, value.is_online)
        if (value.error != "") ProtoAdapter.STRING.encodeWithTag(writer, 5, value.error)
        if (value.latency_ms != 0L) ProtoAdapter.INT64.encodeWithTag(writer, 6, value.latency_ms)
