...
public class LoggingConfiguration(
  ...
  ): LoggingConfiguration = LoggingConfiguration(enable_local_logging, min_log_level,
      include_source_location, include_device_metadata, enable_remote_logging,
      enable_sentry_logging, unknownFields)

  public companion object {
    @JvmField
    public val ADAPTER: ProtoAdapter<LoggingConfiguration> = object :
        ProtoAdapter<LoggingConfiguration>(
      ...
      override fun encodedSize(`value`: LoggingConfiguration): Int {
        ...
        if (value.enable_local_logging != false) size += ProtoAdapter.BOOL.encodedSizeWithTag(1,
            value.enable_local_logging)
        if (value.min_log_level != LogLevel.LOG_LEVEL_TRACE) size +=
            LogLevel.ADAPTER.encodedSizeWithTag(2, value.min_log_level)
        if (value.include_source_location != false) size += ProtoAdapter.BOOL.encodedSizeWithTag(3,
            value.include_source_location)
        if (value.include_device_metadata != false) size += ProtoAdapter.BOOL.encodedSizeWithTag(4,
            value.include_device_metadata)
        if (value.enable_remote_logging != false) size += ProtoAdapter.BOOL.encodedSizeWithTag(5,
            value.enable_remote_logging)
        if (value.enable_sentry_logging != false) size += ProtoAdapter.BOOL.encodedSizeWithTag(6,
            value.enable_sentry_logging)
        return size
      }

      override fun encode(writer: ProtoWriter, `value`: LoggingConfiguration) {
        if (value.enable_local_logging != false) ProtoAdapter.BOOL.encodeWithTag(writer, 1,
            value.enable_local_logging)
        if (value.min_log_level != LogLevel.LOG_LEVEL_TRACE) LogLevel.ADAPTER.encodeWithTag(writer,
            2, value.min_log_level)
        if (value.include_source_location != false) ProtoAdapter.BOOL.encodeWithTag(writer, 3,
            value.include_source_location)
        if (value.include_device_metadata != false) ProtoAdapter.BOOL.encodeWithTag(writer, 4,
            value.include_device_metadata)
        if (value.enable_remote_logging != false) ProtoAdapter.BOOL.encodeWithTag(writer, 5,
            value.enable_remote_logging)
        if (value.enable_sentry_logging != false) ProtoAdapter.BOOL.encodeWithTag(writer, 6,
            value.enable_sentry_logging)
