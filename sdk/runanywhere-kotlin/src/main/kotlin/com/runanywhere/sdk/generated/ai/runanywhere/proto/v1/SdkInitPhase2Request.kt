...
public class SdkInitPhase2Request(
  ...
  ): SdkInitPhase2Request = SdkInitPhase2Request(build_token, force_refresh_assignments,
      flush_telemetry, discover_downloaded_models, rescan_local_models, unknownFields)

  public companion object {
    @JvmField
    public val ADAPTER: ProtoAdapter<SdkInitPhase2Request> = object :
        ProtoAdapter<SdkInitPhase2Request>(
      ...
      override fun encodedSize(`value`: SdkInitPhase2Request): Int {
        ...
        if (value.build_token != "") size += ProtoAdapter.STRING.encodedSizeWithTag(1,
            value.build_token)
        if (value.force_refresh_assignments != false) size +=
            ProtoAdapter.BOOL.encodedSizeWithTag(2, value.force_refresh_assignments)
        if (value.flush_telemetry != false) size += ProtoAdapter.BOOL.encodedSizeWithTag(3,
            value.flush_telemetry)
        if (value.discover_downloaded_models != false) size +=
            ProtoAdapter.BOOL.encodedSizeWithTag(4, value.discover_downloaded_models)
        if (value.rescan_local_models != false) size += ProtoAdapter.BOOL.encodedSizeWithTag(5,
            value.rescan_local_models)
        return size
      }

      override fun encode(writer: ProtoWriter, `value`: SdkInitPhase2Request) {
        if (value.build_token != "") ProtoAdapter.STRING.encodeWithTag(writer, 1, value.build_token)
        if (value.force_refresh_assignments != false) ProtoAdapter.BOOL.encodeWithTag(writer, 2,
            value.force_refresh_assignments)
        if (value.flush_telemetry != false) ProtoAdapter.BOOL.encodeWithTag(writer, 3,
            value.flush_telemetry)
        if (value.discover_downloaded_models != false) ProtoAdapter.BOOL.encodeWithTag(writer, 4,
            value.discover_downloaded_models)
        if (value.rescan_local_models != false) ProtoAdapter.BOOL.encodeWithTag(writer, 5,
            value.rescan_local_models)
