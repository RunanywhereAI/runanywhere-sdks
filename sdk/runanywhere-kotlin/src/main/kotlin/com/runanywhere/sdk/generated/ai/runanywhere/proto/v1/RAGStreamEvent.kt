...
public class RAGStreamEvent(
  ...
  ): RAGStreamEvent = RAGStreamEvent(seq, timestamp_us, request_id, kind, chunk, token, result,
    ...
    public val ADAPTER: ProtoAdapter<RAGStreamEvent> = object : ProtoAdapter<RAGStreamEvent>(
      ...
      override fun encodedSize(`value`: RAGStreamEvent): Int {
        ...
        if (value.request_id != "") size += ProtoAdapter.STRING.encodedSizeWithTag(3,
            value.request_id)
        if (value.kind != RAGStreamEventKind.RAG_STREAM_EVENT_KIND_UNSPECIFIED) size +=
            RAGStreamEventKind.ADAPTER.encodedSizeWithTag(4, value.kind)
        size += RAGSearchResult.ADAPTER.encodedSizeWithTag(5, value.chunk)
        if (value.token != "") size += ProtoAdapter.STRING.encodedSizeWithTag(6, value.token)
      ...
      override fun encode(writer: ProtoWriter, `value`: RAGStreamEvent) {
        ...
        if (value.kind != RAGStreamEventKind.RAG_STREAM_EVENT_KIND_UNSPECIFIED)
            RAGStreamEventKind.ADAPTER.encodeWithTag(writer, 4, value.kind)
        RAGSearchResult.ADAPTER.encodeWithTag(writer, 5, value.chunk)
        if (value.token != "") ProtoAdapter.STRING.encodeWithTag(writer, 6, value.token)
