 ...
 *
 * This module provides JNI bindings to the RunAnywhere Core C++ library with LlamaCPP backend.
 * It mirrors the architecture of runanywhere-swift's CRunAnywhereLlamaCPP module.
 *
 * Architecture:
 *   Kotlin Service Layer (LlamaCppCoreService) -> JNI Bindings (RunAnywhareBridge)
 *     -> Native Library (librunanywhere_jni.so) -> C API (runanywhere_bridge.h)
 *       -> C++ Backend (runanywhere_llamacpp)
 *
 * Build modes:
