...
# Use a separate clone for the standalone WASM module so the static-lib path
# under `third_party/sherpa-onnx` (used by the unified racommons build) keeps
# its own independent source tree + patches.
SHERPA_SRC="${WASM_DIR}/third_party/sherpa-onnx-standalone"
