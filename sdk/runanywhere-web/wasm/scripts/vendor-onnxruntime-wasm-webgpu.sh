...
DEST_DIR="${REPO_ROOT}/sdk/runanywhere-commons/third_party/onnxruntime-wasm-webgpu"
...
require_canonical_emscripten() {
  if ! command -v emcc >/dev/null 2>&1 || ! command -v emcmake >/dev/null 2>&1; then
    ...
    echo "Run: sdk/runanywhere-web/wasm/scripts/setup-emsdk.sh && source emsdk_env.sh" >&2
    exit 1
  fi
  local actual
