...
`tsconfig.json` paths into `sdk/runanywhere-web/packages/*/src` and
`sdk/shared/proto-ts/src`), so an SDK edit shows up here without a publish step.
...
`npm run build:wasm:all` from `sdk/runanywhere-web/` first; the build fails with
