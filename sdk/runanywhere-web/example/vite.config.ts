...
function wasmDir(installedPackage: string, workspacePackage: string): string {
  return useInstalledSDK
    ...
    : path.resolve(workspaceRoot, 'sdk/runanywhere-web/packages', workspacePackage, 'wasm');
}
...
const webCoreSrc = path.resolve(workspaceRoot, 'sdk/runanywhere-web/packages/core/src');
const localSDKSourceAliases = [
  {
    find: /^@runanywhere\/web-llamacpp$/,
    replacement: path.resolve(workspaceRoot, 'sdk/runanywhere-web/packages/llamacpp/src/index.ts'),
  },
    ...
    buildStart() {
      ...
      if (missing.length > 0) {
        this.error(
          ...
          }\nRun \`npm run build:wasm:all\` from sdk/runanywhere-web before building this example.`,
        );
      }
    },
    writeBundle(options) {
