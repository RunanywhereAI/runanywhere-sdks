...
const SKIP = missing.length ? { skip: missing.join('; ') } : {};
...
async function withVoiceModels(run) {
 ...
 */
async function withAgent(sdk, backend, run) {
  const { VoiceAgentAbi, toComposeConfig, missingComponents } = require('../../dist/api/voice-abi');
...
async function spokenPcm16(sdk, text) {
  const { downsample, pcm16Bytes } = require('../../dist/audio');
    ...
    await withVoiceModels(async (sdk) => {
      ...
      try {
        ...
        await assert.rejects(
          () => session.start(),
          (e) => /microphone capture/.test(e.message),
  ...
  async () => {
    ...
    await withSdk(async (sdk) => {
      await assert.rejects(
        ...
        (e) => /voice session is missing/.test(e.message) && /stt/.test(e.message)
      );
    });
  }
);
  ...
  async () => {
    await withVoiceModels(async (sdk, backend) => {
      await withAgent(sdk, backend, async (agent) => {
        ...
        const { decodeWav } = require('../../dist/audio');
...
test('feed: commons closes the utterance itself and returns the finished turn inline',
  ...
  async () => {
    await withVoiceModels(async (sdk, backend) => {
      await withAgent(sdk, backend, async (agent) => {
        const { toAudioFrame } = require('../../dist/api/voice-abi');
...
test('events: one turn walks the pipeline states and reports what was said at each stage',
  ...
  async () => {
    await withVoiceModels(async (sdk, backend) => {
      await withAgent(sdk, backend, async (agent) => {
        const { toTurnRequest } = require('../../dist/api/voice-abi');
        const { PipelineState } = require('../../dist/proto/voice_events');
...
test('stream: a fed turn surfaces through the session-scoped VoiceEvent callback',
  ...
  async () => {
    await withVoiceModels(async (sdk, backend) => {
      await withAgent(sdk, backend, async (agent) => {
        const { toAudioFrame, toPublicVoiceEvent } = require('../../dist/api/voice-abi');
