...
const SKIP = exists(NATIVE_PATH) ? {} : { skip: 'RUNANYWHERE_NATIVE_PATH unset or file missing' };
...
function startControlPlane(options = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (req.url === '/api/v1/auth/sdk/authenticate') {
      ...
      if (req.url === '/api/v1/auth/sdk/refresh') {
        issued += 1;
        return json(200, {
  ...
  return new Promise((resolve) => {
    server.listen(options.port ?? 0, '127.0.0.1', () => {
      resolve({
        ...
        requests,
        pathsHit: (prefix) => requests.filter((r) => r.url.startsWith(prefix)),
 ...
 */
function runInFreshProcess({ baseDir, baseUrl, apiKey, flushTelemetry }) {
  const script = `
...
test('no credentials: the SDK is fully usable and says so', { timeout: 180000, ...SKIP },
  async () => {
    ...
    if (exists(MODEL_FILE)) {
      ...
      const result = await sdk.llm.generate('Say hi.', { maxTokens: 32 });
...
test('handshake: authenticate, register the device, and POST telemetry with a bearer token',
  ...
  async () => {
    ...
    try {
      ...
      const payload = JSON.parse(register[0].body);
      ...
      const batch = JSON.parse(telemetry[0].body);
...
test('an offline start is reported, not swallowed, and recovers on retry',
  { timeout: 180000, ...SKIP },
  async () => {
    const { createRunAnywhere, NativeBackend } = require('../../dist');
    const { addon } = require('../../dist/bridge');
