...
import * as download from '../../dist/download';
import { registerCatalog, catalogEntries } from '../../dist/catalog';
import type { ModelKind } from '../../dist/download';

// The leakage guard below asks for names that are deliberately NOT exported, so
// the module is read through an index signature for those three assertions only.
const moduleSurface = download as unknown as Record<string, unknown>;
...
test('module exports exactly the public surface', () => {
  ...
  assert.equal(typeof moduleSurface.extractTarBz2, 'undefined');
  assert.equal(typeof moduleSurface.hfFiles, 'undefined');
  assert.equal(typeof moduleSurface.pickGguf, 'undefined');
  const own = Object.keys(moduleSurface).filter((k) => typeof moduleSurface[k] === 'function');
...
test('assertRemoteSupported rejects a remote STT/TTS/embedder source', () => {
  for (const kind of ['stt', 'tts', 'embedder'] satisfies ModelKind[]) {
...
/** A localhost server, its recorded Range headers, and the URL to fetch. */
interface Served {
  server: http.Server;
...
function serve(body: Buffer, opts: { ignoreRange?: boolean } = {}): Promise<Served> {
  ...
  return new Promise((resolve) =>
    server.listen(0, '127.0.0.1', () =>
      resolve({ server, seen, url: `http://127.0.0.1:${(server.address() as AddressInfo).port}/f.bin` })
    )
  );
}
...
test('downloadFile keeps the .part when the connection drops mid-stream', async () => {
  ...
  const url = `http://127.0.0.1:${(server.address() as AddressInfo).port}/f.bin`;
  const dir = freshTempRoot();
  const dest = path.join(dir, 'f.bin');
  try {
