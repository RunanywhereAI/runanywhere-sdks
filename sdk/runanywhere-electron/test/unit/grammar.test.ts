...
import { jsonSchemaToGrammar } from '../../dist/grammar';
import type { JsonSchema } from '../../dist/grammar';
...
function rhsOf(grammar: string, ruleName: string): string {
  const line = grammar
    .split('\n')
...
function definedRuleNames(grammar: string): Set<string> {
 ...
 */
function assertClosed(grammar: string): void {
