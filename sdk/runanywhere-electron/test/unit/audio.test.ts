...
import type { AudioDspBackend, AudioNative } from '../../dist/audio';
import { ErrorCode, isSDKException } from '../../dist/errors';

function installForwardingNative(): Array<[string, ...unknown[]]> {
  const calls: Array<[string, ...unknown[]]> = [];
  const native: AudioNative = {
...
test('without bind or inject, DSP fails explicitly (no bridge resolve)', async () => {
  await assert.rejects(
    ...
    (e: unknown) => {
      ...
      assert.equal(e.code, ErrorCode.ERROR_CODE_SERVICE_NOT_AVAILABLE);
...
test('bound AudioDspBackend is preferred over missing native', async () => {
  const calls: Array<[string, ...unknown[]]> = [];
  const dsp: AudioDspBackend = {
    async audioResampleF32(samples, inRate, outRate) {
      calls.push(['rpc-resample', samples.length, inRate, outRate]);
      return new Float32Array(2);
    },
    async audioFloat32ToPcm16() {
      throw new Error('unused');
    },
    async audioPcm16ToFloat32() {
      throw new Error('unused');
    },
    async audioComputeRms() {
      throw new Error('unused');
    },
    async audioFloat32ToWav() {
      throw new Error('unused');
    },
    async audioWavToFloat32() {
      throw new Error('unused');
    },
    async audioPcmBytesToMs() {
      throw new Error('unused');
    },
  };
  bindAudioBackend(dsp);
