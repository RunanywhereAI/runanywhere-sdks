...
const { registerCatalog, clearCatalog, catalogEntries, catalogEntry, isCatalogId } = require('../../dist/catalog');
...
// examples/electron/RunAnywhereAI/model-catalog.js.
...
test('registering __proto__ as a key does not pollute Object.prototype', () => {
  registerCatalog({ __proto__: FIXTURE['fixture-llm'] });
  assert.equal(isCatalogId('__proto__'), false);
...
test('vlm fixture defines an mmproj that matches one of its files', () => {
  ...
  assert.ok(targets.includes(entry.mmproj));
