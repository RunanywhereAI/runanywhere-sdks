...
import { streamWithMetrics } from '../../dist/stream';
import type { LLMStreamEvent } from '../../dist/stream';
...
function clock(vals: number[]): () => number {
  let i = 0;
  return () => vals[i++];
}
...
test('a single-token stream reports one token and a valid rate', async () => {
  ...
  assert.ok(r, 'the final event carries a result');
