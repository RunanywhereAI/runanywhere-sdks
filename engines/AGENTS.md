...
| **mlx** | LLM (`llm_ops`) + STT (`stt_ops`) + TTS (`tts_ops`) + EMBED (`embedding_ops`) + VLM (`vlm_ops`) + DIARIZE (`diarization_ops`) | **our** Swift `mlx-swift-lm` callback bridge (Apple MLX) | **1** — bundles its own runtime (mlx-swift) | **ON (Apple)** | priority 110. PUBLIC availability but Apple-gated: `mlx_capability_check` silent-rejects non-Apple, and `RAC_BACKEND_MLX` is forced OFF off-Apple. Declares `RAC_RUNTIME_CPU` + `RAC_RUNTIME_METAL` (hints); formats SAFETENSORS + FOLDER. `SHARED_ONLY`. Multi-modality (6 filled slots). |
