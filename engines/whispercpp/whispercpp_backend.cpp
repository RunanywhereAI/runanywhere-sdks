...
bool WhisperCppBackend::initialize(const nlohmann::json &config) {
  ...
  if (num_threads_ <= 0) {
    ...
    num_threads_ = std::max(
        1, std::min(8, static_cast<int>(sysconf(_SC_NPROCESSORS_ONLN)) - 2));
#else
    num_threads_ = 4;
#endif
  }
  ...
  RAC_LOG_INFO("STT.WhisperCpp",
               "WhisperCppBackend initialized with %d threads, GPU: %s",
...
size_t WhisperCppBackend::get_memory_usage() const { return 0; }
...
bool WhisperCppSTT::load_model(const std::string &model_path,
  ...
  RAC_LOG_INFO("STT.WhisperCpp", "Loading whisper model from: %s",
               model_path.c_str());

  whisper_context_params cparams = whisper_context_default_params();
  cparams.use_gpu = backend_->is_gpu_enabled();

  if (config.contains("word_timestamps") &&
      config["word_timestamps"].get<bool>()) {
  ...
  RAC_LOG_INFO("STT.WhisperCpp",
               "Whisper model loaded successfully. Multilingual: %s",
...
STTResult WhisperCppSTT::transcribe_internal(const std::vector<float> &audio,
  ...
  int ret =
      whisper_full(ctx_, wparams, audio.data(), static_cast<int>(audio.size()));
      ...
      if (word_timestamps) {
        ...
        for (int j = 0; j < n_tokens; ++j) {
          whisper_token_data token_data =
              whisper_full_get_token_data(ctx_, i, j);
  ...
  result.audio_duration_ms =
      (audio.size() / static_cast<double>(WHISPER_SAMPLE_RATE)) * 1000.0;
  ...
  RAC_LOG_INFO("STT.WhisperCpp",
               "Transcription complete: %d segments, %.0fms inference, lang=%s",
               n_segments, result.inference_time_ms,
               result.detected_language.empty()
                   ? "unknown"
                   : result.detected_language.c_str());
...
std::vector<float>
WhisperCppSTT::resample_to_16khz(const std::vector<float> &samples,
                                 int source_rate) {
  ...
  RAC_LOG_INFO("STT.WhisperCpp",
               "Resampled audio from %d Hz to %d Hz (%zu -> %zu samples)",
