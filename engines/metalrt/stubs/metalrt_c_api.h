 ...
 *   - The backend's vtable entries short-circuit to
 * RAC_ERROR_BACKEND_UNAVAILABLE before calling into the engine, so stubs are a
 * safety net, not the primary error surface.
 */
...
struct MetalRTResult
metalrt_generate_stream(void *handle, const char *prompt, metalrt_stream_cb cb,
                        void *user_data, const struct MetalRTOptions *options);

struct MetalRTResult
metalrt_generate_raw_continue(void *handle, const char *query,
                              const struct MetalRTOptions *options);
...
struct MetalRTVisionResult
metalrt_vision_analyze(void *handle, const char *image_path, const char *prompt,
                       const struct MetalRTVisionOptions *options);

struct MetalRTVisionResult
metalrt_vision_analyze_pixels(void *handle, const uint8_t *rgba_pixels,
                              int width, int height, const char *prompt,
                              const struct MetalRTVisionOptions *options);

struct MetalRTVisionResult
metalrt_vision_analyze_stream(void *handle, const char *image_path,
                              const char *prompt, metalrt_stream_cb cb,
                              void *user_data,
                              const struct MetalRTVisionOptions *options);

struct MetalRTVisionResult metalrt_vision_analyze_pixels_stream(
    void *handle, const uint8_t *rgba_pixels, int width, int height,
    const char *prompt, metalrt_stream_cb cb, void *user_data,
    const struct MetalRTVisionOptions *options);
