...
rac_result_t rac_llm_metalrt_create(const char *model_path,
...
rac_result_t rac_llm_metalrt_generate(rac_handle_t handle, const char *prompt,
  ...
  out_result->total_time_ms =
      static_cast<int64_t>(result.prefill_ms + result.decode_ms);
...
static bool metalrt_stream_bridge(const char *piece, void *ctx) {
  ...
  if (!adapter || !adapter->callback)
    return false;
  if (adapter->max_tokens > 0 &&
      adapter->emitted_tokens >= adapter->max_tokens) {
    return false;
  }
...
rac_result_t rac_llm_metalrt_generate_stream(rac_handle_t handle,
                                             const char *prompt,
                                             const rac_llm_options_t *options,
                                             rac_llm_metalrt_stream_cb callback,
                                             void *user_data) {
...
rac_result_t rac_llm_metalrt_inject_system_prompt(rac_handle_t handle,
                                                  const char *prompt) {
  if (!handle || !prompt)
    return RAC_ERROR_NULL_POINTER;
  auto *impl = static_cast<rac_llm_metalrt_impl *>(handle);
...
rac_result_t rac_llm_metalrt_append_context(rac_handle_t handle,
                                            const char *text) {
  if (!handle || !text)
    return RAC_ERROR_NULL_POINTER;
  auto *impl = static_cast<rac_llm_metalrt_impl *>(handle);
...
rac_result_t
rac_llm_metalrt_generate_from_context(rac_handle_t handle, const char *query,
                                      const rac_llm_options_t *options,
                                      rac_llm_result_t *out_result) {
  ...
  struct MetalRTResult result =
      metalrt_generate_raw_continue(impl->handle, query, &opts);
