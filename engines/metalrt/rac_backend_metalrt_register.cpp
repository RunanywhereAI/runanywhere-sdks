 ...
 * Provides vtable implementations for LLM, STT, TTS, and VLM service
 * interfaces.
 *
...
static std::string resolve_metalrt_model_path(const char *base_path) {
  ...
  while ((entry = readdir(dir)) != nullptr) {
    ...
    std::string nested_config =
        std::string(base_path) + "/" + entry->d_name + "/config.json";
...
// Stream adapter: bridge RAC's callback (token, user_data) to MetalRT's (token,
// is_final, user_data)
...
static rac_bool_t llm_stream_adapter_cb(const char *token, rac_bool_t is_final,
                                        void *ctx) {
...
static rac_result_t llm_vtable_generate_stream(
  ...
  return rac_llm_metalrt_generate_stream(impl, prompt, options,
                                         llm_stream_adapter_cb, &adapter);
}
...
static void llm_vtable_destroy(void *impl) { rac_llm_metalrt_destroy(impl); }

static rac_result_t llm_vtable_inject_system_prompt(void *impl,
                                                    const char *prompt) {
...
static rac_result_t
llm_vtable_generate_from_context(void *impl, const char *query,
                                 const rac_llm_options_t *options,
                                 rac_llm_result_t *out_result) {
  return rac_llm_metalrt_generate_from_context(impl, query, options,
                                               out_result);
}
...
static rac_result_t stt_vtable_transcribe(void *impl, const void *audio_data,
                                          size_t audio_size,
                                          const rac_stt_options_t *options,
                                          rac_stt_result_t *out_result) {
  return rac_stt_metalrt_transcribe(impl, audio_data, audio_size, options,
                                    out_result);
}

static rac_result_t stt_vtable_get_info(void * /*impl*/,
...
static rac_result_t stt_vtable_cleanup(void * /*impl*/) { return RAC_SUCCESS; }

static void stt_vtable_destroy(void *impl) { rac_stt_metalrt_destroy(impl); }
...
static rac_result_t tts_vtable_get_info(void * /*impl*/,
...
static rac_result_t tts_vtable_cleanup(void * /*impl*/) { return RAC_SUCCESS; }

static void tts_vtable_destroy(void *impl) { rac_tts_metalrt_destroy(impl); }
...
static rac_result_t
vlm_vtable_process_stream(void *impl, const rac_vlm_image_t *image,
                          const char *prompt, const rac_vlm_options_t *options,
                          rac_vlm_stream_callback_fn callback,
                          void *user_data) {
  return rac_vlm_metalrt_process_stream(impl, image, prompt, options, callback,
                                        user_data);
}

static rac_result_t vlm_vtable_get_info(void * /*impl*/,
                                        rac_vlm_info_t *out_info) {
  if (!out_info)
    return RAC_ERROR_NULL_POINTER;
  out_info->is_ready = RAC_TRUE;
  out_info->supports_streaming = RAC_TRUE;
...
static void vlm_vtable_destroy(void *impl) { rac_vlm_metalrt_destroy(impl); }
