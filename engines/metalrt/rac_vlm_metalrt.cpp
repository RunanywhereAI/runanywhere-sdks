 ...
 * @brief MetalRT VLM backend — wraps metalrt_vision_* for vision-language
 * inference
 */
...
static std::vector<uint8_t> rgb_to_rgba(const uint8_t *rgb, uint32_t w,
                                        uint32_t h) {
...
rac_result_t rac_vlm_metalrt_create(const char *model_path,
...
rac_result_t rac_vlm_metalrt_process(rac_handle_t handle,
  ...
  if (image->format == RAC_VLM_IMAGE_FORMAT_FILE_PATH && image->file_path) {
    result =
        metalrt_vision_analyze(impl->handle, image->file_path, prompt, &vopts);
    ...
    if (image->width == 0 || image->height == 0 ||
      ...
      RAC_LOG_ERROR(LOG_CAT, "Invalid RGB dimensions: %u x %u", image->width,
                    image->height);
      return RAC_ERROR_VALIDATION_FAILED;
    }
  ...
  out_result->image_encode_time_ms =
      static_cast<int64_t>(result.vision_encode_ms);
...
rac_result_t rac_vlm_metalrt_process_stream(rac_handle_t handle,
  ...
  if (image->format == RAC_VLM_IMAGE_FORMAT_FILE_PATH && image->file_path) {
    result =
        metalrt_vision_analyze_stream(impl->handle, image->file_path, prompt,
    ...
    RAC_LOG_ERROR(LOG_CAT, "Unsupported image format for streaming: %d",
                  image->format);
    return RAC_ERROR_VALIDATION_FAILED;
  }
