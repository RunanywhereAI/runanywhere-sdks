...
rac_result_t rac_vad_sherpa_create(const char *model_path,
  ...
  if (model_path != nullptr) {
    ...
    if (!handle->vad->load_model(model_path, runanywhere::VADModelType::SILERO,
                                 model_config)) {
      delete handle;
...
rac_result_t rac_vad_sherpa_process(rac_handle_t handle, const float *samples,
                                    size_t num_samples,
...
rac_result_t rac_vad_sherpa_set_threshold(rac_handle_t handle,
                                          float threshold) {
  if (handle == nullptr) {
    return RAC_ERROR_NULL_POINTER;
  }
...
void rac_vad_sherpa_destroy(rac_handle_t handle) {
  ...
  rac_event_track("vad.backend.destroyed", RAC_EVENT_CATEGORY_VOICE,
