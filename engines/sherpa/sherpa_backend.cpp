...
STTResult SherpaSTT::transcribe(const STTRequest& request) {
    ...
    RAC_LOG_INFO("Sherpa.STT", "Transcribing %zu samples at %d Hz", request.audio_samples.size(),
                 request.sample_rate);
    ...
    while ((family_entry = readdir(lang_root)) != nullptr) {
        ...
        if (S_ISREG(st.st_mode)) {
            ...
            if (src_f && dst_f) {
                ...
                copied++;
                RAC_LOG_DEBUG("Sherpa.TTS", "[ensure_voices] Copied: %s -> %s", family_path.c_str(),
                              dest.c_str());
            } else {
                errors++;
                RAC_LOG_ERROR("Sherpa.TTS",
        ...
        RAC_LOG_DEBUG("Sherpa.TTS", "[ensure_voices] Scanning family dir: %s", family_entry->d_name);
        ...
        while ((voice_entry = readdir(family_dir)) != nullptr) {
            ...
            if (src_f && dst_f) {
                ...
                errors++;
                RAC_LOG_ERROR(
                    "Sherpa.TTS", "[ensure_voices] FAILED: %s -> voices/%s (src=%p dst=%p errno=%d)",
...
bool SherpaTTS::load_model(const std::string& model_path, TTSModelType model_type,
        ...
        if (diag_dir) {
            ...
            while ((diag_entry = readdir(diag_dir)) != nullptr) {
                if (diag_entry->d_name[0] == '.')
                    continue;
                RAC_LOG_INFO("Sherpa.TTS", "  [%s] %s", diag_entry->d_type == DT_DIR ? "DIR" : "FILE",
                             diag_entry->d_name);
            }
            closedir(diag_dir);
            ...
            if (dir) {
                ...
                while ((entry = readdir(dir)) != nullptr) {
                    ...
                    if (filename.size() > 5 && filename.substr(filename.size() - 5) == ".onnx") {
                        ...
                        RAC_LOG_DEBUG("Sherpa.TTS", "Found model file: %s", model_onnx_path.c_str());
                        break;
                    }
                }
                closedir(dir);
    ...
    if (!espeak_data_dir.empty()) {
        ...
        RAC_LOG_WARNING("Sherpa.TTS", "espeak-ng-data NOT FOUND in model dir — Piper TTS will fail");
    }
...
TTSResult SherpaTTS::synthesize(const TTSRequest& request) {
    ...
    RAC_LOG_INFO("Sherpa.TTS", "Synthesis complete. Duration: %.2fs", (result.duration_ms / 1000.0));
