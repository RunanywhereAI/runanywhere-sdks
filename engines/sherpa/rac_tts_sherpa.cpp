...
rac_result_t rac_tts_sherpa_create(const char *model_path,
  ...
  if (model_path != nullptr) {
    if (!handle->tts->load_model(model_path,
...
rac_result_t rac_tts_sherpa_synthesize(rac_handle_t handle, const char *text,
  ...
  float *audio_copy =
      static_cast<float *>(malloc(result.audio_samples.size() * sizeof(float)));
  if (!audio_copy) {
    return RAC_ERROR_OUT_OF_MEMORY;
  }
  memcpy(audio_copy, result.audio_samples.data(),
  ...
  rac_event_track("tts.synthesis.completed", RAC_EVENT_CATEGORY_TTS,
...
rac_result_t rac_tts_sherpa_get_languages(rac_handle_t handle,
                                          char **out_json) {
...
void rac_tts_sherpa_destroy(rac_handle_t handle) {
  ...
  rac_event_track("tts.backend.destroyed", RAC_EVENT_CATEGORY_TTS,
