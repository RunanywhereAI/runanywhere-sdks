/**
 * @file wakeword_sherpa.cpp
 * @brief Sherpa speech backend wake word compatibility implementation using openWakeWord
 *
...
static const char* LOG_TAG = "WakeWordSherpa";
...
static WakewordSherpaBackend* get_backend(rac_handle_t handle) {
    return static_cast<WakewordSherpaBackend*>(handle);
}
 ...
 */
static void initialize_streaming_buffers(WakewordSherpaBackend* backend) {
 ...
 */
static bool compute_melspectrogram(WakewordSherpaBackend* backend, const std::vector<float>& audio,
 ...
 */
static bool compute_single_embedding(WakewordSherpaBackend* backend, const float* melspec_window,
 ...
 */
static void generate_embeddings_from_melspec(WakewordSherpaBackend* backend) {
    if (!backend->embedding_session) {
        return;
    }
 ...
 */
static float run_classifier(WakewordSherpaBackend* backend, WakewordModel& model) {
...
static bool run_vad(WakewordSherpaBackend* backend, const float* samples, size_t num_samples,
                    bool* out_is_speech) {
    ...
    rac_result_t result = rac_vad_sherpa_process(
...
RAC_ONNX_API rac_result_t rac_wakeword_onnx_create(const rac_wakeword_onnx_config_t* config,
    ...
    auto* backend = new (std::nothrow) WakewordSherpaBackend();
    if (!backend) {
...
RAC_ONNX_API rac_result_t rac_wakeword_onnx_load_vad(rac_handle_t handle,
    ...
    if (backend->vad_handle) {
        rac_vad_sherpa_destroy(backend->vad_handle);
    ...
    rac_result_t result = rac_vad_sherpa_create(vad_model_path, &vad_config, &backend->vad_handle);
...
RAC_ONNX_API rac_result_t rac_wakeword_onnx_reset(rac_handle_t handle) {
    ...
    // Reset VAD
    if (backend->vad_handle && backend->vad_loaded) {
        rac_vad_sherpa_reset(backend->vad_handle);
    }
