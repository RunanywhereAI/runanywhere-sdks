...
static rac_result_t onnx_embed_vtable_initialize(void *impl,
...
static rac_result_t
onnx_embed_vtable_embed(void *impl, const char *text,
  ...
  try {
    ...
    out_result->embeddings[0].data =
        static_cast<float *>(malloc(dim * sizeof(float)));
    ...
    memcpy(out_result->embeddings[0].data, embedding.data(),
           dim * sizeof(float));
    return RAC_SUCCESS;
...
static rac_result_t
onnx_embed_vtable_embed_batch(void *impl, const char *const *texts,
                              size_t num_texts,
                              const rac_embeddings_options_t *options,
                              rac_embeddings_result_t *out_result) {
  (void)options;
...
static rac_result_t
onnx_embed_vtable_get_info(void *impl, rac_embeddings_info_t *out_info) {
  if (!impl || !out_info)
    return RAC_ERROR_NULL_POINTER;

  auto *h = static_cast<onnx_embeddings_handle *>(impl);
  out_info->is_ready =
      (h->provider && h->provider->is_ready()) ? RAC_TRUE : RAC_FALSE;
...
static rac_result_t onnx_embed_create_impl(const char *model_id,
  ...
  *out_impl = nullptr;
  ...
  try {
    ...
    const char *cfg =
        (config_json && config_json[0] != '\0') ? config_json : "";
    handle->provider =
        std::make_unique<runanywhere::rag::ONNXEmbeddingProvider>(model_id,
                                                                  cfg);
...
rac_result_t rac_backend_onnx_embeddings_register(void) {
  ...
  module_info.description =
      "Sentence-transformer embedding provider via ONNX Runtime";
