...
public:
  ...
  std::vector<int64_t> encode_unpadded(const std::string &text,
  ...
  std::vector<int64_t>
  create_attention_mask(const std::vector<int64_t> &token_ids) {
...
private:
  ...
  basic_tokenize_scalar_ascii(const std::string &text) const {
  ...
  std::vector<std::string>
  basic_tokenize_scalar_mixed(const std::string &text) const {
    std::vector<std::string> tokens;
    std::string current;
    current.reserve(text.size());
...
#if defined(__aarch64__) && defined(__ARM_NEON)
  std::vector<std::string>
  basic_tokenize_simd_ascii(const std::string &text) const {
    std::vector<std::string> tokens;
    std::string current;
    current.reserve(text.size());
  ...
  void insert_cache_entry(const std::string &word,
...
std::vector<float> mean_pooling(const float *embeddings,
...
public:
  explicit Impl(const std::string &model_path, const std::string &config_json)
    ...
    if (vocab_path.empty() || !std::filesystem::exists(vocab_path)) {
      ...
      if (std::filesystem::exists(search_dir) &&
          std::filesystem::is_directory(search_dir)) {
    ...
    if (!tokenizer_.load_vocab(vocab_path)) {
      RAC_LOG_ERROR(LOG_TAG, "Failed to load tokenizer vocab: %s",
                    vocab_path.c_str());
      return;
    }
    ...
    ready_ = true;
    RAC_LOG_INFO(LOG_TAG, "ONNX embedding provider initialized: %s",
                 model_path.c_str());
  ...
  std::vector<float> embed(const std::string &text) {
    ...
    try {
      ...
      std::memcpy(input_ids_buf_.data(), token_ids.data(),
      ...
      if (!run_embedding_model(input_ids_buf_, attention_mask_buf_,
                               token_type_ids_buf_, input_shape_, output)) {
        return {};
      }
      ...
      RAC_LOG_INFO(LOG_TAG, "Generated embedding: dim=%zu, norm=1.0",
                   pooled.size());
      return pooled;
  ...
  embed_batch(const std::vector<std::string> &texts) {
    ...
    LOGI("Generated batch embeddings: count=%zu, dim=%zu", all_results.size(),
         embedding_dim_);
    return all_results;
  }
...
private:
  ...
  std::vector<std::vector<float>>
  embed_sub_batch(const std::vector<std::string> &texts, size_t offset,
                  size_t count) {
    try {
      ...
      for (size_t i = 0; i < count; ++i) {
        all_token_ids[i] =
            tokenizer_.encode_unpadded(texts[offset + i], max_seq_length_);
      ...
      LOGI(
          "Sub-batch dynamic padding: max_actual=%zu, pad_length=%zu (was %zu)",
      ...
      runanywhere::runtime::onnxrt::TensorOutput output;
      if (!run_embedding_model(flat_input_ids, flat_attention_mask,
                               flat_token_type_ids, batch_shape, output)) {
        return {};
      }
      ...
      for (size_t i = 0; i < count; ++i) {
        ...
        auto pooled = mean_pooling(sentence_data, attention_masks[i],
  ...
  bool load_model(const std::string &model_path) {
    ...
    std::string error;
    onnx_session_ = runanywhere::runtime::onnxrt::Session::create(
        model_path, options, &error);
    if (!onnx_session_) {
  ...
  bool run_embedding_model(const std::vector<int64_t> &input_ids,
    ...
    const TensorInput inputs[] = {
        ...
        {"attention_mask", attention_mask.data(),
         attention_mask.size() * sizeof(int64_t), shape.data(), shape.size(),
         ElementType::Int64},
        {"token_type_ids", token_type_ids.data(),
         token_type_ids.size() * sizeof(int64_t), shape.data(), shape.size(),
         ElementType::Int64},
    };
    ...
    std::string error;
    rac_result_t rc =
        onnx_session_->run(inputs, 3, output_names, 1, outputs, &error);
...
ONNXEmbeddingProvider::ONNXEmbeddingProvider(
    ONNXEmbeddingProvider &&) noexcept = default;
ONNXEmbeddingProvider &
ONNXEmbeddingProvider::operator=(ONNXEmbeddingProvider &&) noexcept = default;
