...
Java_com_runanywhere_sdk_llm_llamacpp_LlamaCPPBridge_nativeCreate(
    ...
    jint numThreads, jint gpuLayers) {
  (void)clazz;
  ...
  LOGi("nativeCreate: model=%s, ctx=%d, threads=%d, gpu=%d", path, contextSize,
       numThreads, gpuLayers);
...
Java_com_runanywhere_sdk_llm_llamacpp_LlamaCPPBridge_nativeGenerate(
  ...
  rac_result_t status = rac_llm_llamacpp_generate(
      reinterpret_cast<rac_handle_t>(handle), promptStr, &options, &result);
