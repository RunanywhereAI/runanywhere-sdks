...
#define LLAMA_ALOG(...)                                                        \
  __android_log_print(ANDROID_LOG_INFO, "LLM.LlamaCpp.JNI", __VA_ARGS__)
#else
...
rac_result_t rac_llm_llamacpp_create(const char *model_path,
  ...
  // Initialize backend
  LLAMA_ALOG("rac_llm_llamacpp_create: initializing backend for %s",
             model_path);
...
rac_result_t rac_llm_llamacpp_generate(rac_handle_t handle, const char *prompt,
  ...
  RAC_LOG_DEBUG("LLM.LlamaCpp", "rac_llm_llamacpp_generate: START handle=%p",
                handle);
  ...
  if (!h->text_gen) {
    RAC_LOG_ERROR("LLM.LlamaCpp",
                  "rac_llm_llamacpp_generate: text_gen is null!");
    return RAC_ERROR_INVALID_HANDLE;
  }

  // Build request from RAC options
  RAC_LOG_DEBUG("LLM.LlamaCpp",
                "rac_llm_llamacpp_generate: building request, prompt_len=%zu",
                strlen(prompt));
  ...
  RAC_LOG_DEBUG("LLM.LlamaCpp",
                "rac_llm_llamacpp_generate: generate() returned, tokens=%d",
                result.tokens_generated);

  // finish_reason is std::string; TODO: migrate to enum if TextGenerationResult
  // gains one
  if (result.finish_reason == "error") {
    RAC_LOG_ERROR("LLM.LlamaCpp", "rac_llm_llamacpp_generate: generation "
  ...
  // Publish event
  rac_event_track("llm.generation.completed", RAC_EVENT_CATEGORY_LLM,
...
rac_result_t rac_llm_llamacpp_generate_stream(
  ...
  try {
    ...
    rac_error_set_details(
        "Unknown C++ exception during streaming LLM generation");
    return RAC_ERROR_INFERENCE_FAILED;
  }
...
rac_result_t rac_llm_llamacpp_generate_stream_with_timing(
  ...
  try {
    ...
    rac_error_set_details(
        "Unknown C++ exception during timed streaming LLM generation");
    return RAC_ERROR_INFERENCE_FAILED;
  }
...
void rac_llm_llamacpp_cancel(rac_handle_t handle) {
  ...
  rac_event_track("llm.generation.cancelled", RAC_EVENT_CATEGORY_LLM,
                  RAC_EVENT_DESTINATION_ALL, nullptr);
}

rac_result_t rac_llm_llamacpp_get_model_info(rac_handle_t handle,
                                             char **out_json) {
...
rac_result_t rac_llm_llamacpp_load_lora(rac_handle_t handle,
  ...
  if (!h->text_gen->load_lora_adapter(adapter_path, scale)) {
    std::string detail =
        std::string("Failed to load LoRA adapter: ") + adapter_path;
...
rac_result_t rac_llm_llamacpp_get_lora_info(rac_handle_t handle,
                                            char **out_json) {
  if (handle == nullptr || out_json == nullptr) {
    return RAC_ERROR_NULL_POINTER;
  }
...
rac_result_t rac_llm_llamacpp_inject_system_prompt(rac_handle_t handle,
                                                   const char *prompt) {
  ...
  try {
    return h->text_gen->inject_system_prompt(prompt)
               ? RAC_SUCCESS
...
rac_result_t rac_llm_llamacpp_append_context(rac_handle_t handle,
                                             const char *text) {
  ...
  try {
    return h->text_gen->append_context(text) ? RAC_SUCCESS
                                             : RAC_ERROR_INFERENCE_FAILED;
  } catch (const std::exception &e) {
    rac_error_set_details(e.what());
    return RAC_ERROR_INFERENCE_FAILED;
...
rac_result_t
rac_llm_llamacpp_generate_from_context(rac_handle_t handle, const char *query,
                                       const rac_llm_options_t *options,
                                       rac_llm_result_t *out_result) {
...
void rac_llm_llamacpp_destroy(rac_handle_t handle) {
  ...
  rac_event_track("llm.backend.destroyed", RAC_EVENT_CATEGORY_LLM,
                  RAC_EVENT_DESTINATION_ALL, R"({"backend":"llamacpp"})");
}
