...
static rac_result_t llamacpp_vlm_vtable_initialize(void *impl,
...
static rac_result_t llamacpp_vlm_vtable_process(
    void *impl, const rac_vlm_image_t *image, const char *prompt,
...
static rac_bool_t vlm_stream_adapter_callback(const char *token,
...
// Process stream
static rac_result_t llamacpp_vlm_vtable_process_stream(
    void *impl, const rac_vlm_image_t *image, const char *prompt,
...
// Get info
static rac_result_t llamacpp_vlm_vtable_get_info(void *impl,
  ...
  out_info->supports_multiple_images =
      RAC_FALSE; // Current implementation: single image
  ...
  if (config_json && config_json[0] != '\0') {
    try {
      ...
      if (json.contains("mmproj_path") && json["mmproj_path"].is_string()) {
        ...
        RAC_LOG_DEBUG(LOG_CAT, "Parsed mmproj_path from config_json: %s",
                      mmproj_path);
      }
    } catch (const std::exception &e) {
      RAC_LOG_WARNING(LOG_CAT, "config_json parse failed (%s); using defaults",
                      e.what());
    }
  }

  RAC_LOG_INFO(LOG_CAT, "llamacpp_vlm_create_impl: model=%s, mmproj=%s",
