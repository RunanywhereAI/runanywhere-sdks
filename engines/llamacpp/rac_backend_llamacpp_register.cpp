...
rac_result_t
llamacpp_cpu_provider_create_session(const rac_runtime_session_desc_t *desc,
  ...
  *out = nullptr;
  ...
  rac_result_t rc =
      rac_llm_llamacpp_create(desc->model_path, nullptr, &backend_handle);
  if (rc != RAC_SUCCESS)
    return rc;
...
const rac_runtime_io_t *find_io(const rac_runtime_io_t *ios, size_t count,
                                const char *name) {
...
rac_result_t llamacpp_cpu_provider_run_session(rac_runtime_session_t *session,
  ...
  auto *result_io =
      const_cast<rac_runtime_io_t *>(find_io(outputs, n_out, "llm_result"));
  ...
  return rac_llm_llamacpp_generate(
      reinterpret_cast<rac_handle_t>(session), prompt, options,
...
const rac_cpu_runtime_provider_t k_llamacpp_cpu_provider = {
    ...
    /* .formats_count   = */ sizeof(k_llamacpp_cpu_formats) /
        sizeof(k_llamacpp_cpu_formats[0]),
...
// no-op)
static rac_result_t llamacpp_vtable_initialize(void *impl,
...
static rac_result_t llamacpp_vtable_generate(void *impl, const char *prompt,
  ...
  return runtime_impl->runtime->run_session(runtime_impl->runtime_session,
                                            inputs, n_in, &output, 1);
}
...
static rac_bool_t stream_adapter_callback(const char *token,
                                          rac_bool_t is_final, void *ctx) {
...
// Generate stream
static rac_result_t llamacpp_vtable_generate_stream(
...
static rac_result_t llamacpp_vtable_generate_stream_with_timing(
  ...
  return rac_llm_llamacpp_generate_stream_with_timing(
      legacy_handle(impl), prompt, options, stream_adapter_callback, &adapter,
      timing_out);
}

// Get info
static rac_result_t llamacpp_vtable_get_info(void *impl,
                                             rac_llm_info_t *out_info) {
  if (!out_info)
    return RAC_ERROR_NULL_POINTER;
  ...
  out_info->context_length =
      0; // Default if model not loaded or info unavailable

  // Get actual context_length from model info JSON when model is loaded
  if (out_info->is_ready) {
    char *json_str = nullptr;
    if (rac_llm_llamacpp_get_model_info(legacy_handle(impl), &json_str) ==
            RAC_SUCCESS &&
        json_str) {
      try {
...
static rac_result_t
llamacpp_vtable_load_lora(void *impl, const char *adapter_path, float scale) {
  return rac_llm_llamacpp_load_lora(legacy_handle(impl), adapter_path, scale);
}

static rac_result_t llamacpp_vtable_remove_lora(void *impl,
...
static rac_result_t llamacpp_vtable_inject_system_prompt(void *impl,
                                                         const char *prompt) {
  return rac_llm_llamacpp_inject_system_prompt(legacy_handle(impl), prompt);
}

static rac_result_t llamacpp_vtable_append_context(void *impl,
                                                   const char *text) {
  return rac_llm_llamacpp_append_context(legacy_handle(impl), text);
}

static rac_result_t
llamacpp_vtable_generate_from_context(void *impl, const char *query,
                                      const rac_llm_options_t *options,
                                      rac_llm_result_t *out_result) {
  return rac_llm_llamacpp_generate_from_context(legacy_handle(impl), query,
                                                options, out_result);
}
...
rac_result_t llamacpp_llm_create_impl(const char *model_id,
  ...
  *out_impl = nullptr;
  ...
  if (rc != RAC_SUCCESS || runtime == nullptr ||
      runtime->create_session == nullptr || runtime->run_session == nullptr ||
      runtime->destroy_session == nullptr) {
  ...
  rc = rac_cpu_runtime_get_provider_session(runtime_session, nullptr,
                                            &provider_session);
  ...
  runtime_impl->legacy_handle =
      reinterpret_cast<rac_handle_t>(provider_session);
