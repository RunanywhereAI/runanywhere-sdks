...
VLMModelType detect_vlm_model_type(llama_model *model) {
  ...
  int32_t len = llama_model_meta_val_str(model, "general.name", name_buf,
                                         sizeof(name_buf));
  if (len <= 0) {
    len = llama_model_meta_val_str(model, "general.basename", name_buf,
                                   sizeof(name_buf));
  }
...
std::string format_vlm_prompt_with_template(
  ...
  if (tmpl) {
    ...
    RAC_LOG_DEBUG(LOG_CAT,
                  "No chat template in model, using manual formatting");
  }
...
void configure_sampler(LlamaCppVLMBackend *backend,
  ...
  sampler_params.no_perf =
      true; // Disable perf tracking (consistent with LLM backend)
 ...
 */
static VLMModelType
resolve_effective_model_type(VLMModelType detected,
                             const rac_vlm_options_t *options) {
...
rac_result_t prepare_vlm_context(LlamaCppVLMBackend *backend,
  ...
  VLMModelType effective_model_type =
      resolve_effective_model_type(backend->model_type, options);
  ...
  if (backend->mtmd_ctx && bitmap) {
    ...
    if (tokenize_result != 0) {
      RAC_LOG_ERROR(LOG_CAT, "Failed to tokenize prompt with image: %d",
                    tokenize_result);
...
rac_result_t rac_vlm_llamacpp_create(const char *model_path,
...
rac_result_t
rac_vlm_llamacpp_load_model(rac_handle_t handle, const char *model_path,
                            const char *mmproj_path,
                            const rac_vlm_llamacpp_config_t *config) {
  ...
  ctx_params.n_batch = backend->config.batch_size > 0
                           ? backend->config.batch_size
                           : kDefaultBatchSize;
...
rac_result_t rac_vlm_llamacpp_process(rac_handle_t handle,
                                      const rac_vlm_image_t *image,
                                      const char *prompt,
                                      const rac_vlm_options_t *options,
  ...
  // evaluate
  rac_result_t prep_result =
      prepare_vlm_context(backend, image, prompt, options);
  if (prep_result != RAC_SUCCESS) {
    return prep_result;
  }

  // Generate response (batch mode — accumulate all tokens)
  const int max_tokens = (options && options->max_tokens > 0)
                             ? options->max_tokens
                             : kDefaultMaxTokens;
  std::string response;
  response.reserve(
      kDefaultMaxTokens); // Typical VLM responses are a few hundred tokens
...
rac_result_t rac_vlm_llamacpp_process_stream(
...
rac_result_t rac_vlm_llamacpp_get_model_info(rac_handle_t handle,
                                             char **out_json) {
  if (!handle || !out_json) {
    return RAC_ERROR_NULL_POINTER;
  }
