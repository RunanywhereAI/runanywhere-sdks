...
# bindings/swift/scripts/build-core-xcframework.sh, then packages the
# resulting `.xcframework` bundles into the versioned
#      `core/dist/packages/<Framework>-ios-v<version>.zip`
...
XCFRAMEWORK_SCRIPT="${REPO_ROOT}/bindings/swift/scripts/build-core-xcframework.sh"
if [ ! -x "${XCFRAMEWORK_SCRIPT}" ]; then
    echo "error: ${XCFRAMEWORK_SCRIPT} not found or not executable" >&2
    exit 1
fi

echo "▶ Delegating iOS/macOS xcframework build to bindings/swift/scripts/build-core-xcframework.sh"
...
SRC_DIR="${REPO_ROOT}/bindings/swift/Binaries"
...
xcframework_names=(
    ...
    # (RABackendNeuRT-ios-v<version>.zip), so external SPM consumers 404 on
    # `swift package resolve` unless this archive is published. build-core-xcframework.sh
    # already emits RABackendNeuRT.xcframework (Apple-only, RAC_BACKEND_NEURT=ON).
    RABackendNeuRT
    RABackendMLX
