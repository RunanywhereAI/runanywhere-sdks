...
rac_result_t dummy_llm_stream(void* impl, const char*, const rac_llm_options_t* options,
    ...
    if (g_dummy_llm_block_stream.load(std::memory_order_acquire)) {
        ...
        if (g_dummy_llm_callback_after_cancel.load(std::memory_order_acquire)) {
            (void)callback("provider-late-token", RAC_FALSE, nullptr, /*tokens_in_delta*/ 1, user_data);
        }
    ...
    if (!g_dummy_llm_stream_response.empty()) {
        if (callback(g_dummy_llm_stream_response.c_str(), RAC_FALSE, nullptr, /*tokens_in_delta*/ 1, user_data) ==
            RAC_TRUE) {
            return RAC_SUCCESS;
        }
    ...
    if (callback("mock ", RAC_FALSE, nullptr, /*tokens_in_delta*/ 1, user_data) != RAC_TRUE)
        return RAC_ERROR_CANCELLED;
    if (callback("answer", RAC_FALSE, nullptr, /*tokens_in_delta*/ 1, user_data) != RAC_TRUE)
        return RAC_ERROR_CANCELLED;
    return RAC_SUCCESS;
}
