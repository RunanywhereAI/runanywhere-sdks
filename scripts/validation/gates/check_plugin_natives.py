...
Exit 0 when every plugin found is routable, 1 otherwise. Finding no plugins at
all is fine (a package may legitimately ship no native for this platform).
"""
...
#
# Only unconditional symbols belong here. `g_onnx_embeddings_ops` is gated on
# RAC_BACKEND_RAG and `g_llamacpp_{vlm,rerank,embeddings}_ops` vary by build, so
# requiring them would fail honest builds. One required symbol per backend is
# enough to separate routable from stub; this is a stub detector, not a feature
# inventory.
# ---------------------------------------------------------------------------
REQUIRED_OPS: dict[str, tuple[str, ...]] = {
    "sherpa": ("g_sherpa_stt_ops", "g_sherpa_tts_ops", "g_sherpa_vad_ops"),
    "llamacpp": ("g_llamacpp_ops",),
    "onnx": ("g_onnx_segmentation_ops", "g_onnx_diarization_ops"),
...
def main() -> int:
        ...
        for archive in args.tarball:
            ...
            with tarfile.open(archive, "r:gz") as bundle:
                members = [m for m in bundle.getmembers()
                           if m.isfile() and PLUGIN_NAME_RE.match(Path(m.name).name)]
                for member in members:
                    # Flatten: extract by name into dest, preserving the leaf.
    ...
    if findings:
        print("\nERROR: these plugins are stubs and must not be published:")
