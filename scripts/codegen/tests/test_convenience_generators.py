...
#
#   1. Builds a FileDescriptorSet covering scripts/codegen/tests/fixtures/
...
#      scripts/codegen/tests/golden/{swift,kotlin,dart,ts}.expected.
#
...
# entire scripts/codegen/ tree into a tempdir sandbox and populate
...
#   python3 scripts/codegen/tests/test_convenience_generators.py --update-golden
#
#   # Verify (CI / pre-commit):
#   python3 scripts/codegen/tests/test_convenience_generators.py
#
...
CODEGEN_DIR = REPO_ROOT / "scripts" / "codegen"
...
def run_generator(generator_script: str, sandbox: Path) -> tuple[int, str, str]:
    ...
    script_path = sandbox / "scripts" / "codegen" / generator_script
    proc = subprocess.run(
...
# Proto field names from scripts/codegen/tests/fixtures/test_options.proto.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Smoke test for scripts/codegen convenience generators "
                    "(pass2-syn-056).",
    )
    parser.add_argument(
        "--update-golden",
