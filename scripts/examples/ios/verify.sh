...
REPO_ROOT="${RAC_ROOT}"
APP_ROOT="${REPO_ROOT}/examples/ios/RunAnywhereAI"
...
check_xcframeworks() {
    ...
    if [ "${missing}" -ne 0 ] && [ "${ALLOW_MISSING_NATIVE_ARTIFACTS:-0}" != "1" ]; then
        echo "error: local XCFrameworks are missing. Run REFRESH_NATIVE=1 bash scripts/examples/ios/verify.sh to build them." >&2
        exit 1
    fi
}
