...
#
# scripts/build/ios-xcframework.sh builds / packages the native xcframeworks
...
#
# USAGE:
#   ./scripts/examples/ios/build-and-run.sh [target] [options]
#
# TARGETS:
...
#
# EXAMPLES:
#   ./scripts/examples/ios/build-and-run.sh device                     # Run app
#   ./scripts/examples/ios/build-and-run.sh device --build-all --local # Full local build
#   ./scripts/examples/ios/build-and-run.sh simulator --build-commons  # Rebuild commons
#   ./scripts/examples/ios/build-and-run.sh --build-all --skip-app     # Build SDK only

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/../../lib/common.sh"
WORKSPACE_ROOT="${RAC_ROOT}"

# Project directories
COMMONS_DIR="$WORKSPACE_ROOT/sdk/runanywhere-commons"
SWIFT_SDK_DIR="$WORKSPACE_ROOT/sdk/runanywhere-swift"
APP_DIR="$WORKSPACE_ROOT/examples/ios/RunAnywhereAI"

# Build scripts
# The canonical native build entrypoint is scripts/build/ios-xcframework.sh,
# which builds / packages all xcframeworks into sdk/runanywhere-swift/Binaries/.
# Both build steps delegate to scripts/build/ios-xcframework.sh.
COMMONS_BUILD_SCRIPT="$WORKSPACE_ROOT/scripts/build/ios-xcframework.sh"
SWIFT_BUILD_SCRIPT="$WORKSPACE_ROOT/scripts/build/ios-xcframework.sh"
...
build_commons() {
    ...
    log_step "Running: ios-xcframework.sh $FLAGS"
