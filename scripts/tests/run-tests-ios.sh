#!/bin/bash
# iOS build verification + macOS native test execution. See --help for options.
#
# Default mode cross-compiles for iOS Simulator arm64 to verify no iOS-specific
# compile/link errors. iOS Simulator can't run plain C++ executables (requires
# app bundles), so --run builds and runs natively on macOS, which is
# functionally equivalent for the C++ backend tests.
...
IOS_BUILD_DIR="${COMMONS_ROOT}/build/ios-test"
MACOS_BUILD_DIR="${COMMONS_ROOT}/build/test"
...
TOOLCHAIN_FILE="${COMMONS_ROOT}/cmake/ios.toolchain.cmake"
...
cmake -B "${IOS_BUILD_DIR}" -S "${COMMONS_ROOT}" \
...
cmake -B "${MACOS_BUILD_DIR}" -S "${COMMONS_ROOT}" \
    -DRAC_BUILD_TESTS=ON \
    -DRAC_BUILD_BACKENDS=ON \
    -DCMAKE_BUILD_TYPE=Debug
