#!/bin/bash
# Download models for integration tests. See --help for options.
...
# Verify a downloaded file is not an HTML redirect page (Git LFS issue).
verify_not_html() {
    local file="$1"
    if [ -f "$file" ]; then
        local first_bytes
...
# Whisper Tiny EN (~150MB, tar.bz2 via Sherpa-ONNX)
...
# openWakeWord (~20MB total: embedding + melspec + hey_jarvis)
