...
# Builds the same image as run-real-inference-linux.sh (with RAC_BUILD_CLI=ON)
# and exercises: modelless smoke, hermetic pull/rm (loopback HTTP, no network),
# and real inference (tts->stt roundtrip, vad, LLM one-shot, voice turn,
# serve /health). Model expectations match download-test-models.sh layout,
# mounted at /models; rcli's own storage uses a scratch home inside the
# container.
