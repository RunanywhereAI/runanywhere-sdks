...
#
# ios-xcframework.sh — wraps the ios-device, ios-simulator, and
...
#                            the `scripts/release/swift-binaries.sh DRY_RUN=1` path.
set -euo pipefail
...
if [ "$(uname -s)" != "Darwin" ]; then
    die "ios-xcframework.sh only runs on macOS"
fi

DRY_RUN="${DRY_RUN:-0}"
RAC_BACKEND_ONNX="${RAC_BACKEND_ONNX:-ON}"
COMMONS_HEADERS="${COMMONS_DIR}/include"
STAGING_DIR="${COMMONS_DIR}/build/ios-xcframework-staging"
...
merge_commons_slice() {
    ...
    local inputs=(
        "${build_root}/${slice_dir}/librac_commons.a"
...
Run this first (one-time, per checkout):
  ./scripts/build/deps/download-onnx.sh
...
Run this first (one-time, per checkout):
  ./scripts/build/deps/download-sherpa-onnx.sh
...
# CMake presets live in the commons dir (the CMake root); cmake --preset
# resolves CMakePresets.json from the cwd.
cd "${COMMONS_DIR}"
...
# ────────────────────────────────────────────────────────────────────────────
DEV_BIN="${COMMONS_DIR}/build/ios-device"
SIM_BIN="${COMMONS_DIR}/build/ios-simulator"
MAC_BIN="${COMMONS_DIR}/build/macos-release"
...
sync_react_native_frameworks() {
    ...
    # package-swift.sh --natives-from staging and the new podspec contract).
