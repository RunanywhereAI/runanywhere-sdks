...
#
# swift-binaries.sh — builds + zips + checksums all Swift binary
...
# not at the repo root — kept consistent with scripts/build/deps/
# download-onnx.sh and the FetchONNXRuntime.cmake module.
...
zip_target() {
    ...
    if [ ! -d "${xcf}" ]; then
        ...
        echo "       scripts/build/ios-xcframework.sh should have produced it." >&2
        exit 1
    fi
    echo "  ▶ ${zip}"
...
"${REPO_ROOT}/scripts/release/sync-checksums.sh" "${DEST}"
