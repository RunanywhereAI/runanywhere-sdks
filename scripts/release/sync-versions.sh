...
#   core/VERSION                        (single line)
#   core/VERSIONS                       (PROJECT_VERSION line)
...
#   bindings/electron/package.json + package-lock.json  (Electron/Node package version)
#   bindings/react-native/package.json              (root)
#   bindings/react-native/packages/*/package.json   (each package + first-party deps)
#   bindings/react-native backend Gradle fallbacks  (native archive version)
#   bindings/react-native/lerna.json                (fixed package train version)
#   bindings/react-native/.../SDKConstants.ts       (RN version constant)
#   bindings/flutter/packages/*/pubspec.yaml        (each version + core dep)
...
# truth = core/VERSION above), so there is no string literal
...
bump_line "${REPO_ROOT}/bindings/swift/Sources/RunAnywhere/Generated/Versions.swift" \
...
echo ""
echo ">> Kotlin SDK:"
KOTLIN_PROPS="${REPO_ROOT}/bindings/kotlin/gradle.properties"
...
bump_line "${REPO_ROOT}/bindings/kotlin/src/main/kotlin/com/runanywhere/sdk/foundation/constants/SDKConstants.kt" \
    'const val VERSION = "[^"]+"' \
...
echo ">> Web SDK:"
for pkg in \
    "${REPO_ROOT}/bindings/web/package.json" \
    "${REPO_ROOT}/bindings/web/packages/core/package.json" \
    "${REPO_ROOT}/bindings/web/packages/llamacpp/package.json" \
    "${REPO_ROOT}/bindings/web/packages/onnx/package.json"; do
...
bump_line "${REPO_ROOT}/bindings/web/packages/core/src/Foundation/Version.ts" \
...
echo ">> Electron SDK:"
for pkg in \
    "${REPO_ROOT}/bindings/electron/package.json" \
    "${REPO_ROOT}/bindings/electron/native/package.json" \
...
bump_npm_lock_root_version "${REPO_ROOT}/bindings/electron/package-lock.json"

# 5. React Native SDK packages
echo ""
echo ">> React Native SDK:"
for pkg in \
    "${REPO_ROOT}/bindings/react-native/package.json" \
    "${REPO_ROOT}/bindings/react-native/packages/core/package.json" \
    "${REPO_ROOT}/bindings/react-native/packages/llamacpp/package.json" \
    "${REPO_ROOT}/bindings/react-native/packages/mlx/package.json" \
    "${REPO_ROOT}/bindings/react-native/packages/onnx/package.json" \
    "${REPO_ROOT}/bindings/react-native/packages/qhexrt/package.json"; do
    bump_json_version "$pkg"
    bump_npm_proto_ts_dep "$pkg"
    bump_npm_core_dep "$pkg"
done
bump_line "${REPO_ROOT}/bindings/react-native/lerna.json" \
    '^  "version": "[^"]+"' \
    "  \"version\": \"${NEW_VERSION}\""
...
bump_line "${REPO_ROOT}/bindings/react-native/packages/core/src/Foundation/Constants/SDKConstants.ts" \
    "version: '[^']+'" \
    "version: '${NEW_VERSION}'"
bump_line "${REPO_ROOT}/bindings/react-native/packages/qhexrt/src/QHexRTProvider.ts" \
    "static readonly version = '[^']+'" \
    "static readonly version = '${NEW_VERSION}'"
for gradle_file in \
    "${REPO_ROOT}/bindings/react-native/packages/llamacpp/android/build.gradle" \
    "${REPO_ROOT}/bindings/react-native/packages/onnx/android/build.gradle"; do
...
echo ">> Flutter SDK:"
for pkg in \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere/pubspec.yaml" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_llamacpp/pubspec.yaml" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_mlx/pubspec.yaml" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_onnx/pubspec.yaml" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_qhexrt/pubspec.yaml"; do
...
for package_manifest in \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere/ios/runanywhere/Package.swift" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_llamacpp/ios/runanywhere_llamacpp/Package.swift" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_onnx/ios/runanywhere_onnx/Package.swift"; do
...
for gradle_file in \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere/android/build.gradle" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_llamacpp/android/build.gradle" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_onnx/android/build.gradle" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_qhexrt/android/build.gradle"; do
    bump_gradle_version "$gradle_file"
done
for binary_config in \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere/android/binary_config.gradle" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_llamacpp/android/binary_config.gradle" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_onnx/android/binary_config.gradle"; do
    bump_line "$binary_config" \
        'fallbackCoreVersion = "[^"]+"' \
        "fallbackCoreVersion = \"${NEW_VERSION}\""
done
bump_line "${REPO_ROOT}/bindings/flutter/packages/runanywhere/android/src/main/kotlin/ai/runanywhere/sdk/RunAnywherePlugin.kt" \
...
bump_line "${REPO_ROOT}/bindings/flutter/packages/runanywhere/ios/runanywhere/Sources/runanywhere/RunAnywherePlugin.swift" \
    'result\("[0-9]+\.[0-9]+\.[0-9]+([.-][A-Za-z0-9.-]+)?"\)' \
    "result(\"${NEW_VERSION}\")"

# Flutter QHexRT carries native metadata outside pubspec.yaml.
bump_line "${REPO_ROOT}/bindings/flutter/packages/runanywhere_qhexrt/lib/qhexrt.dart" \
    "static const String version = '[^']+'" \
    "static const String version = '${NEW_VERSION}'"
bump_line "${REPO_ROOT}/bindings/flutter/packages/runanywhere_qhexrt/android/src/main/kotlin/ai/runanywhere/sdk/qhexrt/QhexrtPlugin.kt" \
...
for podspec in \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere/ios/runanywhere.podspec" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_llamacpp/ios/runanywhere_llamacpp.podspec" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_mlx/ios/runanywhere_mlx.podspec" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_onnx/ios/runanywhere_onnx.podspec"; do
    bump_line "$podspec" \
...
echo ""
echo ">> Python SDK:"
bump_line "${REPO_ROOT}/bindings/python/pyproject.toml" \
    '^version = "[0-9]+\.[0-9]+\.[0-9]+([.-][A-Za-z0-9.-]+)?"' \
    "version = \"${NEW_VERSION}\""
bump_line "${REPO_ROOT}/bindings/python/runanywhere/__init__.py" \
...
for release_doc in \
    "${REPO_ROOT}/bindings/react-native/AGENTS.md" \
    "${REPO_ROOT}/bindings/react-native/packages/mlx/README.md" \
    "${REPO_ROOT}/bindings/flutter/AGENTS.md" \
    "${REPO_ROOT}/bindings/flutter/README.md" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere/README.md" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_llamacpp/README.md" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_mlx/README.md" \
    "${REPO_ROOT}/bindings/flutter/packages/runanywhere_onnx/README.md" \
...
echo "    git diff -- core/ bindings/ apps/ Package.swift"
echo "    corepack yarn install --mode=skip-build"
echo "    (cd bindings/react-native && corepack yarn install --mode=skip-build)"
echo "    (cd bindings/web && npm install --package-lock-only --ignore-scripts)"
...
echo ""
echo ">> After v${NEW_VERSION} is tagged, cut the Swift SPM distribution repo."
echo "   Swift consumers depend on RunanywhereAI/runanywhere-swift, not this repo;"
echo "   leave it stale and 'from: \"${NEW_VERSION}\"' resolves to nothing for them."
echo "    git clone https://github.com/RunanywhereAI/runanywhere-swift.git /tmp/ra-swift"
echo "    bindings/swift/scripts/sync-dist-repo.sh --zips <zip_dir> --tag /tmp/ra-swift"
echo "    git -C /tmp/ra-swift push origin main --follow-tags"
echo "   (scripts/validation/gates/check_swift_dist_repo_sync.sh fails every PR"
echo "    once v${NEW_VERSION} exists until that repo carries the ${NEW_VERSION} tag.)"
