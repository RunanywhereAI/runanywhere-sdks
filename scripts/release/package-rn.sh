...
usage() {
    cat <<'EOF'
Usage: scripts/release/package-rn.sh [--mode local|ci] [--natives-from PATH]
...
Outputs:
  sdk/runanywhere-react-native/dist/sdk-rn/*.tgz + .sha256 (one per workspace)
EOF
}

RN_ROOT="${RAC_ROOT}/sdk/runanywhere-react-native"
...
info "React Native SDK packaging (mode=${RAC_BUILD_MODE})"

if [ -n "$NATIVES_FROM" ]; then
    [ -d "$NATIVES_FROM" ] || die "--natives-from not found: $NATIVES_FROM"
    info "Staging natives from $NATIVES_FROM with explicit package ownership"
...
if [ -f "yarn.lock" ] || [ -f "${RAC_ROOT}/yarn.lock" ] || [ "$WANTS_YARN" = "1" ]; then
...
if [ -f "yarn.lock" ]; then
    YARN_CWD="$RN_ROOT"
elif [ -f "${RAC_ROOT}/yarn.lock" ]; then
    YARN_CWD="$RAC_ROOT"
fi

if [ -n "$YARN_CWD" ] && command -v yarn >/dev/null 2>&1; then
    info "yarn install (cwd=$YARN_CWD)"
    ...
    info "npm install --legacy-peer-deps (RAC_ALLOW_NPM_FALLBACK=1)"
    ...
    error "Yarn workspace install required but yarn/corepack is unavailable."
    error "Install Node 18+ with Corepack (or 'npm i -g corepack') so 'yarn@3.6.1'"
    error "can be activated, or set RAC_ALLOW_NPM_FALLBACK=1 to opt into the"
    error "legacy npm install path explicitly (NOT recommended for release builds — see comment above)."
    exit 1
fi

# Generate Nitro bindings if nitrogen is wired up
if [ "$HAS_YARN" = "1" ] && yarn run 2>/dev/null | grep -q "nitrogen"; then
    info "yarn core:nitrogen (if defined)"
...
if [ -x "${RAC_ROOT}/scripts/release/validate-artifact.sh" ]; then
    if [ "$RAC_BUILD_MODE" = "ci" ]; then
        "${RAC_ROOT}/scripts/release/validate-artifact.sh" "$DIST_DIR"/*.tgz 2>/dev/null
    else
        "${RAC_ROOT}/scripts/release/validate-artifact.sh" "$DIST_DIR"/*.tgz 2>/dev/null || true
    fi
fi
