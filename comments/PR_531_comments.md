...
| DR-86 | P1 | IDL regression prevention | The deprecated-surface gate covered first-party declarations emitted in Kotlin, Swift, Dart, TypeScript, and C/C++, but did not reject `[deprecated = true]` fields in the canonical protobuf IDL that would regenerate those compatibility members everywhere. | Fixed with a fail-closed `idl/**/*.proto` scan. Removed fields may retain numeric reservations, but a deprecated source declaration now fails the global source-validation gate before generation or merge. |
