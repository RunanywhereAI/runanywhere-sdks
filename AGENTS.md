...
| Web SDK (TypeScript) | `npm run build -w packages/core` (from `sdk/web/`) | N/A | `npm run typecheck -w packages/core` | `llamacpp` package has a pre-existing duplicate index signature TS error |
...
- **`local.properties`**: Auto-created at root, `sdk/kotlin/`, and `examples/android/RunAnywhereAI/` with `sdk.dir=/opt/android-sdk`.
...
export LD_LIBRARY_PATH="../../core/dist/linux/x86_64:../../core/third_party/sherpa-onnx-linux/lib"
