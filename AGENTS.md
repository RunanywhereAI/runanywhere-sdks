...
**Most generated bindings are NOT tracked.** A fresh clone has no Swift, TypeScript,
Dart, React Native or Python bindings until codegen runs. `./scripts/setup/setup.sh`
runs it first for exactly that reason; `./run codegen` runs it on demand.

| tree | tracked? | why |
|---|---|---|
| `core/src/generated/proto/` (76 files, ~336k lines) | **yes** | ~20 native CI jobs across macOS/iOS/Linux/Windows/Android×3/WASM/rcli compile this. protoc stamps `#if PROTOBUF_VERSION != 7035001` into the headers, so every one of those hosts would need protoc **35.1 exactly** — Homebrew and apt do not offer that, Windows has no bash to run the generator, and `RAC_REGENERATE_PROTO` defaults OFF by explicit design so release builds need no protoc at all. The cost is ~20 toolchain installs on 6 OS/arch targets; the benefit is a diff nobody reads. Not worth it. |
| `core/include/rac/rac_defaults_generated.h` | **yes** | Same C/C++ plane, same reasoning. It is also a shipped public header (`install(DIRECTORY include/)`, XCFramework `Headers/`), so it must exist at install time on all of those hosts. |
| `bindings/kotlin/.../sdk/generated/` (373 files) | **yes** | Build input only — it is compiled into the AAR and never shipped as source, so de-committing buys no publish-safety. It would instead force protoc 35.1 + the wire-compiler JAR + a Python protobuf runtime onto every Gradle host, including Windows (no bash) and JitPack. |
| `bindings/swift/Sources/RunAnywhere/Generated/` | no | ships in the SwiftPM tag — generated by `sync-dist-repo.sh` |
| `bindings/proto-ts/src/` and `dist/` | no | `dist` ships in 7 npm packages — generated by each `package-sdk.sh` |
| `bindings/flutter/packages/runanywhere/lib/generated/` | no | ships in the pub package |
| the two `RADefaultsPool.kt` under flutter/ and react-native/ | no | ship inside the pub / npm packages |
| `bindings/python/runanywhere/_proto/`, `_generated_{errors,defaults}.py` | no | ship in the sdist + wheel |
