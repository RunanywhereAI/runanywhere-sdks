...
if ([string]::IsNullOrWhiteSpace($Version)) {
    $Version = (Get-Content (Join-Path $RepoRoot "core\VERSION") -Raw).Trim()
}
$Version = $Version.TrimStart("v")

$BinaryCandidates = @(
    (Join-Path $BuildDir "apps\rcli\rcli.exe"),
    (Join-Path $BuildDir "apps\rcli\Release\rcli.exe")
)
...
$SherpaLib = Join-Path $RepoRoot "core\third_party\sherpa-onnx-windows\lib"
