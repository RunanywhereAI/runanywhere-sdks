...
#
# Output:
#   bindings/proto-ts/src/convenience/<base>_convenience.ts (one file per
...
def _ts_proto_function_prefix(symbol: str) -> str:
    ...
    from a type symbol. Verified at bindings/proto-ts/src/stt_options.ts:58
...
def _proto_field_to_ts_camel(name: str) -> str:
    ...
    bindings/proto-ts/src/stt_options.ts:266 — `modelId`, not `modelID`).
    """
    parts = name.split("_")
    if not parts:
        return name
...
def _ts_zero_literal(
    ...
    in bindings/proto-ts/src/stt_options.ts:540, where enum-typed fields
...
def main() -> int:
    ...
    out_dir = repo_root / "bindings" / "proto-ts" / "src" / "convenience"
