...
| 2 | Auth window drift fixed (Kotlin 5-min → C++ 60-sec) + auth client gone | **PARTIAL** | The 5-min vs 60-sec drift is **FIXED** (`REFRESH_WINDOW_MS = 60L * 1000L` in `CppBridgeAuth.kt` L65). The "auth client gone" half remains: `CppBridgeAuth.kt` is **still 182 LOC** with HTTP/JSON state bookkeeping. Needs `rac_auth_*` JNI thunks to delete; tracked in `v2_remaining_work.md` "v2.1 follow-ups". |
