...
| LLM component LoRA JSON: `rac_llm_component_get_lora_info` (`out_json`) and `rac_llm_component_check_lora_compat` (`out_error`) in `features/llm/rac_llm_component.h` | `delete after SDK migration` | LoRA proto APIs (`rac_lora_*_proto`) own the public surface. Keep JSON helpers as internal until callers migrate. |
| Tool-calling JSON helpers in `features/llm/rac_tool_calling.h` (`rac_tool_call_normalize_json`, `rac_tool_call_definitions_to_json`, `rac_tool_call_result_to_json`, JSON-flavored validate/format APIs ending in `_json`) | `delete after SDK migration` for SDK-facing helpers; `internal` for parsers | Use `rac_tool_call_parse_proto`/`rac_tool_call_validate_proto`/`rac_tool_call_format_prompt_proto` over generated `ToolParseRequest`/`ToolParseResult`/`ToolCallValidationRequest`/`ToolCallValidationResult`/`ToolPromptFormatRequest`/`ToolPromptFormatResult` bytes. |
| Structured-output JSON helpers in `features/llm/rac_llm_structured_output.h` (`rac_structured_output_extract_json`, `rac_structured_output_parse`, `rac_structured_output_find_*`, `rac_structured_output_prepare_prompt`, `rac_structured_output_get_system_prompt`, `rac_structured_output_validate`) and the `rac_structured_output_parse_result_t` struct | `delete after SDK migration` for SDK-facing entry points; `internal` for parser primitives | Use `rac_structured_output_parse_proto`/`generate_proto`/`prepare_prompt_proto`/`validate_proto`/`generate_stream_proto` over generated `StructuredOutputRequest`/`StructuredOutputResult`/`StructuredOutputValidation`/`StructuredOutputStreamEvent` bytes. |
...
| `runanywhere_commons_jni.cpp` (2773–2980) | `racModelRegistrySave`, `racModelRegistryGet`, `racModelRegistryGetAll`, `racModelRegistryGetDownloaded`, `racModelRegistryRemove`, `racModelRegistryUpdateDownloadStatus` | Builds `rac_model_info_t` from per-field JNI args, returns JSON arrays | Delete; replace with proto variants (`racModelRegistry*Proto`) | `gaps/gaps/inconsistencies/kotlin.md` (registry) |
| `runanywhere_commons_jni.cpp` (4573–4732) | `racToolCallFormatPromptJson`, `racToolCallFormatPromptJsonWithFormat`, `racToolCallFormatPromptJsonWithFormatName`, `racToolCallBuildInitialPrompt`, `racToolCallBuildFollowupPrompt`, `racToolCallNormalizeJson` | JSON in / JSON out using `rac_tool_call_*_json` | Delete; replace with `racToolCallParseProto` / `racToolCallFormatPromptProto` (no SDK caller in Kotlin today — these are dead code on the JVM/Android side, but still link) | `gaps/gaps/inconsistencies/kotlin.md` |
...
| `runanywhere_commons_jni.cpp` (7732–7785) | `racVadComponentGetStatistics`, `racModelRegistryFetchAssignments` | Build raw JSON from struct values | Replace with proto: `racVadComponentGetStatisticsProto` already exists; `racModelRegistryFetchAssignments` needs a proto wrapper around assignment refresh. Latter is on commons backlog. | `gaps/gaps/inconsistencies/kotlin.md` |
