...
> | # | Item | Closes | Effort |
> |---|------|--------|--------|
> | ~~v2.1-1~~ | ~~Wire `VoiceSessionEvent` to use the codegen'd proto type in 5 SDKs~~ **DONE in v2.1-1 PR** (5 commits `540deec2` → `64661d07`): Swift full migration (`VoiceSessionEvent.from(_:)` maps all 8 proto payloads to the legacy enum); Kotlin/Dart/RN scaffolds (`@Deprecated` sealed classes + mapper stubs awaiting per-SDK body in v2.1-1b/c/d follow-ups); Web trivially satisfied (never had a parallel type). GAP 09 #6 flipped PARTIAL → OK. | GAP 09 #6 | DONE (per-SDK runtime rewires = v2.1-1a/b/c/d/e follow-ups) |
...
> | ~~v2.1-4~~ | ~~Implement 16 `rac_auth_*` JNI thunks~~ **DONE in v2.1 quick-wins Item 4** (4 commits `bd7da766` → `52e9e48d`): 16 thunks added to runanywhere_commons_jni.cpp, 16 declarations added to RunAnywhereBridge.kt, CppBridgeAuth.kt shrunk 182 → 152 LOC with state + refresh-window math + JSON parsing moved to native. HTTP transport stays in Kotlin (KeyStore-backed secure storage callbacks for rac_auth_init are the v2.1-2 follow-up). | GAP 08 #2 | DONE |
