...
The MLX runtime target needs the Swift 6.2 toolchain, so Xcode 26 is a hard floor for a full build even though `swift-tools-version` is 5.9. Canonical toolchain pins live in `core/VERSIONS`.
...
| `bindings/swift/Package.swift` | SDK development. References the git-ignored local `Binaries/` directory. |
...
# Build and stage the native XCFrameworks into bindings/swift/Binaries/
...
- [Commons development guide](../../../core/docs/DEVELOPMENT.md) — the C core the SDK binds to
