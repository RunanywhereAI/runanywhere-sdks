...
- Xcode required: 15+
- Current SDK version: `0.20.29` (`bindings/swift/VERSION:1`)
...
File: `bindings/swift/Sources/RunAnywhere/Public/RunAnywhere.swift` (~404 LOC)
...
File: `bindings/swift/Sources/RunAnywhere/Public/Configuration/SDKEnvironment.swift` (~239 LOC)
...
File: `bindings/swift/Sources/RunAnywhere/Public/Events/EventBus.swift` (~91 LOC)
...
**File:** `bindings/swift/Sources/RunAnywhere/Public/Extensions/LLM/RunAnywhere+TextGeneration.swift` (LOC: 76)
...
**Path prefix:** `bindings/swift/Sources/RunAnywhere/Foundation/Bridge/Extensions/`
...
|---|---|
| `version` | `"0.20.29"` (line 14) — kept in sync with `core/VERSION` |
...
`generate_swift.sh` invokes `protoc --swift_out="Visibility=Public:<OUT_DIR>"` against every `.proto` file in `idl/`. Output: `bindings/swift/Sources/RunAnywhere/Generated/`.
...
### §13.4 Plugin ABI v9 — `rac_engine_vtable_t`
...
- 10 active primitive slots: `llm_ops`, `stt_ops`, `tts_ops`, `vad_ops`, `embedding_ops`, `vlm_ops`, `diffusion_ops`, `diarization_ops`, `segmentation_ops`, `rerank_ops`. NULL means the engine does not serve that primitive. (`rerank_ops` was revived at wire value 11 in ABI v8; the original wire value 6 remains retired.)
- One optional `get_stream_token_counts` callback. NULL means commons estimates token totals and marks them as estimated.
- 6 `const void* reserved_slot_N` fields.

`metadata.abi_version` must equal `RAC_PLUGIN_API_VERSION` (currently `9u`); mismatch causes `RAC_ERROR_ABI_VERSION_MISMATCH`. On iOS, `RAC_STATIC_PLUGINS=ON` forces static registration via `RAC_STATIC_PLUGIN_REGISTER(name)` + `-force_load`; no `dlopen`.
...
The file at `bindings/swift/Package.swift` is the **local development manifest**. A second `Package.swift` exists at the repository root for external SPM consumers; that root-level file downloads XCFrameworks from GitHub releases. The local manifest references `Binaries/` directly.
...
All five XCFrameworks live in `bindings/swift/Binaries/` (git-ignored):
...
Tests require the `Binaries/` XCFrameworks (built locally via `bindings/swift/scripts/build-core-xcframework.sh` or copied from CI artifacts).
...
- **`RAC_PLUGIN_API_VERSION`** — C macro currently `9u`. `rac_engine_vtable_t.metadata.abi_version` must equal this; mismatch causes `RAC_ERROR_ABI_VERSION_MISMATCH`.
