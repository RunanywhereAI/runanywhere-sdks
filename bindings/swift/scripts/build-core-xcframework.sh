...
#
#   bindings/swift/Binaries/RACommons.xcframework
#   bindings/swift/Binaries/RABackendLLAMACPP.xcframework
#   bindings/swift/Binaries/RABackendONNX.xcframework          (skipped if RAC_BACKEND_ONNX=OFF)
#   bindings/swift/Binaries/RABackendSherpa.xcframework
#   bindings/swift/Binaries/RABackendNeuRT.xcframework        (Apple-only; NeuRT — ANE LLM + CoreML diffusion)
#   bindings/swift/Binaries/RABackendMLX.xcframework           (Apple-only, skipped if RAC_BACKEND_MLX=OFF)
#   bindings/swift/Binaries/RunAnywhereMLXRuntime.xcframework  (Apple-only, skipped if RAC_BACKEND_MLX=OFF)
#   bindings/swift/Binaries/RunAnywhereMLXMetal.xcframework    (platform-selected Metal resource framework)
#   bindings/swift/Binaries/RunAnywhereMLXRuntimeResources/   (Swift resource bundles/notices)
#
...
DEST="${REPO_ROOT}/bindings/swift/Binaries"
ARCHIVE_MEMBER_NORMALIZER="${REPO_ROOT}/bindings/swift/scripts/normalize-static-archive.py"
...
MLX_RUNTIME_SOURCE="${REPO_ROOT}/bindings/swift/Sources/MLXRuntimeDistribution"
...
merge_commons_slice() {
    ...
    local inputs=(
        "${build_root}/core/${slice_dir}/librac_commons.a"
...
merge_commons_macos_slice() {
    ...
    local inputs=(
        "${build_root}/core/librac_commons.a"
...
merge_onnx_backend_slice() {
    ...
    if [ "${RAC_BACKEND_SHERPA}" = "OFF" ]; then
        ...
        local sherpa_dir
        if [ "${slice_dir}" = "Release-iphoneos" ]; then
            sherpa_dir="${REPO_ROOT}/core/third_party/sherpa-onnx-ios/sherpa-onnx.xcframework/ios-arm64"
        else
            sherpa_dir="${REPO_ROOT}/core/third_party/sherpa-onnx-ios/sherpa-onnx.xcframework/ios-arm64_x86_64-simulator"
        fi
...
# ────────────────────────────────────────────────────────────────────────────
IOS_ONNXRT="${REPO_ROOT}/core/third_party/onnxruntime-ios/onnxruntime.xcframework"
...
Run this first (one-time, per checkout):
  ./core/scripts/ios/download-onnx.sh
...
# ────────────────────────────────────────────────────────────────────────────
IOS_SHERPA="${REPO_ROOT}/core/third_party/sherpa-onnx-ios/sherpa-onnx.xcframework"
...
Run this first (one-time, per checkout):
  ./core/scripts/ios/download-sherpa-onnx.sh
...
MACOS_SHERPA_ROOT="${REPO_ROOT}/core/third_party/sherpa-onnx-macos"
MACOS_SHERPA_STATIC_DEPS=(
...
if [ "${RAC_BACKEND_ONNX}" = "ON" ] && [ "${DRY_RUN}" != "1" ]; then
    ...
    for required in "${macos_required[@]}"; do
        if [ ! -f "${required}" ]; then
            ...
            echo "       Run ./core/scripts/macos/download-sherpa-onnx.sh" >&2
            exit 1
        fi
...
cmake_extra=(
    ...
    # Force the complete requested engine set on every configure. These preset
    # build directories are intentionally reusable, so relying on option
    # defaults can otherwise preserve an earlier OFF cache entry while the
    # packaging phase still expects the corresponding archive.
...
sync_react_native_frameworks() {
    local rn_root="${REPO_ROOT}/bindings/react-native/packages"
    if [ ! -d "${rn_root}" ]; then
        return
    fi
...
sync_flutter_frameworks() {
    local flutter_root="${REPO_ROOT}/bindings/flutter/packages"
