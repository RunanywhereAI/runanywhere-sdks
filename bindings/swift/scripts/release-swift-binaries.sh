...
#   2. core/third_party/onnxruntime-ios/onnxruntime.xcframework
#      present. Run:
...
#
# Usage:
#   bindings/swift/scripts/release-swift-binaries.sh <version>          # builds + checksums
#   bindings/swift/scripts/release-swift-binaries.sh 0.20.0
#
...
#   DRY_RUN=1 bindings/swift/scripts/release-swift-binaries.sh 0.20.0
#
# Outputs:
...
# not at the repo root — kept consistent with core/
...
BINARIES_DIR="${REPO_ROOT}/bindings/swift/Binaries"
...
zip_target() {
    ...
    echo "  ▶ ${zip}"
    "${REPO_ROOT}/bindings/swift/scripts/create-reproducible-xcframework-zip.sh" \
        "${xcf}" "${zip}"
}
...
zip_resources() {
    ...
    echo "  ▶ ${zip}"
    "${REPO_ROOT}/bindings/swift/scripts/create-reproducible-directory-zip.sh" \
        "${resources}" "${zip}"
}
...
"${REPO_ROOT}/bindings/swift/scripts/sync-checksums.sh" "${DEST}"
...
echo "       git diff Package.swift bindings/flutter/packages/runanywhere_mlx/ios/runanywhere_mlx.podspec"
echo "  2. Verify checksums and the local-native Swift build:"
echo "       bindings/swift/scripts/sync-checksums.sh --check ${DEST}"
...
echo "       git add Package.swift bindings/flutter/packages/runanywhere_mlx/ios/runanywhere_mlx.podspec && \\"
