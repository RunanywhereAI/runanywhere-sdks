...
const ONNX_SHERPA_WASM = path.join(REPO_ROOT, 'bindings/web/packages/onnx/wasm/racommons-onnx-sherpa.wasm');
...
async function registerOnnx(page) {
  ...
  try {
    await page.evaluate(async ({ repoRoot }) => {
      const onnxPath = `/@fs${repoRoot}/bindings/web/packages/onnx/src/index.ts`;
...
async function registerLlamaCpp(page) {
  try {
    await page.evaluate(async ({ repoRoot }) => {
      const llamaPath = `/@fs${repoRoot}/bindings/web/packages/llamacpp/src/index.ts`;
