...
#
# Stages:
#   core/third_party/onnxruntime-wasm/
...
# Release docs: bindings/web/docs/ONNX_WEBGPU.md
#
# Usage:
#   npm --prefix bindings/web run vendor:wasm:onnxruntime
# Then:
#   npm --prefix bindings/web run build:wasm -- --onnx --clean
set -euo pipefail
...
DEST_DIR="${REPO_ROOT}/core/third_party/onnxruntime-wasm"
...
canonical_llvm_nm() {
  # setup-emsdk.sh installs beside wasm/, at bindings/web/emsdk. Keep
...
require_canonical_emscripten() {
  if ! command -v emcc >/dev/null 2>&1 || ! command -v emcmake >/dev/null 2>&1; then
    ...
    echo "Run: bindings/web/wasm/scripts/setup-emsdk.sh" >&2
    exit 1
  fi
