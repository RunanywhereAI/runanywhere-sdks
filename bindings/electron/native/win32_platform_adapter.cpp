 ...
 *
 * Rejects path traversal characters, reserved device names (CON, NUL, etc.),
 * absolute/drive paths, ADS colons, and trailing dots/spaces.
 *
 * @param key Key identifier string to validate.
 * @return true If the key is non-null, non-empty, and valid under Windows filename rules.
 * @return false If the key is null, empty, or violates filename constraints.
 */
