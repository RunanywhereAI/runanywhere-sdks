...
import type { Catalog, RagSession, RunAnywhereApi } from '../../dist';
...
async function withSdk(run: (sdk: RunAnywhereApi) => Promise<void>): Promise<void> {
  clearCatalog();
...
async function withCorpus(
  sdk: RunAnywhereApi,
  run: (session: RagSession) => Promise<void>
): Promise<void> {
...
test('rag: a streamed query emits retrieval, then tokens, then a completed result',
  ...
  async () => {
    await withSdk(async (sdk) => {
      await withCorpus(sdk, async (session) => {
        const types: string[] = [];
        let completed: Awaited<ReturnType<RagSession['query']>> | null = null;
