...
import { toAsyncIterable } from '../../dist/stream';
import { isSDKException } from '../../dist/errors';
...
// `toAsyncIterable` declares a string token because that is what every real
// caller pushes, but the adapter itself never validates or converts — the
// pass-through test below is the assertion that it does not.
type TokenPush = (t: string) => void;
const pushAny = (onToken: TokenPush): ((v: unknown) => void) =>
  onToken as unknown as (v: unknown) => void;
...
test('rejection: buffered tokens delivered before the throw', async () => {
  ...
  await assert.rejects(() => iter.next(), (e: unknown) => {
...
test('rejection while consumer is already waiting in next()', async () => {
  ...
  await assert.rejects(() => iter.next(), (e: unknown) => {
    assert.equal((e as Error).message, 'late-boom');
    return true;
  });
});
...
test('after rejection, further next() calls keep throwing the same error', async () => {
  ...
  await assert.rejects(() => iter.next(), (e: unknown) => {
    assert.equal((e as Error).message, 'sticky');
    return true;
  });
  await assert.rejects(() => iter.next(), (e: unknown) => {
    assert.equal((e as Error).message, 'sticky');
...
test('token pushed AFTER the promise resolves (done already true) is still yielded', async () => {
  ...
  assert.ok(cb, 'start() handed over its callback synchronously');
  cb('late');
...
test('tokens pass through with no coercion (non-string values preserved by identity)', async () => {
  ...
  assert.equal(got[0] as unknown, 5);
  assert.equal(got[1] as unknown, null);
  assert.equal(got[2] as unknown, obj); // same reference, not a copy
});
