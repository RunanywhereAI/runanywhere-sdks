...
import { speakableText } from '../../dist/speech';

// The signature says string, but a renderer can hand this an undefined message
// body, and the guard is what keeps that from throwing mid-utterance.
...
test('empty and nullish input is safe', () => {
  ...
  assert.equal(speakableText(notAString(undefined)), '');
});
