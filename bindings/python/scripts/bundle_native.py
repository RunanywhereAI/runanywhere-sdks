...
to a sensible ``build/*/bindings/python/native/Release`` path. If ``_core`` is not in
...
# Repo root holds the shared ``build/`` tree (runanywhere-python is one SDK under ``bindings/``).
...
def _default_build_dir() -> Path:
    ...
    Mirrors the Electron script's ``build/windows-release/bindings/<sdk>/native/Release`` default,
    ...
    """
    return (
        REPO_ROOT / "build" / "windows-release" / "bindings"
        / "python" / "native" / "Release"
    )
...
def _extra_sidecar_roots() -> list[Path]:
    ...
    ``core/third_party/sherpa-onnx-<os>/lib`` (downloaded prebuilts),
    not under ``build/``, so the sidecar search must include them.
    """
    tp = REPO_ROOT / "core" / "third_party"
    roots: list[Path] = []
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "build/windows-release/bindings/python/native/Release). If _core is not "
