#!/usr/bin/env bash
# =============================================================================
# bindings/react-native/scripts/package-sdk.sh
# =============================================================================
...
if [ -n "$NATIVES_FROM" ]; then
    ...
    if find "$NATIVES_FROM" -maxdepth 1 -type d -name '*.xcframework' -print -quit | grep -q .; then
        ...
        stage_ios \
            ...
            RunAnywhereMLXMetal
        stage_mlx_ios_resources
        stage_ios "$RN_ROOT/packages/onnx" RABackendONNX RABackendSherpa RABackendNeuRT
    fi
...
PUBLIC_RAC_HEADERS="$REPO_ROOT/core/include/rac"
CANONICAL_PRIVACY_MANIFEST="$REPO_ROOT/bindings/swift/Sources/RunAnywhere/PrivacyInfo.xcprivacy"
